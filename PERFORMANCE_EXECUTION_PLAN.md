...
Актуальний повний залишок після `7077a0c1` і порядок передачі простішій моделі:
[IMPLEMENTATION_HANDOFF_2026-09-07.md](docs/catalog-v2/IMPLEMENTATION_HANDOFF_2026-09-07.md).
Нижче збережено початковий план та історію; давні незавершені пункти звіряти
з handoff і поточним STATUS, не виконувати вже закриті виправлення повторно.
...
Додатковий результат T4: усі 15 163 підтримувані записи з 14 джерел пройшли
порівняння нормалізованих умов, збережених політик та побудованої проєкції на
відновленій тестовій базі після версійного виправлення чотирьох Ilmberger.
Вони залишаються NEEDS_REVIEW, але тепер мають scope=moto. 1 999 KW та 223 FI
досі потребують інтеграції канонічних політик; загальну готовність це блокує.
Списки варіантів фільтра тепер групуються в PostgreSQL без обмеження кількості
варіантів. Реальний SQL та відповідність року/кузова перевірені інтеграційним
тестом. Покриття публічних endpoints, змішаних legacy/canonical джерел та
продуктивність у середовищі Production залишаються окремими перевірками.

Наступний етап T4 зберігає докази KW/FI в імпорті: вихідний товар разом із
нормалізацією KW або окремим записом сумісності FI. Повторний імпорт перевіряє
версію/хеш, а характеристики вкладених варіантів прив'язуються через зовнішні ID.
Це підготовка до канонічних політик: самі 2 222 записи ще не пройшли публікацію
в нову модель фільтрів. Невідому марку KW не можна підміняти вигаданою або
відкидати відому модель/кузов, щоб спростити інтеграцію.
