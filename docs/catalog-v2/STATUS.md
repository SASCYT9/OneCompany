...
| C2-P2-002 | Mutation coordinator and outbox publisher         | In progress | Archive writer and scheduled recovery worker green; remaining admin writers pending |
...
| C2-P2-002 | Add mutation coordinator and transactional outbox publisher                            | In progress | P1 complete           | Product archive now commits audit + immutable revision + outbox atomically and triggers immediate bounded publication; remaining writers and failure drill remain |
...
- Atomic persistence, immutable-revision source loading, durable rebuild checkpoints, mutation coordination, and leased outbox processing exist. `/api/cron/shop-catalog` runs a bounded recovery batch every five minutes and rebuilds exclusively from the event's immutable revision. The soft-archive admin writer is the first adopted writer and schedules immediate publication after returning; full editor, bulk status, imports, pricing, and inventory adoption plus retry/dead-letter operational drills remain.
