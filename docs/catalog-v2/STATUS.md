...
| C2-P2-002 | Mutation coordinator and outbox publisher         | In progress | Full product editor + archive and recovery worker green; bulk/import/price/inventory writers pending |
...
| C2-P2-002 | Add mutation coordinator and transactional outbox publisher                            | In progress | P1 complete           | Full product editor and archive commit audit + immutable revision + outbox atomically, use optimistic version checks, and trigger immediate bounded publication; remaining writers and failure drill remain |
...
- Atomic persistence, immutable-revision source loading, durable rebuild checkpoints, mutation coordination, and leased outbox processing exist. `/api/cron/shop-catalog` runs a bounded recovery batch every five minutes and rebuilds exclusively from the event's immutable revision. The full product editor and soft-archive writer now schedule immediate publication after returning. Bulk status, imports, pricing, and inventory adoption plus retry/dead-letter operational drills remain.
