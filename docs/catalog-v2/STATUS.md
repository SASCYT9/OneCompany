...
The disposable PostgreSQL publication gate was rerun at `b757eefa`: all 44 migrations applied, 30 samples completed with p95 82.926 ms and p99 126.240 ms, one concurrent publication won, and the expected two global publications plus three receipts were recorded. No production database was read or written.

Commit `d7bb2166` bounds V2/SSR recommendation candidates to a canonical projection query and a maximum of 100 hydrated IDs, interleaves brands, preserves the legacy path only when the reader is off, and normalizes protocol-relative Shopify media URLs. Commit `139f3e08` rejects duplicate or orphaned provenance instead of silently collapsing it. New recommendation/image and provenance regressions pass locally; production reader activation remains gated on complete source evidence.

After these commits, the selected catalog/stock/pricing/recommendation/image suite is **422/422**, the source coverage suite is **6/6** (one intentionally skipped DB-dependent case), and the supported Node 22.14.0 direct Next build completed all 590 static pages. No production data or deployment was touched.

Commit `fc97f8ba` additionally requires complete durable clause coverage and rejects `UNKNOWN` dimensions before selector readiness. Commit `7d0bdb11` makes vehicle suggestions token-order independent while preserving correlated make/model clauses. Focused selector/suggestion regressions are **33/33**, and the final acceptance artifact is bound to `7d0bdb11`.

Commit `9b60ecc0` routes every projection query containing text through the native SQL matcher, restoring exact variant-SKU lookup for text-only searches. Commit `6cf69360` makes Catalog V2 filter transitions preserve the complete storefront query state (multi-brand, price, stock, scope, type/kind, strict and facet mode) while retaining canonical URL ordering and descendant reset behavior. The expanded regression suite is **425/425**; a fresh acceptance rerun is required after the final code commit.
