...
### 0. Напрямок: портал + Urban на нашому домені
- **Портал:** головний сайт One Company веде в існуючі магазини на Shopify (картки/посилання). Ці магазини лишаються на Shopify — інтеграція не потрібна, тільки навігація.
- **Urban:** один магазин (бренд Urban) має жити на нашому домені в складі нашого сайту. Дизайн повторюємо з референсу (готова Shopify-тема Urban) — переверстати в Next.js, бекенд наш (каталог, кошик, checkout за поточним планом). Це об’ємна робота, але час на реалізацію є.
- **Референс теми Urban:** розпакована тема лежить у `reference/urban-shopify-theme/` (експорт з Shopify). Використовувати для повторення layout, sections, стилів і компонентів. Див. `reference/urban-shopify-theme/README.md`.
- Усе нижче (фази, API, каталог, B2B/B2C тощо) стосується саме **Urban** (магазин на нашому стекі). Інші бренди — тільки посилання з порталу.
