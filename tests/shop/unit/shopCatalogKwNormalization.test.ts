...
test("single-make evidence resolves vehicles in multi-brand products", () => {
  ...
  const policy = buildKwCompatibilityPolicy("p3", normalized);
  assert.equal(policy.mode, "VEHICLE_SPECIFIC");
  assert.ok(policy.clauses.every((clause) => clause.verification === "VERIFIED"));
  assert.ok(
    policy.clauses.every((clause) =>
      clause.constraints.some(
        (constraint) => constraint.dimension === "engine" && constraint.state === "UNKNOWN"
      )
    )
  );
});
...
test("unresolved multi-brand correlations are quarantined instead of cross-joined", () => {
  const target = product("p4", [
    "brand:AUDI",
    "brand:VW",
    "veh:UNKNOWN (X1) 01/2020-",
    "eng:2.0 TSI",
  ]);
...
test("missing vehicle tags fall back to structured Shopify brand/model metafields", () => {
  ...
  row.metafields = [
    { id: "m1", namespace: "custom", key: "brand", value: '["BMW"]' },
    {
      ...
      namespace: "custom",
      key: "model",
      value: '["5 (G60, G90) 07/2023- | 5 Touring (G61, G99) 03/2024-"]',
    },
  ];
  const normalized = normalizeKwShopifyCatalog([row])[0]!;
  assert.equal(normalized.applications.length, 2);
  assert.deepEqual(
    normalized.applications.map(({ make, chassisCodes }) => ({ make, chassisCodes })),
    [
...
test("multi-brand make correlation can use explicit make+model evidence in the title", () => {
  const row = product("p6", [
    "brand:SKODA",
    "brand:VW",
    "veh:OCTAVIA IV (NX3) 01/2020-",
    "eng:2.0 TSI",
  ]);
...
test("KW fitment becomes OR clauses without cross-joining vehicle engines", () => {
  const rows = [product("p7", ["brand:BMW", "veh:3 (G20 G80) 11/2018-", "eng:320 i"])];
