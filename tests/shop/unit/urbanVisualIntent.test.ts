...
import {
  buildUrbanCollectionPhotoGallery,
...
} from "../../../src/lib/urbanVisualIntent";
...
function buildCollectionConfig(): UrbanCollectionPageConfig {
  return {
    ...
    gallery: {
      ...
      slides: [
        {
          externalImageUrl: "/images/shop/urban/test/gallery-neutral-1.jpg",
          caption: "Urban G-Wagon - Image 1",
        },
        {
          externalImageUrl: "/images/shop/urban/test/gallery-detail-1.jpg",
          caption: "Urban G-Wagon - Detail 1",
        },
      ],
      paddingTop: 0,
      paddingBottom: 0,
...
test("builds package photo gallery from real collection photos only and preserves order", () => {
  ...
  assert.equal(
    gallery.some((url) => url.includes("blueprint-")),
    false
  );
});
...
test("keeps the curated W465 bonnet context shots in the PDP gallery", () => {
  const closeUp =
  ...
  const gallery = resolveUrbanProductGallery(
    {
      slug: "urb-hoo-25358201-v1",
      title: { ua: "Bullnose капот W465", en: "W465 Bullnose Bonnet" },
      category: { ua: "Капоти", en: "Hoods" },
      productType: "Hoods",
      tags: [],
      bundle: null,
      image: closeUp,
      gallery: [closeUp, ...contextShots],
    } as never,
    ["mercedes-g-wagon-w465-aerokit"],
    null
  );

  assert.deepEqual(gallery, [closeUp, ...contextShots]);
});
