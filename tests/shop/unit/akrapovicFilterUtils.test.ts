...
test("extractVehicleModelsForBrand includes every chassis code from shared model groups", () => {
  const title =
    "AKRAPOVIC DI-BM/CA/9/G Rear Diffuser (Carbon Fiber / Gloss) for BMW M3 (G80 / G81) / M4 (G82 / G83)";
...
test("extractVehicleModelNamesForBrand picks marketing names per brand", () => {
  assert.deepEqual(
    extractVehicleModelNamesForBrand(
      "AKRAPOVIC Slip-On Line for PORSCHE 911 Carrera (992)",
      "Porsche"
    ),
    ["911"]
  );
...
test("extractVehicleModelNamesForBrand keeps M340i out of the M3 bucket", () => {
  assert.deepEqual(
    extractVehicleModelNamesForBrand(
      "AKRAPOVIC s-BM/T/18h Slip-On Line for BMW M340i (G20/G21)",
      "BMW"
    ),
    ["M340i/M340d"]
  );
...
test("extractVehicleModelNamesForBrand recognises Boxster/Cayman as 718", () => {
  assert.deepEqual(
    extractVehicleModelNamesForBrand(
      "AKRAPOVIC Slip-On Line for PORSCHE Boxster / Cayman (982)",
      "Porsche"
    ),
    ["718"]
  );
...
test("extractVehicleModelNamesForBrand matches both M235i and M240i for the same model", () => {
  assert.deepEqual(extractVehicleModelNamesForBrand("AKRAPOVIC for BMW M235i (F22)", "BMW"), [
    "M235i/M240i",
  ]);
  assert.deepEqual(extractVehicleModelNamesForBrand("AKRAPOVIC for BMW M240i (G42)", "BMW"), [
    "M235i/M240i",
  ]);
  assert.deepEqual(
...
test("extractChassisForBrandAndModel returns empty for the wrong model", () => {
  assert.deepEqual(extractChassisForBrandAndModel("AKRAPOVIC for BMW M3 (G80)", "BMW", "M5"), []);
});

test("determineProductScope accurately distinguishes auto and moto products", () => {
  // Ilmberger Carbon products must always be moto
  assert.equal(determineProductScope("Ilmberger Carbon", "some-sku", "Some carbon part"), "moto");

  // Akrapovič motorcycle products
  assert.equal(
    determineProductScope("AKRAPOVIC", "S-B10E9-APLT", "Evolution Line for BMW S1000RR"),
    "moto"
  );
  assert.equal(
    determineProductScope("AKRAPOVIC", "S-D9E7-CKOT", "Slip-On Line for Ducati Panigale V2"),
    "moto"
  );
  assert.equal(
    determineProductScope("AKRAPOVIC", "E-B12E6", "Optional Header for BMW R1250GS"),
    "moto"
  );
  assert.equal(
    determineProductScope("AKRAPOVIC", "P-HSB9SO1", "Heat Shield for BMW F900R"),
    "moto"
  );
  assert.equal(determineProductScope("AKRAPOVIC", "CB-B13T1", "Protection Bar Set"), "moto");

  // Akrapovič automotive products
  assert.equal(
    determineProductScope(
      "AKRAPOVIC",
      "S-PO991TSO-HT",
      "Slip-On Line (Titanium) for Porsche 911 Turbo"
    ),
    "auto"
  );
  assert.equal(
    determineProductScope("AKRAPOVIC", "S-AUR8SO3-T", "Slip-On Line for Audi R8"),
    "auto"
  );
  assert.equal(
    determineProductScope("AKRAPOVIC", "S-BM/T/18H", "Slip-On Line for BMW M440i"),
    "auto"
  );
  assert.equal(
    determineProductScope("AKRAPOVIC", "ME-BM/T/5", "Evolution Line for BMW M6"),
    "auto"
  );
});
