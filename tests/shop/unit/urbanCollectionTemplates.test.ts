...
const TEMPLATE_DIR = path.resolve(process.cwd(), "reference", "urban-shopify-theme", "templates");
...
test("every urban collection handle has a valid collection template file", () => {
  assert.equal(
    ...
    true,
    "Urban collection template directory should exist"
  );
...
test("smgassets URLs use the migrated local file when it is available", () => {
  const source =
  ...
  assert.equal(
    resolveUrbanThemeAssetUrl(source, { collectionHandle: "lamborghini-urus-se" }),
    "/images/shop/urban/carousel/models/urusSE/webp/urban-automotive-urus-se-widetrack-1-2560.webp"
  );
});
