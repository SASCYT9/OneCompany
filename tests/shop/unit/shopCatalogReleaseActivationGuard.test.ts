...
test("catalog reader stays unaffected outside requested production activation", () => {
  assert.equal(
    evaluateShopCatalogReleaseActivation({ nodeEnv: "production", readerMode: "off" }).allowed,
    true
  );
  assert.equal(
    evaluateShopCatalogReleaseActivation({ nodeEnv: "development", readerMode: "ssr" }).allowed,
    true
  );
});
test("signed canary evidence permits only its authorized percentage", () => {
  ...
  const exceeded = evaluateShopCatalogReleaseActivation({
    nodeEnv: "production",
    readerMode: "canary",
    canaryPercentage: 6,
    deployedCommit: commit,
...
test("full SSR requires separate explicit signed approval", () => {
  ...
  assert.equal(
    evaluateShopCatalogReleaseActivation({
      ...
      deployedCommit: commit,
      marker: marker({
        rollout: { maxCanaryPercentage: 100, fullSsrApproved: true, approvedBy: "Catalog Owner" },
      }),
      secret,
      now,
    }).allowed,
  ...
  assert.equal(
    evaluateShopCatalogReleaseActivation({
      ...
      deployedCommit: commit,
      marker: marker({
        shadow: { sampledRequests: 1000, mismatches: 0, errorRate: 0, windowHours: 72 },
        rollout: { maxCanaryPercentage: 100, fullSsrApproved: true, approvedBy: "Catalog Owner" },
      }),
      secret,
      now,
...
test("activation fails closed for stale ownership, mismatch, lag, weak samples, and SLO failures", () => {
  const result = evaluateShopCatalogReleaseActivation({
    nodeEnv: "production",
    readerMode: "canary",
    canaryPercentage: 1,
    deployedCommit: commit,
    marker: marker({
      ...
      sourcesReady: 13,
      projectionLag: 1,
      shadow: { sampledRequests: 999, mismatches: 1, errorRate: 0.01, windowHours: 1 },
      performance: { scaleP95Ms: 201, publicationP95Ms: 2001 },
      rollout: { maxCanaryPercentage: 5, fullSsrApproved: false, approvedBy: "" },
    }),
    secret,
    now,
  });
...
test("activation rejects tampering, old marker versions, and commit mismatch", () => {
  ...
  assert.equal(
    evaluateShopCatalogReleaseActivation({
      ...
      canaryPercentage: 1,
      deployedCommit: commit,
      marker: marker({ shadow: null, performance: null, rollout: null }),
      secret,
      now,
    }).allowed,
    false
...
test("official marker builder signs valid bounded rollout evidence", () => {
  const payload = JSON.parse(
    Buffer.from(marker().split(".")[0]!, "base64url").toString("utf8")
  ...
  assert.throws(
    () =>
      createShopCatalogReleaseMarker({ evidence: { ...payload, projectionLag: 1 }, secret, now }),
    /projection version lag/
  );
  assert.throws(
    () => createShopCatalogReleaseMarker({ evidence: payload, secret: "weak", now }),
    /32 bytes/
  );
});

test("native vehicle activation requires full SSR and the same signed coverage evidence", () => {
  for (const readerMode of [undefined, "off", "canary", "invalid"]) {
    const decision = evaluateShopCatalogReleaseActivation({
      nodeEnv: "production",
      readerMode,
      vehicleReaderMode: "projection",
    });
    assert.equal(decision.allowed, false);
    assert.match(decision.reasons.join(" "), /full SSR/);
  }
  assert.equal(
    evaluateShopCatalogReleaseActivation({
      nodeEnv: "production",
