...
test("full SSR requires separate explicit signed approval", () => {
  ...
  assert.equal(evaluateShopCatalogReleaseActivation({ nodeEnv: "production", readerMode: "ssr", deployedCommit: commit, marker: marker({ rollout: { maxCanaryPercentage: 100, fullSsrApproved: true } }), secret, now }).allowed, true);
});
