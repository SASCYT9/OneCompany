...
const SOFTKIT_PROGRAM_FALLBACK =
...
function buildCollectionConfig(): UrbanCollectionPageConfig {
  return {
    ...
    gallery: {
      ...
      slides: [
        {
          externalImageUrl: "/images/shop/urban/gallery/models/gwagonWidetrack2024/gallery-1.jpg",
          caption: "",
        },
        {
          externalImageUrl: "/images/shop/urban/gallery/models/gwagonWidetrack2024/gallery-2.jpg",
          caption: "",
        },
      ],
      paddingTop: 0,
...
test("builds a real Urban collection image pool from collection page media", () => {
  const images = buildUrbanCollectionImagePool(buildCollectionConfig(), [
...
test("resolves placeholder card images to a stable collection photo while preserving real images", () => {
  const placeholder =
    "https://cdn.shopify.com/s/files/1/0733/4058/4242/files/Gwagon_e9292903-5bf9-49aa-92da-8264c9bb9586.png?v=1776081527";
  const imagePool = buildUrbanCollectionImagePool(buildCollectionConfig(), [
    "mercedes-g-wagon-w465-widetrack",
  ]);
  ...
  assert.equal(first, "/images/shop/urban/overview/models/gwagonWidetrack2024/overview-1.jpg");
...
test("prefers the first compatible gallery image when the primary image is a cross-model mismatch", () => {
  ...
  assert.equal(
    resolveUrbanProductImage(DEFENDER_IMAGE, ["mercedes-g-wagon-softkit"], "urb-bun-25358198-v1"),
    SOFTKIT_PROGRAM_FALLBACK
  );
});
