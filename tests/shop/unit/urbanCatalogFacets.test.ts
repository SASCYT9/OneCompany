...
} from "../../../src/lib/urbanCatalogFacets";
...
test("buildUrbanCatalogEntries uses all Urban collection links for model filtering", () => {
  ...
  assert.deepEqual(entries[0]?.modelHandles, ["land-rover-defender"]);
  ...
  assert.equal(
    getUrbanProductFamily(
      buildProduct({
        ...
        title: {
          ua: "Капот Visual Carbon Fibre для G-Wagon Aerokit",
          en: "G-Wagon Aerokit Vented Bonnet",
        },
...
test("buildUrbanCatalogEntries applies explicit Discovery 5 programme titles", () => {
  ...
  assert.equal(entries[0]?.title, "Повний пакет Urban для Land Rover Discovery 5 Facelift 2020+");
  assert.equal(
    entries[1]?.title,
    "Комплект обвісу Urban для Land Rover Discovery 5 Pre-Facelift 2017-2020"
  );
});
