...
import {
  buildCsfSourceRecordDraft,
  type CsfSnapshotProduct,
} from "../../../src/lib/shopCatalogCsfNormalization";
const databaseUrl =
...
test(
  "CSF persists correlated chassis and exact manual transmission without broad engine match",
  { skip: !databaseUrl },
  async () => {
    const client = new PrismaClient({ datasources: { db: { url: databaseUrl } } });
    const suffix = Date.now().toString(),
      productId = `csf-${suffix}`,
    ...
    try {
      ...
      const title = "CSF Radiator for PORSCHE 944 Turbo (951) 1985-1991";
      const product: CsfSnapshotProduct = {
        id: productId,
        slug: productId,
        sku,
        scope: "auto",
        title: { ua: title, en: title },
        category: { ua: "Радіатори та аксесуари", en: "Radiators" },
        stock: "inStock",
        tags: ["fits-make:porsche"],
        longDescription: {
          en: "Applications: 1985-1991 Porsche 944 Turbo – Manual Transmission Only",
        },
        variants: [{ id: variantId, sku, isDefault: true }],
      };
      const draft = buildCsfSourceRecordDraft({ product, sourceRevision: "csf-v1" });
      await persistCsfSourceRecordPageWithClient(client, {
        sourceKey: `csf-${suffix}`,
        drafts: [draft],
      });
      ...
      assert.equal(
        clause.constraints.find((entry) => entry.dimension === "CHASSIS")?.values[0]?.textValue,
        "951"
      );
      assert.equal(
        clause.constraints.find((entry) => entry.dimension === "TRANSMISSION")?.values[0]
          ?.textValue,
        "MANUAL"
      );
      assert.equal(
