...
test(
  "Burger persists correlated BMW engine/chassis and quarantines polluted multi-make tags",
  { skip: !databaseUrl },
  async () => {
    const client = new PrismaClient({ datasources: { db: { url: databaseUrl } } }),
      suffix = Date.now().toString(),
      { persistBurgerSourceRecordPageWithClient } = await backfillModule;
    try {
      const cases = [
        {
          key: "bmw",
          title: "JB4 for B58 BMW 3-Series",
          tags: [
            "brand:BMW",
            "model:3-Series",
            "engine:B58",
        ...
        {
          key: "polluted",
          title: "KIA Stinger / Hyundai Genesis Package",
          tags: [
            "brand:BMW",
            "brand:Kia",
            "brand:Hyundai",
      ...
      for (const item of cases) {
        const productId = `burger-${item.key}-${suffix}`,
          variantId = `${productId}-variant`,
          sku = `shared-${suffix}`;
        await createProduct(client, productId, variantId, `${sku}-${item.key}`);
        const draft = buildBurgerSourceRecordDraft({
          ...
          sourceRevision: "burger-v1",
        });
        await persistBurgerSourceRecordPageWithClient(client, {
          sourceKey: `burger-${item.key}-${suffix}`,
          drafts: [draft],
        });
        ...
        if (item.key === "bmw") {
          const constraints = policy.clauses[0]!.constraints,
            engineValue = constraints.find((entry) => entry.dimension === "ENGINE")?.values[0];
          assert.equal(
            constraints.find((entry) => entry.dimension === "CHASSIS")?.values[0]?.textValue,
            "G20"
          );
