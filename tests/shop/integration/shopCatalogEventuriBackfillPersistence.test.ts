...
test(
  "Eventuri backfill persists mixed per-SKU policies without broadening unknown engines",
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const exactProduct = `eventuri-exact-${suffix}`,
        exactVariant = `${exactProduct}-variant`,
        exactSku = `EVE-EXACT-${suffix}`;
      await createProduct(client, exactProduct, exactVariant, exactSku);
      const exactDraft = buildEventuriSourceRecordDraft({
        product: snapshot({
          productId: exactProduct,
          variantId: exactVariant,
          sku: exactSku,
          slug: "g8x-m3-m4-s58-intake",
          title: "BMW M3 M4 S58 Intake",
          tags: ["category:intake", "BMW", "M3", "G80", "G81", "M4", "G82", "G83", "S58"],
        }),
        sourceRevision: "eventuri-exact-v1",
      });
      const exactResult = await persistEventuriSourceRecordPageWithClient(client, {
        sourceKey: `eventuri-exact-${suffix}`,
        drafts: [exactDraft],
      });
      ...
      for (const clause of exactPolicy.clauses) {
        ...
        assert.ok(
          clause.constraints.find((entry) => entry.dimension === "ENGINE")?.values[0]?.powertrainId
        );
        assert.equal(
          clause.constraints.find((entry) => entry.dimension === "FUEL")?.values[0]?.textValue,
          "petrol"
        );
      }
      assert.equal(
        await client.vehicleTaxonomyAlias.count({ where: { sourceId: exactResult.sourceId } }),
        8
      );

      const universalProduct = `eventuri-universal-${suffix}`,
        universalVariant = `${universalProduct}-variant`,
        universalSku = `EVE-UNI-${suffix}`;
      ...
      const universalDraft = buildEventuriSourceRecordDraft({
        product: snapshot({
          productId: universalProduct,
          variantId: universalVariant,
          sku: universalSku,
      ...
      await persistEventuriSourceRecordPageWithClient(client, {
        sourceKey: `eventuri-universal-${suffix}`,
        drafts: [universalDraft],
      });
      ...
      assert.equal(
        universalPolicy.clauses[0]?.constraints.find((entry) => entry.dimension === "ENGINE")
          ?.state,
        "NOT_APPLICABLE"
      );

      const reviewProduct = `eventuri-review-${suffix}`,
        reviewVariant = `${reviewProduct}-variant`,
        reviewSku = `EVE-REVIEW-${suffix}`;
