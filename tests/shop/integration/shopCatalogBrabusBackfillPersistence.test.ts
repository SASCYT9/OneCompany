...
test(
  "Brabus backfill persists exact chassis and quarantines missing engine identity",
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const productId = `brabus-exact-${suffix}`,
        variantId = `${productId}-variant`,
        sku = `BRABUS-${suffix}`;
      await createProduct(client, productId, variantId, sku);
      const exactDraft = buildBrabusSourceRecordDraft({
        product: snapshot({
          productId,
          variantId,
          sku,
          title: "Carbon parts for Mercedes – X 167 – Maybach GLS 600",
          tags: ["fits-make:mercedes-benz", "fits-model:mercedes-benz:gls"],
        }),
        sourceRevision: "brabus-v1",
      });
      const result = await persistBrabusSourceRecordPageWithClient(client, {
        sourceKey: `brabus-${suffix}`,
        drafts: [exactDraft],
      });
      ...
      const reviewProduct = `brabus-review-${suffix}`,
        reviewVariant = `${reviewProduct}-variant`,
        reviewSku = `BRABUS-REVIEW-${suffix}`;
      await createProduct(client, reviewProduct, reviewVariant, reviewSku);
      const reviewDraft = buildBrabusSourceRecordDraft({
        product: snapshot({
        ...
        sourceRevision: "brabus-review-v1",
      });
      await persistBrabusSourceRecordPageWithClient(client, {
        sourceKey: `brabus-review-${suffix}`,
        drafts: [reviewDraft],
      });
