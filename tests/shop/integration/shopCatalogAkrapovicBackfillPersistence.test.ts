...
import {
  buildAkrapovicSourceRecordDraft,
  type AkrapovicSnapshotProduct,
} from "../../../src/lib/shopCatalogAkrapovicNormalization";
const databaseUrl =
...
test(
  "Akrapovic persists isolated auto/moto taxonomy and review-only exhaust",
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const cases = [
        {
          key: "auto",
          scope: "auto" as const,
          title: "AKRAPOVIC Rear Diffuser for BMW M3 (G80)",
          tags: ["fits-make:bmw"],
          expected: "VEHICLE_SPECIFIC",
        },
        {
          key: "moto",
          scope: "moto" as const,
          title: "Akrapovic Adventure Footpeg Set for BMW R 1300 GS 2024-2026",
          tags: ["fits-make:bmw", "fits-year:2024", "fits-year:2025", "fits-year:2026"],
          expected: "VEHICLE_SPECIFIC",
        },
        {
          key: "review",
          scope: "auto" as const,
          title: "AKRAPOVIC Slip-On Exhaust for AUDI RS6 (C8) OPF",
          tags: ["fits-make:audi"],
          expected: "NEEDS_REVIEW",
        },
      ];
      for (const item of cases) {
        const productId = `akrapovic-${item.key}-${suffix}`,
          variantId = `${productId}-variant`,
          sku = `AKR-${item.key}-${suffix}`;
        await createProduct(client, productId, variantId, sku);
        const draft = buildAkrapovicSourceRecordDraft({
          product: snapshot(productId, variantId, sku, item.scope, item.title, item.tags),
          sourceRevision: "akrapovic-v1",
        });
        await persistAkrapovicSourceRecordPageWithClient(client, {
          sourceKey: `akrapovic-${item.key}-${suffix}`,
          drafts: [draft],
        });
        ...
        assert.ok(
          policy.clauses.every(
            (clause) =>
              clause.constraints.find((entry) => entry.dimension === "SCOPE")?.values[0]
                ?.textValue === item.scope
          )
        );
        if (item.key === "review")
          assert.ok(
      ...
      assert.equal(
        await client.vehicleMake.count({
          where: { normalizedName: "bmw", scope: { in: ["auto", "moto"] } },
        }),
        2
      );
