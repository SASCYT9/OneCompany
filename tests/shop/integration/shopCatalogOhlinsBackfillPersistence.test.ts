...
import {
  buildOhlinsSourceRecordDraft,
  type OhlinsSnapshotProduct,
} from "../../../src/lib/shopCatalogOhlinsNormalization";
const databaseUrl =
...
test(
  "Ohlins persists exact chassis, universal parts, and review-only drivetrain qualifiers",
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const cases = [
        {
          key: "exact",
          title: "OHLINS BMW M2 (G87) / M3 (G80) / M4 (G82)",
          tags: ["fits-make:bmw"],
          expected: "VEHICLE_SPECIFIC",
        },
        {
          key: "universal",
          title: "OHLINS Rubber Bushing",
          tags: ["fits-make:universal"],
          expected: "UNIVERSAL",
        },
        {
          key: "review",
          title: "OHLINS BMW M3 (G80) RWD Only",
          tags: ["fits-make:bmw"],
          expected: "NEEDS_REVIEW",
        },
      ];
      for (const item of cases) {
        const productId = `ohlins-${item.key}-${suffix}`,
          variantId = `${productId}-variant`,
          sku = `OHL-${item.key}-${suffix}`;
        await createProduct(client, productId, variantId, sku);
        const draft = buildOhlinsSourceRecordDraft({
          product: snapshot(productId, variantId, sku, item.title, item.tags),
          sourceRevision: "ohlins-v1",
        });
        await persistOhlinsSourceRecordPageWithClient(client, {
          sourceKey: `ohlins-${item.key}-${suffix}`,
          drafts: [draft],
        });
        ...
        if (item.key === "exact")
          assert.deepEqual(
            policy.clauses.map(
              (clause) => clause.constraints.find((entry) => entry.dimension === "CHASSIS")?.state
            ),
            ["EXACT", "EXACT", "EXACT"]
          );
      }
