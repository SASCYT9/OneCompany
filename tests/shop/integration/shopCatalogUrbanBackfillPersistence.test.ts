...
import {
  buildUrbanSourceRecordDraft,
  type UrbanSnapshotProduct,
} from "../../../src/lib/shopCatalogUrbanNormalization";
const databaseUrl =
...
test(
  "Urban backfill persists correlated multi-chassis wheels and review-only exhaust",
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      const productId = `urban-wheel-${suffix}`,
        variantId = `${productId}-variant`,
        sku = `URB-WHEEL-${suffix}`;
      await createProduct(client, productId, variantId, sku);
      const draft = buildUrbanSourceRecordDraft({
        product: snapshot({
          productId,
          variantId,
          sku,
          title: '24" Urban wheel (L405/L494)',
          tags: [
            "urban-family:wheels",
            "urban-vehicle-brand:range-rover",
            "fits-model:range-rover:24-uc9",
          ],
        }),
        sourceRevision: "urban-v1",
      });
      const result = await persistUrbanSourceRecordPageWithClient(client, {
        sourceKey: `urban-${suffix}`,
        drafts: [draft],
      });
      ...
      assert.deepEqual(
        policy.clauses
          .map(
            (clause) =>
              clause.constraints.find((entry) => entry.dimension === "CHASSIS")?.values[0]
                ?.textValue
          )
          .sort(),
        ["L405", "L494"]
      ...
      assert.equal(
        await client.vehicleTaxonomyAlias.count({ where: { sourceId: result.sourceId } }),
        5
      );

      const reviewProduct = `urban-exhaust-${suffix}`,
        reviewVariant = `${reviewProduct}-variant`,
        reviewSku = `URB-EXH-${suffix}`;
      await createProduct(client, reviewProduct, reviewVariant, reviewSku);
      const reviewDraft = buildUrbanSourceRecordDraft({
        product: snapshot({
          productId: reviewProduct,
          variantId: reviewVariant,
          sku: reviewSku,
          title: "Bentley Continental GT exhaust",
          tags: ["urban-family:exhaust", "urban-vehicle-brand:bentley"],
        }),
        sourceRevision: "urban-review-v1",
      });
      await persistUrbanSourceRecordPageWithClient(client, {
        sourceKey: `urban-review-${suffix}`,
        drafts: [reviewDraft],
      });
      const review = await client.shopCatalogCompatibilityPolicy.findFirstOrThrow({
        where: { targetKey: `variant:${reviewVariant}` },
        include: { clauses: { include: { constraints: true } } },
      });
      assert.equal(review.mode, "NEEDS_REVIEW");
      assert.equal(
        review.clauses[0]?.constraints.find((entry) => entry.dimension === "ENGINE")?.state,
        "UNKNOWN"
      );
    } finally {
