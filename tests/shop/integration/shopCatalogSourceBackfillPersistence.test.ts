...
const toBaselineSnapshot = (value: unknown) =>
  JSON.parse(
...
test(
  "shared source writer supports true product-level records without synthetic variants",
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      ...
      await client.shopCollection.create({
        data: {
          id: collectionId,
          handle: collectionId,
          titleUa: "Колекція",
      ...
      await client.shopProduct.create({
        data: {
          ...
          priceEur: "199.00",
          image: "/media/source-primary.jpg",
          gallery: ["/media/source-primary.jpg", "/media/source-detail.jpg"],
          media: {
            create: [
              { id: `source-media-${suffix}`, src: "/media/source-primary.jpg", position: 1 },
            ],
          },
          options: {
            create: [
              { id: `source-option-${suffix}`, name: "Finish", position: 1, values: ["Gloss"] },
            ],
          },
          metafields: {
            create: [
              { id: `source-meta-${suffix}`, namespace: "spec", key: "material", value: "carbon" },
            ],
          },
          collections: { create: [{ collectionId, sortOrder: 3 }] },
        },
      });
      const commerceInclude = {
        variants: true,
        media: true,
        options: true,
        metafields: true,
      ...
      const beforeCommerce = buildShopCatalogBaselineProductEntry(
        toBaselineSnapshot(
          await client.shopProduct.findUniqueOrThrow({
            where: { id: productId },
            include: commerceInclude,
          })
        )
      );
      const draft = {
      ...
      const result = await persistCatalogSourceRecordPageWithClient(
        client,
        { drafts: [draft], sourceKey: `product-source-${suffix}` },
        {
          ...
          async persistCompatibility(input) {
            callbackRecords.push(input.sourceRecordId);
          },
        }
      );
      assert.equal(result.inserted, 1);
      assert.equal(callbackRecords.length, 1);
      const replay = await persistCatalogSourceRecordPageWithClient(
        client,
        { drafts: [draft], sourceKey: `product-source-${suffix}` },
        {
          label: "Product fixture",
      ...
      assert.equal(callbackRecords.length, 1);
      const afterCommerce = buildShopCatalogBaselineProductEntry(
        toBaselineSnapshot(
          await client.shopProduct.findUniqueOrThrow({
            where: { id: productId },
            include: commerceInclude,
      ...
      const sourceRecord = await client.shopCatalogSourceRecord.findFirstOrThrow({
...
test(
  "shared source writer serializes concurrent binding and compatibility promotion",
  { skip: !databaseUrl },
  async () => {
    ...
    try {
      ...
      assert.equal(
        await first.shopCatalogSourceRecord.count({ where: { sourceId: source.id } }),
        1
      );
      assert.equal(
        await first.shopCatalogSourceBindingHead.count({ where: { sourceId: source.id } }),
        1
      );
    } finally {
...
test(
  "source ledger refuses to promote a resilience snapshot into a missing canonical product",
  { skip: !databaseUrl },
  async () => {
    const client = new PrismaClient({ datasources: { db: { url: databaseUrl } } });
    const suffix = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
      ...
      provenance: [
        {
          ...
          normalizedValue: productId,
          mappingStatus: "MAPPED" as const,
          mapperVersion: "missing-product-test-v1",
          confidence: 1,
          reason: null,
          productId,
          variantId: null,
    ...
    try {
      ...
      await assert.rejects(
        () =>
          persistCatalogSourceRecordPageWithClient(
            client,
            { drafts: [draft], sourceKey },
            {
              label: "Missing canonical fixture",
              defaultSourceKey: "unused",
              defaultDisplayName: "Missing canonical fixture",
              decisionReason: "must fail closed",
              async persistCompatibility() {
      ...
      assert.equal(
        await client.shopCatalogSourceRecord.count({ where: { recordKey: productId } }),
        0
      );
    } finally {
      await client.$disconnect();
...
test(
  "versioned full-commerce import creates a publishable canonical aggregate before source promotion",
  { skip: !databaseUrl },
  async () => {
    const client = new PrismaClient({ datasources: { db: { url: databaseUrl } } });
    const suffix = `${Date.now()}-${Math.random().toString(16).slice(2)}`;
    ...
    try {
      await client.shopCollection.create({
        data: { id: collectionId, handle: collectionId, titleUa: "Імпорт", titleEn: "Import" },
      });
      const [
        { normalizeAdminProductPayload, buildAdminProductCreateData },
        { coordinateShopCatalogProductCreationWithClient },
        { buildShopCatalogAdminSnapshot },
      ] = await Promise.all([
      ...
      const normalized = normalizeAdminProductPayload({
        slug: `full-commerce-${suffix}`,
        sku: `FULL-${suffix}`,
        scope: "auto",
        storefront: "main",
        brand: "Future Brand",
        titleUa: "Повний товар",
        titleEn: "Full product",
        shortDescUa: "Опис",
        shortDescEn: "Description",
        priceEur: 499,
        image: "/media/full-main.jpg",
        gallery: ["/media/full-main.jpg", "/media/full-detail.jpg"],
        collectionIds: [collectionId],
        tags: ["future-brand"],
        isPublished: true,
        media: [
          { src: "/media/full-main.jpg", position: 1 },
          { src: "/media/full-detail.jpg", position: 2 },
        ],
      ...
      const creation = await coordinateShopCatalogProductCreationWithClient(client, {
        ...
        snapshot(tx, productId, initialVersion) {
          return buildShopCatalogAdminSnapshot(tx, productId, initialVersion, {
            type: "IMPORT",
            id: "integration",
            reason: "full commerce creation",
          });
        },
      });
      assert.equal(creation.canonicalVersion, "1");
      const product = await client.shopProduct.findUniqueOrThrow({
        where: { id: creation.productId },
        include: {
          media: true,
          options: true,
          variants: true,
      ...
      assert.equal(product.metafields.length, 1);
      assert.equal(product.collections.length, 1);
      assert.equal(product.titleUa, "Повний товар");
      assert.equal(product.titleEn, "Full product");
      assert.equal(product.priceEur?.toString(), "499");
      assert.equal(
        await client.shopCatalogProductRevision.count({ where: { productId: product.id } }),
        1
      );
      assert.equal(await client.shopCatalogOutbox.count({ where: { productId: product.id } }), 1);
      assert.ok(
        await client.shopCatalogPublicationReceipt.count({ where: { productId: product.id } })
      );
    } finally {
      await client.$disconnect();
    }
