/**
 * Burger Motorsports — Full Batch Translation via TranslateGemma 4B (Ollama)
 * Translates titleEn → titleUa, bodyHtmlEn → bodyHtmlUa
 * Combines title+desc in one request for speed
 */
import 'dotenv/config';
import { PrismaClient } from '../node_modules/.prisma/client/index.js';

const prisma = new PrismaClient();
const OLLAMA = 'http://localhost:11434/api/generate';
const MODEL = 'translategemma:4b';
const MAX_DESC_CHARS = 3000; // Truncate very long descriptions to avoid timeouts
const TIMEOUT_MS = 90_000;

const SEPARATOR = '|||SPLIT|||';

async function translate(text, retries = 2) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), TIMEOUT_MS);

  try {
    const resp = await fetch(OLLAMA, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      signal: controller.signal,
      body: JSON.stringify({
        model: MODEL,
        prompt: `Translate the following English text to Ukrainian. Output ONLY the Ukrainian translation, nothing else. No explanations, no alternatives, no bullet points.\n\n${text}`,
        stream: false,
        options: { temperature: 0.1, num_predict: 4096 },
      }),
    });

    clearTimeout(timer);
    const data = await resp.json();
    let result = data.response?.trim() || '';
    // Clean up common TranslateGemma artifacts
    result = result.replace(/^(Один з варіантів перекладу:|Переклад:|Ось переклад:|Translation:)\s*/i, '');
    result = result.replace(/\n\n\*\*Інші можливі варіанти.*/s, '');
    result = result.replace(/\n\nNote:.*/s, '');
    return result;
  } catch (err) {
    clearTimeout(timer);
    if (retries > 0) {
      console.warn(`  ⚠ Retry (${retries} left): ${err.message?.slice(0, 50)}`);
      await new Promise(r => setTimeout(r, 2000));
      return translate(text, retries - 1);
    }
    return null;
  }
}

// Post-process Ukrainian text
function postProcess(text) {
  if (!text) return text;
  return text
    .replace(/Бургер Моторспортс?/gi, 'Burger Motorsports')
    .replace(/Бі-Ем-Ес\b/gi, 'BMS')
    .replace(/Ф'юел-Іт!/gi, 'Fuel-It!')
    .replace(/Паливо-Це!/gi, 'Fuel-It!')
    .replace(/ДжейБі4/gi, 'JB4')
    .replace(/ДжейБі\+/gi, 'JB+')
    .replace(/підключи (та|і) грай/gi, 'підключи і працюй')
    .replace(/Зроблено в США/gi, 'Виробництво США')
    .replace(/вироблено в США/gi, 'Виробництво США');
}

async function main() {
  console.log('🍔🇺🇦 Burger Motorsports — Full Translation Batch');
  console.log('   Model: TranslateGemma 4B (Ollama, GPU)');
  console.log('='.repeat(55));

  const products = await prisma.shopProduct.findMany({
    where: { brand: 'Burger Motorsports', bodyHtmlUa: '' },
    select: { id: true, titleEn: true, bodyHtmlEn: true },
    orderBy: { priceUsd: 'desc' },
  });

  console.log(`\n📦 Products to translate: ${products.length}\n`);
  if (products.length === 0) {
    console.log('✅ All already translated!');
    await prisma.$disconnect();
    return;
  }

  // Warm up model
  console.log('🔥 Warming up model...');
  await translate('Hello world');
  console.log('   Ready!\n');

  let done = 0;
  let errors = 0;
  const startTime = Date.now();

  for (const product of products) {
    const t0 = Date.now();

    try {
      // Translate title
      const titleUa = await translate(product.titleEn);

      // Translate description (truncated if too long)
      const descEn = product.bodyHtmlEn.slice(0, MAX_DESC_CHARS);
      const descUa = descEn.length > 10 ? await translate(descEn) : '';

      if (titleUa) {
        await prisma.shopProduct.update({
          where: { id: product.id },
          data: {
            titleUa: postProcess(titleUa),
            bodyHtmlUa: postProcess(descUa || ''),
          },
        });
      }

      done++;
      const elapsed = ((Date.now() - t0) / 1000).toFixed(1);
      const totalElapsed = ((Date.now() - startTime) / 60000).toFixed(1);
      const avgPerItem = (Date.now() - startTime) / done / 1000;
      const remaining = ((products.length - done) * avgPerItem / 60).toFixed(0);

      if (done % 10 === 0 || done <= 5) {
        process.stdout.write(
          `\r  ✅ ${done}/${products.length} | ${elapsed}s | ${totalElapsed}min elapsed | ~${remaining}min remaining    `
        );
      }
    } catch (err) {
      errors++;
      console.error(`\n  ❌ [${product.titleEn?.slice(0, 40)}]: ${err.message?.slice(0, 80)}`);
    }
  }

  const totalMin = ((Date.now() - startTime) / 60000).toFixed(1);
  console.log(`\n\n${'='.repeat(55)}`);
  console.log(`✅ Done: ${done} | ❌ Errors: ${errors} | ⏱️ ${totalMin} min`);

  // Verify
  const remaining = await prisma.shopProduct.count({ where: { brand: 'Burger Motorsports', bodyHtmlUa: '' } });
  const translated = await prisma.shopProduct.count({ where: { brand: 'Burger Motorsports', bodyHtmlUa: { not: '' } } });
  console.log(`\n📊 Final: ${translated} translated, ${remaining} remaining`);

  // Show 3 samples
  const samples = await prisma.shopProduct.findMany({
    where: { brand: 'Burger Motorsports', bodyHtmlUa: { not: '' } },
    select: { titleEn: true, titleUa: true, bodyHtmlUa: true },
    take: 3,
    orderBy: { priceUsd: 'desc' },
  });

  console.log('\n=== SAMPLES ===');
  for (const s of samples) {
    console.log(`  EN: ${s.titleEn}`);
    console.log(`  UA: ${s.titleUa}`);
    console.log(`  Desc: ${s.bodyHtmlUa.slice(0, 120)}...`);
    console.log('');
  }

  await prisma.$disconnect();
}

main().catch(console.error);
