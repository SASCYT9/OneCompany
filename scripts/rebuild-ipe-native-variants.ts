...
function pickPriceForG80Combo(rows: ParsedRow[], combo: string[]): number | null {
  ...
  const catbackRow = rows.find(
    (r) =>
      (r.section || "")
        .toLowerCase()
        .includes(`cat back system (${isFactory ? "factory" : "equal-length"} front pipe)`) &&
      (r.material || "").toLowerCase().trim() === "ss" &&
      r.retail_usd
  );
  // Downpipe price
  const downpipeRow = rows.find(
    (r) =>
...
const GT3_992_PRICER: Pricer = (rows, combo) => {
  ...
  const cb = rows.find(
    (r) =>
      ...
      (r.material || "").toLowerCase().trim() === (isTi ? "ti" : "ss") &&
      r.retail_usd
  );
...
const PORSCHE_718_GT4_BODYKIT_PRICER: Pricer = (rows, combo) => {
  ...
  const cb = rows.find(
    (r) =>
      ...
      (r.material || "").toLowerCase().trim() === "ss" &&
      /bodykit|with 718 gt4/i.test(r.description || "") &&
      r.retail_usd
  );
  const dp = rows.find(
    (r) =>
...
const M240I_G42_PRICER: Pricer = (rows, combo) => {
  ...
  const dp = rows.find(
    (r) =>
      (r.section || "").toLowerCase() === "downpipe" &&
      (r.material || "").toLowerCase().trim() === "ss" &&
      (is200Cell ? /catted/i.test(r.description || "") : /catless/i.test(r.description || "")) &&
      r.retail_usd
  );
  if (!cb || !dp) return null;
  return (cb.retail_usd || 0) + (dp.retail_usd || 0);
...
const PRODUCTS: Array<{
  ...
  bonusTi?: {
    ...
    option2: string | null;
    option3: string | null;
    rowFinder: (rows: ParsedRow[]) => ParsedRow | undefined;
  };
}> = [
  {
    slug: "ipe-bmw-m3-m4-g80-g82-exhaust",
    ...
    bonusTi: {
      ...
      option1: "Catback System",
      option2: "Equal-Length Front Pipe",
      option3: "Titanium",
      rowFinder: (rows) =>
        rows.find(
          (r) =>
            (r.material || "").toLowerCase() === "ti" &&
            (r.section || "").toLowerCase().includes("cat back") &&
            r.retail_usd
        ),
    },
  },
...
async function rebuildOne(target: (typeof PRODUCTS)[0]) {
  ...
  if (!official) {
    console.log("  Official feed missing — skip");
    return;
  }
  ...
  if (!product) {
    console.log("  Product missing in DB — skip");
    return;
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (let i = 0; i < officialOpts.length; i++) {
      ...
      const values =
        isLast && target.bonusTi
          ? [
              ...new Set([
                ...opt.values,
                target.bonusTi.option3 || target.bonusTi.option2 || target.bonusTi.option1,
              ]),
            ]
          : opt.values;
