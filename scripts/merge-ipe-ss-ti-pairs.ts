...
const PAIRS: Array<[string, string]> = [
  ...
  [
    "porsche-718-gts-4-0-718-cayman-gt4-718-spyder-exhaust",
    "porsche-718-gts-4-0-718-cayman-gt4-718-spyder-titanium-exhaust",
  ],
  [
    "porsche-911-carrera-4-s-4s-992-exhaust",
    "porsche-911-carrera-4-s-4s-992-titanium-exhaust-system",
  ],
...
function stripBrandWords(slug: string): string {
  return slug.replace(
    /^(porsche|ferrari|mclaren|audi|lamborghini|bmw|mercedes(-benz)?|toyota|subaru)-/,
    ""
  );
}
...
async function planPairs(): Promise<{
  ...
  const items = (snapshot.products || snapshot) as Array<{
    handle: string;
    images: string[];
    title: string;
  }>;
  ...
  for (const pair of PAIRS) {
    ...
    if (!ssOfficial || !tiOfficial) {
      skipped.push({
        pair,
        reason: `Missing in iPE snapshot: ss=${!!ssOfficial}, ti=${!!tiOfficial}`,
      });
      continue;
    }
    ...
    const tiProduct = await prisma.shopProduct.findFirst({
      where: { slug: tiSlug },
      select: { id: true },
    });
    ...
    if (!ss && !ti) {
      skipped.push({
        pair,
        reason: `Pricelist match missing both SS and Ti, tokens=${tokens.join(",")}`,
      });
      continue;
    }
...
async function applyAction(action: PairAction) {
  ...
  for (let i = 0; i < action.tiImageUrls.length; i++) {
    ...
    try {
      ...
      const result = await putPublicBlob(
        blobPath,
        buffer,
  ...
  const newGallery = [...action.existingGallery, ...newMedia.map((m) => m.relPath)];
  const materials = [
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (existingMat) {
      await tx.shopProductMetafield.update({
        ...
        data: { value: materials, valueType: "string" },
      });
    } else {
  ...
  const priceSummary = newVariants
    .map((v) => `${v.title.split(" · ")[1]}=$${v.priceUsd}`)
    .join(" / ");
...
async function main() {
  ...
  for (const a of actions) {
    ...
    console.log(
      `  SS Cat-back: ${a.ssCatbackPrice !== null ? `MSRP $${a.ssMsrp} → retail $${a.ssCatbackPrice}` : "— (not in Excel)"}`
    );
    console.log(
      `  Ti Cat-back: ${a.tiCatbackPrice !== null ? `MSRP $${a.tiMsrp} → retail $${a.tiCatbackPrice}` : "— (not in Excel)"}`
    );
    console.log(`  Ti images to upload: ${a.tiImageUrls.length}`);
    console.log(
      `  Existing SS gallery: ${a.existingMediaCount}, will become ${a.existingMediaCount + a.tiImageUrls.length}`
    );
  }
  if (skipped.length) {
    console.log("\n--- Skipped ---");
