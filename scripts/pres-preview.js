...
const DIR = path.join(repositoryRoot, "wiki", "screenshots");
const presentationPath = process.env.PRESENTATION_HTML
  ? path.resolve(process.env.PRESENTATION_HTML)
  : null;

async function run() {
  if (!presentationPath || !fs.existsSync(presentationPath)) {
    throw new Error("Set PRESENTATION_HTML to an existing local HTML presentation");
  }
