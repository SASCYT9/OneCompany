...
function assertCommitTarget() {
  ...
  if ([process.env.VERCEL_ENV, process.env.DEPLOY_ENV, process.env.APP_ENV].some((value) => value?.toLowerCase() === "production")) throw new Error("Brabus backfill commit is disabled in production");
  if (process.env.CATALOG_BRABUS_BACKFILL_ALLOW_WRITE !== "1") throw new Error("Set CATALOG_BRABUS_BACKFILL_ALLOW_WRITE=1 to authorize non-production commit");
  const url = process.env.CATALOG_BRABUS_BACKFILL_DATABASE_URL;
  if (!url) throw new Error("CATALOG_BRABUS_BACKFILL_DATABASE_URL is required for commit");
  return url;
}
async function loadDrafts() {
  ...
  return products.map((product) => buildBrabusSourceRecordDraft({ product, sourceRevision: hash }))
    .sort((left, right) => left.sourceRecord.recordKey.localeCompare(right.sourceRecord.recordKey));
}
async function main() {
  const commit = process.argv.includes("--commit"), after = option("after") ?? null, limit = Number(option("limit") ?? 50);
  if (!Number.isSafeInteger(limit) || limit < 1 || limit > 50) throw new TypeError("--limit must be between 1 and 50");
  ...
  const drafts = remaining.slice(0, limit);
  const summary = { mode: commit ? "commit" : "dry-run", totalRecords: allDrafts.length, after, selected: drafts.length,
    ...
    nextRecordKey: drafts.at(-1)?.sourceRecord.recordKey ?? null, complete: remaining.length <= drafts.length };
  if (!commit || !drafts.length) return void console.log(JSON.stringify(summary, null, 2));
  const client = new PrismaClient({ datasources: { db: { url: assertCommitTarget() } } });
  try {
    const persistence = await persistBrabusSourceRecordPageWithClient(client, { drafts, reviewedById: process.env.CATALOG_BRABUS_BACKFILL_REVIEWED_BY_ID });
    console.log(JSON.stringify({ ...summary, persistence }, null, 2));
