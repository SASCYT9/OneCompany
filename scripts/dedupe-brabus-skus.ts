...
const VEHICLE_HINT =
  /(mercedes|porsche|bentley|rolls|maybach|lamborghini|range\s?rover|smart|w\s?\d+|x\s?\d+|c\s?\d+|r\s?\d+|amg|gle|gls|gla|glb|glc|cls|sl-|sl\s|s-class|g-class|e-class|c-class)/i;
...
async function main() {
  ...
  console.log(
    `Found ${pairs.length} duplicate SKU groups (${pairs.reduce((s, p) => s + p.rows.length, 0)} rows).`
  );
  ...
  for (const pair of sized2) {
    ...
    planned.push({
      ...
      loser,
      inheritImage:
        isExternalBrabusImage(winner.image) && isLocalImage(loser.image) ? loser.image : null,
  ...
  const bundleRefs = await (prisma as any).shopBundleItem
    .findMany({
      where: { componentProductId: { in: allLoserIds } },
  ...
  if (blockedIds.size) {
    console.log(
      `⚠ ${blockedIds.size} losers are referenced by ShopBundleItem (won't delete those).`
    );
    planned.forEach((p) => {
  ...
  safe.slice(0, 5).forEach(({ pair, winner, loser, inheritImage }) => {
    ...
    console.log(
      `    winner=${winner.sku} (${winner.id}) titleLen=${winner.titleEn.length} descEn=${descLen(winner, "en")}`
    );
    console.log(
      `    loser =${loser.sku} (${loser.id})  titleLen=${loser.titleEn.length}  descEn=${descLen(loser, "en")}`
    );
