...
function assertCommitTarget() { const environment = option("environment")?.toLowerCase(); if (!environment || !safeEnvironments.has(environment)) throw new Error("Commit requires an explicit non-production --environment");
  if ([process.env.VERCEL_ENV, process.env.DEPLOY_ENV, process.env.APP_ENV].some((value) => value?.toLowerCase() === "production")) throw new Error("GiroDisc backfill commit is disabled in production");
  if (process.env.CATALOG_GIRODISC_BACKFILL_ALLOW_WRITE !== "1") throw new Error("Set CATALOG_GIRODISC_BACKFILL_ALLOW_WRITE=1 to authorize non-production commit"); const url = process.env.CATALOG_GIRODISC_BACKFILL_DATABASE_URL;
  if (!url) throw new Error("CATALOG_GIRODISC_BACKFILL_DATABASE_URL is required for commit"); return url; }
async function loadDrafts() { const manifestPath = resolve("public", "catalog-fallback", "manifest.json"), manifest = JSON.parse(await readFile(manifestPath, "utf8")) as { stores?: Record<string, { file?: string; count?: number }> };
  ...
  return products.map((product) => buildGirodiscSourceRecordDraft({ product, sourceRevision: hash })).sort((left, right) => left.sourceRecord.recordKey.localeCompare(right.sourceRecord.recordKey)); }
async function main() { const commit = process.argv.includes("--commit"), after = option("after") ?? null, limit = Number(option("limit") ?? 50); if (!Number.isSafeInteger(limit) || limit < 1 || limit > 50) throw new TypeError("--limit must be between 1 and 50");
  ...
  try { const persistence = await persistGirodiscSourceRecordPageWithClient(client, { drafts, reviewedById: process.env.CATALOG_GIRODISC_BACKFILL_REVIEWED_BY_ID }); console.log(JSON.stringify({ ...summary, persistence }, null, 2)); } finally { await client.$disconnect(); } }
