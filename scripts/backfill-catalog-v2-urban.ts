...
function assertCommitTarget() { const environment = option("environment")?.toLowerCase(); if (!environment || !safeEnvironments.has(environment)) throw new Error("Commit requires an explicit non-production --environment");
  if ([process.env.VERCEL_ENV, process.env.DEPLOY_ENV, process.env.APP_ENV].some((value) => value?.toLowerCase() === "production")) throw new Error("Urban backfill commit is disabled in production");
  if (process.env.CATALOG_URBAN_BACKFILL_ALLOW_WRITE !== "1") throw new Error("Set CATALOG_URBAN_BACKFILL_ALLOW_WRITE=1 to authorize non-production commit");
  const url = process.env.CATALOG_URBAN_BACKFILL_DATABASE_URL; if (!url) throw new Error("CATALOG_URBAN_BACKFILL_DATABASE_URL is required for commit"); return url; }
async function loadDrafts() { const manifestPath = resolve("public", "catalog-fallback", "manifest.json"); const manifest = JSON.parse(await readFile(manifestPath, "utf8")) as { stores?: Record<string, { file?: string; count?: number }> };
  const descriptor = manifest.stores?.urban; if (!descriptor?.file || !descriptor.count) throw new Error("Urban fallback shard is missing"); const raw = await readFile(resolve(dirname(manifestPath), descriptor.file), "utf8");
  const hash = createHash("sha256").update(raw).digest("hex").slice(0, 12); if (!descriptor.file.includes(`.${hash}.json`)) throw new Error("Urban shard hash mismatch"); const products = JSON.parse(raw) as UrbanSnapshotProduct[];
  if (products.length !== descriptor.count) throw new Error("Urban shard count mismatch"); return products.map((product) => buildUrbanSourceRecordDraft({ product, sourceRevision: hash })).sort((left, right) => left.sourceRecord.recordKey.localeCompare(right.sourceRecord.recordKey)); }
async function main() { const commit = process.argv.includes("--commit"), after = option("after") ?? null, limit = Number(option("limit") ?? 50); if (!Number.isSafeInteger(limit) || limit < 1 || limit > 50) throw new TypeError("--limit must be between 1 and 50");
  ...
  try { const persistence = await persistUrbanSourceRecordPageWithClient(client, { drafts, reviewedById: process.env.CATALOG_URBAN_BACKFILL_REVIEWED_BY_ID }); console.log(JSON.stringify({ ...summary, persistence }, null, 2)); } finally { await client.$disconnect(); } }
