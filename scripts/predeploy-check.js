...
function ensureDirectoryEmpty(targetPath, label, ignoreNames = []) {
  ...
  console.error(
    `[DETAIL] ${targetPath} has ${fileCount} file(s) that should have been migrated to Blob storage.`
  );
  console.error(
    "[DETAIL] Run the Blob migration, remove tracked legacy files from git, and redeploy."
  );
  process.exit(1);
}
...
try {
  ...
  if (fs.existsSync(brabusImagesDir)) {
    ...
    if (localFiles > 0 && trackedFiles < localFiles) {
      ...
      console.error(
        "[DETAIL] Preview deploys will miss these images unless you migrate them to tracked storage or Git LFS."
      );
      process.exit(1);
    }
  }
