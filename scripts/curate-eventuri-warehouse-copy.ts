...
async function main() {
  const rows = await prisma.shopProduct.findMany({
    where: { slug: { in: EVENTURI_WAREHOUSE_EDITORIAL_SLUGS } },
    select: {
  ...
  console.log(JSON.stringify({ mode: COMMIT ? "commit" : "dry-run", matched: rows.length, updates: updates.length, products: updates.map(({ slug, changed }) => ({ slug, changed })) }, null, 2));
  if (!COMMIT) return;
  ...
  let published = 0;
  for (let pass = 0; pass < 5 && published < updates.length; pass += 1) {
    const result = await runShopCatalogOutboxRuntime({ workerId: `eventuri-copy-cli:${process.pid}`, limit: Math.max(20, updates.length) });
    published += result.completed;
    if (result.claimed === 0) break;
  }
  console.log(JSON.stringify({ updated: updates.length, published }, null, 2));
}
