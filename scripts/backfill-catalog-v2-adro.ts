...
import { buildAdroSourceRecordDraft, type AdroSnapshotProduct } from "../src/lib/shopCatalogAdroNormalization";
...
function assertCommitTarget() {
  ...
  if ([process.env.VERCEL_ENV, process.env.DEPLOY_ENV, process.env.APP_ENV].some((value) => value?.toLowerCase() === "production")) {
    throw new Error("ADRO backfill commit is disabled in production");
  }
  if (process.env.CATALOG_ADRO_BACKFILL_ALLOW_WRITE !== "1") {
    throw new Error("Set CATALOG_ADRO_BACKFILL_ALLOW_WRITE=1 to authorize non-production commit");
  }
  const url = process.env.CATALOG_ADRO_BACKFILL_DATABASE_URL;
  if (!url) throw new Error("CATALOG_ADRO_BACKFILL_DATABASE_URL is required for commit");
  return url;
}
...
async function loadDrafts() {
  ...
  if (!descriptor?.file || !descriptor.count) throw new Error("ADRO fallback shard is missing");
  const shardPath = resolve(dirname(manifestPath), descriptor.file);
  const raw = await readFile(shardPath, "utf8");
  const hash = createHash("sha256").update(raw).digest("hex").slice(0, 12);
  if (!descriptor.file.includes(`.${hash}.json`)) throw new Error("ADRO shard hash mismatch");
  const products = JSON.parse(raw) as AdroSnapshotProduct[];
  if (products.length !== descriptor.count) throw new Error("ADRO shard count mismatch");
  return products
    .map((product) => buildAdroSourceRecordDraft({ product, sourceRevision: hash }))
...
async function main() {
  ...
  try {
    const result = await persistAdroSourceRecordPageWithClient(client, {
      drafts,
      reviewedById: process.env.CATALOG_ADRO_BACKFILL_REVIEWED_BY_ID,
    });
