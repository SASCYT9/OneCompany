...
function integer(name: string, fallback: number) {
  ...
  if (!Number.isInteger(value) || value < 1 || value > 50)
    throw new Error(`--${name} must be within 1..50`);
  return value;
}

function viewportDimension(name: string, fallback: number, minimum: number, maximum: number) {
  const value = Number(argument(name, String(fallback)));
...
function cleanCommit() {
  if (git("status", "--porcelain"))
    throw new Error("Browser evidence requires a clean committed worktree");
...
function targetUrl() {
  ...
  if (
    ...
    process.env.CATALOG_STOREFRONT_BROWSER_ALLOW_REMOTE !== "1"
  ) {
...
async function main() {
  ...
  const viewport = {
    width: viewportDimension("width", 1440, 320, 2400),
    height: viewportDimension("height", 1000, 568, 2000),
  };
  ...
  try {
    for (let index = 0; index < sampleCount; index += 1) {
      ...
      const measuredLcp = await page.evaluate(
        () => (window as Window & { __catalogLcp?: number }).__catalogLcp ?? 0
      );
      ...
      await page.waitForURL((next) => next.searchParams.get("brand") === option, {
        timeout: 10_000,
      });
      await page
        .locator('[data-catalog-v2="true"][aria-busy="false"], [data-catalog-v2="true"]')
        .waitFor();
  ...
  const localAnalyticsFailures = failedResponses.filter((entry) => {
    ...
    return (
      failedUrl.origin === url.origin &&
      failedUrl.pathname === "/_vercel/insights/script.js" &&
      entry.status === 404
    );
  });
  ...
  const artifact = {
    ...
    measurements: {
      lcpMs: {
        p75: Number(lcpP75.toFixed(3)),
        p95: Number(lcpP95.toFixed(3)),
        max: Number(Math.max(...lcp).toFixed(3)),
      },
      filterNavigationMs: {
        p95: Number(filterP95.toFixed(3)),
        max: Number(Math.max(...filterLatency).toFixed(3)),
      },
    },
    limits: {
      lcpP75Ms: LCP_P75_LIMIT_MS,
      lcpP95Ms: LCP_P95_LIMIT_MS,
      filterP95Ms: FILTER_P95_LIMIT_MS,
    },
    diagnostics: {
