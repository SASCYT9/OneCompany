...
async function main() {
  ...
  try {
    ...
    const [products, variants, images, videos, sourceRecords, productHeads, variantHeads, unpublished, texts, fitments, issueGroups, uaProjections, enProjections, outboxGroups] = await Promise.all([
      ...
      prisma.shopCatalogProjection.count({ where: { product: where, locale: "ua", isPublished: true } }),
      prisma.shopCatalogProjection.count({ where: { product: where, locale: "en", isPublished: true } }),
      prisma.shopCatalogOutbox.groupBy({ by: ["status"], where: { product: where }, _count: true }),
    ]);
    ...
    const checks = {
      ...
      publicationState: activated ? unpublished === 0 : unpublished === expected.products,
      ...
      projectionsReady: !activated || (uaProjections === expected.products && enProjections === expected.products),
      outboxHealthy: !activated || outboxGroups.every((group) => group.status === "COMPLETED"),
    };
    const report = { expected, actual: { products, variants, images, videos, applications: actualApplications }, ownership: { sourceRecords, productHeads, variantHeads }, unpublished, projections: { ua: uaProjections, en: enProjections }, outbox: Object.fromEntries(outboxGroups.map((group) => [group.status, group._count])), issueCounts: Object.fromEntries(issueGroups.map((group) => [group.code, group._count])), checks, passed: Object.values(checks).every(Boolean) };
