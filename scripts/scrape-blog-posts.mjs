...
async function main() {
  ...
  for (const shortcode of SHORTCODES) {
    ...
    try {
      ...
      console.log(
        `Caption from fastdl: ${caption ? caption.substring(0, 80) + "..." : "(none found)"}`
      );
    } catch (e) {
    ...
    if (!caption || caption.length < 10) {
      ...
      try {
        ...
        await igPage.goto(`https://www.instagram.com/p/${shortcode}/`, {
    ...
    try {
      ...
      await datePage.goto(`https://www.instagram.com/p/${shortcode}/`, {
        waitUntil: "domcontentloaded",
        timeout: 8000,
      });
    ...
    if (existingIdx !== -1) {
      ...
      if (
        oldPost.title.ua !== "One Company tuning project" &&
        (!title || title === `One Company Post ${shortcode}`)
      ) {
