...
<footer class="footer"><p class="question">${t.contact}</p><div class="closing"><div class="seller"><p>${esc(company)}${seller?.appAddress ? `<br>${esc(seller.appAddress)}` : ""}${seller?.fopEdrpou ? `<br>${t.code}: ${esc(seller.fopEdrpou)}` : ""}</p><p>${esc(seller?.appContactEmail || "info@onecompany.global")}${seller?.appContactPhone ? `<br>${esc(seller.appContactPhone)}` : ""}</p><div class="bank"><p>${seller?.paymentPurpose ? `${locale === "ua" ? "Призначення платежу:" : "Payment reference (use exactly as written):"}<br><b>${esc(seller.paymentPurpose)}</b>` : `${t.transfer}<br><b>${esc(order.orderNumber)}</b>`}</p>${bankReady ? `<p>IBAN: ${esc(seller?.fopIban)}${seller?.fopBankName ? `<br>${t.bank}: ${esc(seller.fopBankName)}` : ""}</p>${bankExtra}${seller?.fopDetails ? `<p>${esc(seller.fopDetails)}</p>` : ""}` : `<p>${t.absent}</p>`}</div></div><p class="thanks">${t.thanks}</p></div><p class="web">onecompany.global</p><p class="legal">${t.note}</p></footer></main>
<script>
const proformaReady=Promise.all([document.fonts.ready,...Array.from(document.images,image=>new Promise(resolve=>{
  const fallbacks=JSON.parse(image.dataset.imageFallbacks||'[]');
  const finish=()=>{image.removeEventListener('load',finish);image.removeEventListener('error',retry);resolve()};
  const retry=()=>{const next=fallbacks.shift();if(next){image.src=next}else{finish()}};
  image.addEventListener('load',finish);image.addEventListener('error',retry);
  if(image.complete){if(image.naturalWidth){finish()}else{retry()}}
}))]).catch(()=>{});
async function printDocument(button){button.disabled=true;await proformaReady;button.disabled=false;window.print();document.getElementById('print-help').hidden=false}
const recipientSelect=document.getElementById('recipient');
const currencySelect=document.getElementById('currency');
function syncRecipientCurrency(){const required=recipientSelect?.selectedOptions[0]?.dataset.currency;if(!currencySelect)return;if(required)currencySelect.value=required;Array.from(currencySelect.options).forEach(option=>{option.disabled=Boolean(required&&option.value!==required)})}
recipientSelect?.addEventListener('change',syncRecipientCurrency);syncRecipientCurrency();
document.querySelector('.recipient-picker')?.addEventListener('change',()=>{const link=document.getElementById('download-proforma');if(link){link.removeAttribute('href');link.setAttribute('aria-disabled','true')}});
</script></body></html>`;
}
