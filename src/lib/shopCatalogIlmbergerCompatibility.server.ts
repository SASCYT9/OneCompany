...
export function persistIlmbergerCompatibilityInTransaction(input: {
  tx: Prisma.TransactionClient;
  sourceId: string;
  sourceRecordId: string;
  payloadHash: string;
  normalization: IlmbergerNormalization;
}) {
  ...
  return persistVehicleCompatibilityInTransaction({
    ...
    normalization: {
      ...
      applications: normalization.applications.map((application) => ({
        ...application,
        scope: "moto",
        engineCode: null,
        fuel: null,
