...
export type FiFitmentEntry = {
  id: string;
  handle: string;
  status: string;
  applications: FiFitment[];
};
...
const OFFICIAL_MEDIA_ENRICHMENT: Readonly<Record<string, readonly string[]>> = {
  "fi-exhaust-valvetronic-exhaust-system-toyota-gr86": [
    "https://www.fi-exhaust.com/uploads/product-detail/en/ZN8-GR86.jpg",
  ],
  "fi-exhaust-valvetronic-exhaust-system-ferrari-roma": Array.from(
    { length: 8 },
    (_, index) =>
      `https://www.fi-exhaust.com/uploads/gallery/en/ferrari_roma_exhaust_${8 - index}.jpg`
  ),
};
...
function vehicleLabel(applications: readonly FiFitment[]) {
  return [
    ...new Set(
      applications.map(({ brand, model, body }) => [brand, model, body].filter(Boolean).join(" "))
    ),
  ].join(" / ");
}
...
function extractKitContents(html: string) {
  const section =
    html.match(/<strong[^>]*>Комплектація:\s*<\/strong>[\s\S]*?<li[^>]*>([\s\S]*?)<\/li>/iu)?.[1] ??
    html.match(
      /<strong[^>]*>Основна конфігурація:\s*<\/strong>[\s\S]*?<p[^>]*>([\s\S]*?)<\/p>/iu
    )?.[1] ??
    "";
  return section
    .replace(/<[^>]+>/gu, " ")
...
function kitContentsEn(html: string) {
  return extractKitContents(html)
    ...
    .replace(
      /з індивідуально спроектованими внутрішніми елементами/giu,
      "with custom-engineered internals"
    )
    .replace(/з унікальним внутрішнім дизайном/giu, "with a unique internal design")
    .replace(
      /з (?:ретельно |індивідуально |спеціально |унікально )?(?:спроектованим|розробленим) внутрішнім дизайном/giu,
      "with a purpose-engineered internal design"
    )
    ...
    .replace(
      /для гібридної силової установки з твін-турбо V6/giu,
      "for the hybrid twin-turbo V6 powertrain"
    )
...
function bodyEn(product: FiSourceProduct, applications: readonly FiFitment[]) {
  ...
  return [
    ...
    isDownpipe
      ? ""
      : "<p><strong>System control:</strong> Depending on the selected configuration, valve operation can retain the OEM strategy or use the optional Fi Pro remote and mobile app. The exact supplied components are listed above.</p>",
...
function normalizedFitment(fitment: FiFitmentEntry) {
  const applications = fitment.applications;
  const incompleteApplications = applications.some(
    (application) => !application.brand.trim() || !application.model.trim()
  );
  const knownStatus = VERIFIED_FITMENT_STATUSES.has(fitment.status);
  const verified = knownStatus && applications.length > 0 && !incompleteApplications;
  const reviewNote =
    fitment.status === "REVIEW_REQUIRED"
      ? "fi_fitment_review_required"
      : knownStatus
        ? "fi_fitment_applications_incomplete"
        : "fi_fitment_status_unrecognized";
  ...
  return {
    ...
    vehicleType: rows.length ? "car" : "unknown",
    make: new Set(rows.map((row) => row.make)).size === 1 ? (rows[0]?.make ?? null) : null,
    models: [...new Set(rows.flatMap((row) => row.models))],
    chassisCodes: [...new Set(rows.flatMap((row) => row.chassisCodes))],
    yearRanges: [],
    applications: rows,
    confidence: verified ? "high" : "unknown",
    source: "import",
    verifiedAt: null,
    verifiedBy: verified ? "fi-shopify-csv-v1" : null,
    note: verified ? null : reviewNote,
    dependency: null,
  };
}
...
export function buildFiCanonicalDraft(product: FiSourceProduct, fitment: FiFitmentEntry) {
  ...
  if (!VERIFIED_FITMENT_STATUSES.has(fitment.status) && fitment.status !== "REVIEW_REQUIRED") {
    issues.push("fitment_status_unrecognized");
  }
  if (
    fitment.applications.some(
      (application) => !application.brand.trim() || !application.model.trim()
    )
  ) {
    issues.push("fitment_applications_incomplete");
  }
  if (!variant?.sku) issues.push("sku_missing");
  if (!variant?.price || !/^\d+(?:\.\d+)?$/u.test(variant.price)) issues.push("price_uah_missing");
  const officialImages = product.images.length
    ? []
    : [...(OFFICIAL_MEDIA_ENRICHMENT[product.handle] ?? [])];
  if (!product.images.length && !officialImages.length) issues.push("images_missing");
  if (CYRILLIC_RE.test(englishTitle) || CYRILLIC_RE.test(englishBody))
    issues.push("english_contains_cyrillic");
  const media = [
    ...product.images
      .sort((a, b) => a.position - b.position)
      .map((image) => ({
        externalMediaId: String(image.id),
        mediaType: "IMAGE" as const,
        src: image.src,
        altText: englishTitle,
        position: image.position,
      })),
    ...officialImages.map((src, index) => ({
      externalMediaId: `official:${product.id}:${index + 1}`,
      mediaType: "IMAGE" as const,
      src,
      altText: englishTitle,
      position: index + 1,
    ...
    ...videos.map((video, index) => ({
      externalMediaId: `video:${product.id}:${video.videoId}`,
      mediaType: "EXTERNAL_VIDEO" as const,
      src: video.src,
      altText: `${englishTitle} sound video`,
      position: product.images.length + officialImages.length + index + 1,
    })),
  ];
  const fitmentJson = JSON.stringify(normalizedFitment(fitment));
  return {
    source: {
      sourceKey: "shopify-fi-exhaust",
      externalProductId: String(product.id),
      revision: product.updated_at,
      payloadHash: createHash("sha256").update(JSON.stringify(product)).digest("hex"),
    },
    product: {
      slug: product.handle,
      ...
      seoTitleUa: product.title,
      seoTitleEn: englishTitle,
      seoDescriptionUa: null,
      seoDescriptionEn: null,
      productType: product.product_type || "Вихлопна система",
      productCategory: "exhaust-systems",
      tags: [...new Set(product.tags)],
      stock: "preOrder" as const,
      status: "ACTIVE" as const,
      isPublished: false as const,
      priceUah: variant?.price ?? null,
      priceEur: null,
      compareAtUah: variant?.compare_at_price ?? null,
      image: product.images[0]?.src ?? officialImages[0] ?? null,
    },
    variants: variant
      ? [
          {
            ...
            compareAtUah: variant.compare_at_price,
            isDefault: true,
          },
        ]
      : [],
    ...
    metafields: [
      ...
      {
        externalMetafieldId: `derived:${product.id}:fi.source_vendor`,
        namespace: "fi",
        key: "source_vendor",
        value: product.vendor,
        valueType: "single_line_text_field",
      },
      ...(extractKitContents(product.body_html)
        ? [
            {
              externalMetafieldId: `derived:${product.id}:fi.kit_contents`,
              namespace: "fi",
              key: "kit_contents",
              value: extractKitContents(product.body_html),
      ...
      ...(officialImages.length
        ? [
            {
              externalMetafieldId: `derived:${product.id}:fi.official_media_source`,
              namespace: "fi",
              key: "official_media_source",
              value: "https://www.fi-exhaust.com/",
              valueType: "url",
            },
          ]
        : []),
