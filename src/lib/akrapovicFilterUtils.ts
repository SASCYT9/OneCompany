...
export const MODEL_PATTERNS_BY_BRAND: Record<string, { model: string; pattern: RegExp }[]> = {
  Porsche: [
    { model: "911", pattern: /\b(?:911|99[127]|GT3(?:\s*RS)?)\b/i },
  ...
  BMW: [
    { model: "1M", pattern: /\b1M\b/i },
    { model: "335i/435i", pattern: /\b(?:335i|435i)\b/i },
    { model: "340i/440i", pattern: /\b(?:340i|440i)\b/i },
    { model: "XM", pattern: /\bXM\b/i },
    ...
    { model: "M6", pattern: /\bM6\b/i },
    { model: "Z4", pattern: /\bZ4\b/i },
    { model: "S1000RR", pattern: /\bS\s*1000\s*RR\b/i },
    { model: "S1000R", pattern: /\bS\s*1000\s*R(?!\s*[RX])\b/i },
    { model: "S1000XR", pattern: /\bS\s*1000\s*XR\b/i },
    { model: "M1000RR", pattern: /\bM\s*1000\s*RR\b/i },
    { model: "M1000R", pattern: /\bM\s*1000\s*R(?!\s*[RX])\b/i },
    { model: "M1000XR", pattern: /\bM\s*1000\s*XR\b/i },
    { model: "R1300GS / Adventure", pattern: /\bR\s*1300\s*GS\b/i },
    { model: "R1300R / RS", pattern: /\bR\s*1300\s*(?:R|RS)\b/i },
    { model: "F 900 R", pattern: /\bF\s*900\s*R\b/i },
  ],
  Audi: [
    { model: "S7", pattern: /\bS7\b/i },
  ...
  "Mercedes-AMG": [
    { model: "A35", pattern: /\bA\s*35L?\b/i },
    { model: "C43", pattern: /\bC\s*43\b/i },
    { model: "CLA35", pattern: /\bCLA\s*35\b/i },
    { model: "GLS63", pattern: /\bGLS\s*63\b/i },
    { model: "G500/G550", pattern: /\bG\s*(?:500|550)\b/i },
    ...
    { model: "AMG GT", pattern: /\b(?:AMG\s+GT|Roadster\s+GT)\b/i },
  ],
  Toyota: [
  ...
  Renault: [{ model: "Mégane RS", pattern: /\bM[eé]gane\b/i }, { model: "Clio RS", pattern: /\bClio\b/i }],
  Mini: [{ model: "Cooper", pattern: /\bCooper\b/i }],
  Abarth: [
    { model: "500/500C", pattern: /\b500C?\b/i },
  ...
  Ducati: [
    { model: "Panigale V4", pattern: /\bPanigale\s*V4\b/i },
    { model: "Panigale V2", pattern: /\bPanigale\b(?!\s*V4)/i },
    { model: "Streetfighter V4 2025", pattern: /\bStreetfighter\s*V4\b/i },
    { model: "Streetfighter V2", pattern: /\bStreetfighter\b(?!\s*V4)/i },
    { model: "Multistrada V4 / S / RS / RALLY", pattern: /\bMultistrada\b/i },
    { model: "Diavel 1260", pattern: /\bDiavel\s*1260\b/i },
    { model: "Diavel V4", pattern: /\bDiavel\s*V4\b/i },
  ],
...
const VALID_CHASSIS_BY_BRAND_MODEL: Record<string, Record<string, string[]>> = {
  ...
  BMW: {
    "1M": ["E82"],
    "335i/435i": ["F30", "F31", "F32", "F33", "F36"],
    "340i/440i": ["F30", "F31", "F32", "F33", "F36"],
    XM: ["G09"],
    M2: ["F87", "G87"],
...
export const LINE_PATTERNS: { key: string; label: string; patterns: RegExp[] }[] = [
  ...
  {
    key: "header-ti",
    label: "Optional Header (Titanium)",
    patterns: [/header\b.*(?:titanium|\(ti)/i],
  },
  {
    key: "header-ss",
    label: "Optional Header (SS)",
    patterns: [/header\b.*(?:stainless|ss|\(ss)/i],
  },
  { key: "header", label: "Optional Header", patterns: [/header/i] },
  { key: "heat-shield", label: "Carbon Heat Shield", patterns: [/heat\s*shield/i] },
  {
    key: "muffler-bracket",
    label: "Muffler Bracket",
    patterns: [/muffler\s*bracket/i, /bracket\s*for\s*(?:racing|muffler)/i],
  },
  { key: "noise-damper", label: "Noise Damper", patterns: [/noise\s*damper/i] },
  ...
  { key: "racing", label: "Racing Line", patterns: [/racing\s*(line|link|header)/i] },
  ...
  { key: "protection-bars", label: "Protection Bars", patterns: [/protection\s*bar/i] },
  { key: "footpegs", label: "Footpegs", patterns: [/footpeg/i] },
  {
    key: "accessories",
