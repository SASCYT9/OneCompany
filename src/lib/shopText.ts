...
function titleAlreadyImpliesUrbanQualifier(
...
function buildUrbanTitleQualifiers(locale: SupportedLocale, titleEn: string) {
  ...
  if (/\bfront\b/.test(en))
    push(looksLikeWheelFitment ? "передня вісь" : "передній", "Front", ["front"]);
  ...
  if (/\bpre[-\s]?facelift\b/.test(en))
    push("Pre-Facelift", "Pre-Facelift", ["pre-facelift", "pre facelift"]);
  if (/\bfacelift\b/.test(en) && !/\bpre[-\s]?facelift\b/.test(en))
    push("Facelift", "Facelift", ["facelift"]);
  ...
  if (/\bplain\b/.test(en) && /\burban shield\b/.test(en))
    push("Plain або Urban Shield", "Plain or Urban Shield");
  if (/\boem splitter\b/.test(en)) {
    push(
      /\bwithout\b/.test(en) ? "без OEM splitter" : "з OEM splitter",
      /\bwithout\b/.test(en) ? "without OEM splitter" : "with OEM splitter"
    );
  }
  ...
  if (spacerMatch)
    push(`${spacerMatch[1]} мм`, `${spacerMatch[1]}mm`, [
      `${spacerMatch[1]}mm`,
      `${spacerMatch[1]} мм`,
    ]);
  ...
  if (/\bupper turning signal trim\b/.test(en))
    push("верхні повторювачі повороту", "Upper Turning Signal Trim");
...
function disambiguateUrbanTitle(
  locale: SupportedLocale,
  product: Pick<ShopProduct, "slug" | "title">,
  baseTitle: string
) {
  ...
  for (const qualifier of buildUrbanTitleQualifiers(locale, titleEn)) {
    if (
      !titleHasAny(title, [qualifier]) &&
      !titleAlreadyImpliesUrbanQualifier(locale, title, qualifier)
    ) {
...
function humanizeSlug(slug: string) {
  const raw = normalizeWhitespace(slug).replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim();
  ...
  return raw
    ...
    .map((token) => {
      if (
        /^\d+[a-z]{0,3}$/i.test(token) ||
        /^[a-z]{1,4}\d+[a-z0-9-]*$/i.test(token) ||
        /^[A-Z0-9/-]+$/.test(token)
      ) {
...
function stripOhlinsSkuPrefix(title: string): string {
  if (!title) return title;
  return title
    .replace(/^\s*ÖHLINS\s+[A-Z]{2,4}\s+[A-Z0-9-]{3,10}\s+/i, "")
    .replace(/^\s*OHLINS\s+[A-Z]{2,4}\s+[A-Z0-9-]{3,10}\s+/i, "");
}
...
function fixDo88UaTitleTerms(value: string): string {
  ...
  out = out.replace(
    /\bExtra\s+радіатор\s+охолодження(?![\p{L}\p{N}])/gu,
    "додаткові радіатори охолодження"
  );
...
function applyShopTitleFixes(
  ...
  const isOhlins = brand === "ohlins" || brand === "öhlins" || product.slug.startsWith("ohlins-");
  if (isOhlins) {
    out = stripOhlinsSkuPrefix(out);
  }
  if (brand === "brabus" && locale === "ua") {
    out = out.replace(/спойлерНакладка/g, "спойлер / накладка");
  }
  return out;
}
...
export function localizeShopProductTitle(
  ...
  if (locale === "en") {
    ...
    if (translatedFromDb && !containsCyrillic(translatedFromDb)) {
      return applyShopTitleFixes(
        locale,
        product,
        disambiguateUrbanTitle(locale, product, translatedFromDb)
      );
    }
    ...
    if (translatedFromUa && !containsCyrillic(translatedFromUa)) {
      return applyShopTitleFixes(
        locale,
        product,
        disambiguateUrbanTitle(locale, product, translatedFromUa)
      );
    }
  }
  ...
  return applyShopTitleFixes(
    locale,
    product,
    disambiguateUrbanTitle(
      locale,
      product,
      localizeShopText(locale, product.title, { slugFallback: product.slug })
    )
  );
}

export function localizeShopDescription(locale: SupportedLocale, value: LocalizedValue) {
