...
): { columns: string[]; records: CsvRecord[] } {
...
function shopCatalogStatus(status: string | undefined): AdminShopProductPayload["status"] {
  ...
  if (normalized === "draft" || normalized === "unlisted") return "DRAFT";
  if (normalized === "archived") return "ARCHIVED";
  return "ACTIVE";
}

function isVariantRow(row: CsvRecord): boolean {
  return Boolean(
...
function buildProductPayload(
  ...
  const titleEn =
    firstNonEmptyFromColumns(rows, ["Title (EN)", "Title EN", "title_en", "titleEn"]) ||
    firstNonEmptyValue(rows, "Title");
  const bodyHtmlEn = firstNonEmptyFromColumns(rows, [
    "Body (HTML) (EN)",
    "Body (HTML) EN",
    "body_html_en",
    "bodyHtmlEn",
  ]);
  const categoryEn = firstNonEmptyFromColumns(rows, [
    "Type (EN)",
    "Type EN",
    "category_en",
    "Category EN",
  ...
  const metafields = columns
    .map((column) => {
      ...
      if (!meta) return null;
      const rawValue = rows.find((row) => stringValue(row[column]))?.[column] ?? "";
  ...
  const published = status === "ACTIVE" && boolValue(first["Published"], true);
  const brand =
  ...
  return {
    ...
    bodyHtmlEn: nullableString(bodyHtmlEn || bodyHtml)
      ? sanitizeRichTextHtml(bodyHtmlEn || bodyHtml)
      : null,
    leadTimeUa: null,
    leadTimeEn: null,
    ...
    seoTitleEn:
      firstNullableFromColumns(rows, [
        "SEO Title (EN)",
        "SEO Title EN",
        "seo_title_en",
        "seoTitleEn",
      ]) ?? firstNullableValue(rows, "SEO Title"),
...
export function buildProductsFromShopifyCsv(
  ...
  const productRows = Array.from(grouped.entries())
    .map(([handle, rows]) => {
      ...
      if (!title) {
        errors.push({
          ...
          message: "Title is required for handle group",
        });
        return null;
      }
      return {
    ...
    .filter((entry): entry is { rowNumber: number; product: AdminShopProductPayload } =>
      Boolean(entry)
    );
