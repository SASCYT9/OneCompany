...
export default function CategoryPage() {
  ...
  const categoryInfo = categorySlug
    ? categoryData.find((cat) => cat.slug === categorySlug)
    : undefined;
  const brandCategory: BrandCategory =
    categoryInfo?.segment === "moto" ? "moto" : FALLBACK_BRAND_CATEGORY;
    ...
    return (
      <div className="min-h-screen bg-black text-white flex items-center justify-center p-10">
        <div className="text-white/60">
          {locale === "ua" ? "Категорію не знайдено" : "Category not found"}
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="relative bg-black min-h-screen text-white overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute top-0 left-1/3 w-[600px] h-[600px] bg-linear-to-br from-orange-500/10 via-rose-500/5 to-transparent rounded-full blur-3xl" />
        <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-linear-to-tl from-amber-500/10 via-yellow-500/5 to-transparent rounded-full blur-3xl" />
      </div>
      ...
      <main className="relative container mx-auto px-6 py-20">
        ...
        <header className="mb-16 text-center animate-fade-in">
          ...
          <h1 className="text-5xl md:text-6xl font-light tracking-tight mb-6 bg-clip-text text-transparent bg-linear-to-b from-white to-white/70">
            {locale === "ua" ? meta.title.ua : meta.title.en}
          </h1>
          <p className="text-white/70 font-light text-lg max-w-3xl mx-auto">
            {locale === "ua" ? meta.intro.ua : meta.intro.en}
          </p>
        </header>
        ...
        >
          <div className="relative group">
            <input
              ...
              className="w-full bg-linear-to-br from-white/8 to-white/3 text-white px-8 py-4 border border-white/10 focus:border-orange-500/30 focus:outline-hidden transition-all duration-300 text-sm tracking-wider placeholder:text-white/30 backdrop-blur-xs shadow-lg rounded-xl"
              value={searchQuery}
        ...
        <div className="flex gap-8 items-start">
          ...
          <aside className="hidden lg:block sticky top-24 w-16 shrink-0">
                  ...
                  <a
                    ...
                    className={`group relative flex items-center justify-center w-12 h-12 text-xs font-light tracking-wider transition-all duration-300 ${
                      has
                        ...
                        : "bg-white/2 text-white/15 border border-white/5 cursor-default"
                    }`}
              ...
              {lettersWithBrands.map((letter, idx) => (
                ...
                >
                  <div className="flex items-center gap-3 mb-5">
                    <div className="w-10 h-10 flex items-center justify-center bg-linear-to-br from-white/10 to-white/5 text-white/80 text-sm font-light border border-white/10 rounded-lg shadow-lg">
                      {letter}
                    </div>
                    <div className="flex-1 h-px bg-linear-to-r from-white/20 via-white/10 to-transparent" />
                    <span className="text-xs text-white/40 font-light">
                      {grouped[letter]!.length} {locale === "ua" ? "брендів" : "brands"}
                    </span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {grouped[letter]!.map((brand) => {
                      ...
                      const subcategory = meta
                        ? subcategoryNames[meta.subcategory]?.[lang]
                        : undefined;
                      ...
                      return (
                        <button
                          ...
                          className="group relative p-6 bg-linear-to-br from-white/6 to-white/2 hover:from-white/10 hover:to-white/5 transition-all duration-500 backdrop-blur-xs text-left border border-white/10 rounded-xl shadow-lg hover:shadow-orange-500/10 hover:scale-[1.02]"
                        >
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div className="text-white/90 group-hover:text-white text-lg font-light mb-1">
                                {brand.name}
                              </div>
                              <div className="text-white/40 text-xs tracking-wider uppercase">
                                {country ? (
                                  <span className="text-orange-400/80">{country}</span>
                                ) : null}
                                {country && subcategory ? (
            ...
            {brands.length === 0 && (
              <div className="text-center py-24 text-white/50 animate-fade-in">
                ...
                <p className="text-white/40 text-sm tracking-wider">
                  {locale === "ua" ? "Бренди не знайдено" : "No brands found"}
                </p>
              </div>
            )}
          </div>
      ...
      {showBackToTop && (
        <button
          ...
          className="fixed bottom-8 right-8 z-50 w-12 h-12 bg-linear-to-br from-orange-500 to-rose-500 text-white rounded-full shadow-xl hover:shadow-orange-500/40 hover:scale-110 transition-all duration-300 flex items-center justify-center animate-fade-in"
