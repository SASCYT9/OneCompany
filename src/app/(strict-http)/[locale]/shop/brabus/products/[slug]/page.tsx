...
export async function generateStaticParams() {
  const slugs = await getTopProductSlugsByBrand("brabus", 25);
  const params = [];
...
export async function generateMetadata({ params }: Props) {
  ...
  return getShopProductPageMetadata({ locale: resolvedLocale, slug, mode: "brabus" });
}

export default async function BrabusShopProductPage({ params }: Props) {
  const { locale, slug } = await params;
  const resolvedLocale = resolveLocale(locale);
  return <ShopProductDetailPage locale={resolvedLocale} slug={slug} mode="brabus" />;
}
