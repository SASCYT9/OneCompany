...
> = {
  // Legacy Shopify domains are retained only for webhook compatibility; all
  // customer-facing links now resolve to the OneCompany storefront.
  "1t4mqk-bv.myshopify.com": {
    displayName: "Eventuri · OneCompany",
    publicDomain: "onecompany.global",
    accentColor: "#E2001A",
    logoUrl: "https://onecompany.global/brands/eventuri-logo-email.jpg",
  },
  "eventuri-ua.myshopify.com": {
    displayName: "Eventuri · OneCompany",
    publicDomain: "onecompany.global",
    accentColor: "#E2001A",
    logoUrl: "https://onecompany.global/brands/eventuri-logo-email.jpg",
  },
  "dvstz5-vy.myshopify.com": {
    displayName: "Fi Exhaust",
    publicDomain: "fiexhaust.shop",
    accentColor: "#F47920",
    logoUrl: "https://onecompany.global/brands/fi-logo-dark.jpg",
  },
  "fiexaust.myshopify.com": {
    displayName: "Fi Exhaust",
    publicDomain: "fiexhaust.shop",
    accentColor: "#F47920",
    logoUrl: "https://onecompany.global/brands/fi-logo-dark.jpg",
  },
  "dhs4v6-0y.myshopify.com": {
    displayName: "KW Suspension",
    publicDomain: "kwsuspension.shop",
    accentColor: "#FFD100",
    logoUrl: "https://onecompany.global/images/shop/stores/kw-logo.jpg",
  },
...
function getStoreBranding(domain: string) {
  return (
    STORE_BRANDING[domain] || {
      displayName: "One Company",
      publicDomain: "onecompany.global",
      accentColor: "#c29d59",
      logoUrl: "https://onecompany.global/branding/email-logo-light.jpg",
      logoWidth: 150,
    }
  );
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (storeDomain.includes("fiexhaust") || storeDomain.includes("dvstz5")) {
      ...
      isValid =
        (storeSecret && verifyShopifyWebhook(rawBody, hmac, storeSecret)) ||
        (appSecret && verifyShopifyWebhook(rawBody, hmac, appSecret)) ||
        false;
    ...
    if (!isCrypto) {
      console.log(
        `[Shopify Webhook] Ignored order #${order.order_number}: Selected gateways ${gateways.join(", ")}`
      );
    ...
    console.log(
      `[Shopify Webhook] Generating Whitepay invoice for external_order_id: ${externalOrderId}`
    );
    ...
    if (!whitepayRes.success || !whitepayRes.url) {
      console.error(
        `[Shopify Webhook] Failed to generate Whitepay invoice for #${order.order_number}`,
        whitepayRes
      );
    ...
    const emailLocale =
      shopifyLocale.startsWith("uk") || shopifyLocale.startsWith("ru") ? "ua" : "en";
