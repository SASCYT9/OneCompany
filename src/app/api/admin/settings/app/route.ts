...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (typeof body.contactPhone === "string") {
      updateData.appContactPhone =
        sanitizeText(body.contactPhone).slice(0, MAX_LENGTHS.contactPhone) || null;
    }
    ...
    if (typeof body.showPricesWithVat === "boolean")
      updateData.appShowPricesWithVat = body.showPricesWithVat;

    if (typeof body.metaTitle === "string") {
      updateData.appMetaTitle =
        sanitizeText(body.metaTitle).slice(0, MAX_LENGTHS.metaTitle) || null;
    }

    if (typeof body.metaDescription === "string") {
      updateData.appMetaDescription =
        sanitizeText(body.metaDescription).slice(0, MAX_LENGTHS.metaDescription) || null;
    }
