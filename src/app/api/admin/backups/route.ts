...
import {
  buildAdminPgDumpInvocation,
  getAdminBackupRuntimePolicy,
  pruneLocalBackupFiles,
      ...
      child.on("close", async (code) => {
        if (code !== 0) {
          ...
          resolve(
            NextResponse.json(
              {
                error:
                  "pg_dump завершився з помилкою. Перевірте доступність БД та зовнішню backup automation.",
              },
              { status: 500 }
            )
          );
        ...
        try {
          ...
          const retention = Math.max(
            1,
            parseInt(process.env.ADMIN_LOCAL_BACKUP_RETENTION || "10", 10)
          );
