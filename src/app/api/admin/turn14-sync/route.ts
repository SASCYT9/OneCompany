...
export async function POST(req: Request) {
  try {
    ...
    for (const markup of markups) {
      ...
      await prisma.turn14BrandMarkup.update({
        ...
        data: {
          ...
          syncProgress: 0,
          syncTotal: 0,
          syncMessage: "Starting catalog scan...",
        },
      });
      ...
      while (hasMore) {
        try {
          ...
          console.log(
            `[Turn14 Sync] Fetched page ${page} for brand ${markup.brandName} (${items.length} items)`
          );
          ...
          for (let i = 0; i < validItems.length; i += CHUNK_SIZE) {
            ...
            const processPromises = chunk.map(async (item: any) => {
              ...
              if (attrs.vehicle_make && attrs.vehicle_year && attrs.vehicle_model) {
                fitmentPayload.push({
                  year: Array.isArray(attrs.vehicle_year)
                    ? attrs.vehicle_year[0]
                    : String(attrs.vehicle_year),
                  make: Array.isArray(attrs.vehicle_make)
                    ? attrs.vehicle_make[0]
                    : String(attrs.vehicle_make),
                  model: Array.isArray(attrs.vehicle_model)
                    ? attrs.vehicle_model[0]
                    : String(attrs.vehicle_model),
                  submodel: attrs.vehicle_submodel
                    ? Array.isArray(attrs.vehicle_submodel)
                      ? attrs.vehicle_submodel[0]
                      : String(attrs.vehicle_submodel)
                    : null,
                });
              }

              const basePrice =
                parseFloat(attrs.retail_price || attrs.list_price || attrs.price || "0") || 0;
              ...
              if (fitmentPayload.length > 0) {
                ...
                await prisma.turn14Fitment.createMany({
                  data: fitmentPayload.map((f) => ({ ...f, itemId: item.id })),
                  skipDuplicates: true,
                });
              }
            });
          ...
          console.error(
            `[Turn14 Sync] Error on brand ${markup.brandName} page ${page}:`,
            err.message
          );
          await prisma.turn14BrandMarkup.update({
            where: { brandId: markup.brandId },
          ...
          return NextResponse.json(
            { success: false, error: err.message, step: `Brand ${markup.brandName} page ${page}` },
            { status: 500 }
          );
        }
      }
    ...
    return NextResponse.json({
      success: true,
      message: `Synced ${totalSaved} items across ${markups.length} brands.`,
    });
  } catch (error: any) {
