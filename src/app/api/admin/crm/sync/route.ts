      ...
      for (const atCustomer of customers) {
        try {
          ...
          const existing = await prisma.shopCustomer.findFirst({
            where: {
              OR: [{ notes: { contains: airtableId } }, { companyName: customerName }],
            },
          });
          ...
          if (existing) {
            ...
            await prisma.shopOrder.update({
              ...
              data: {
                ...
                paymentStatus:
                  atOrder.paymentStatus === "Оплачено"
                    ? "PAID"
                    : atOrder.paymentStatus === "Частично оплачено"
                      ? "PARTIALLY_PAID"
                      : "UNPAID",
                pricingSnapshot: {
            ...
            await prisma.shopOrder.create({
              data: {
                ...
                amountPaid:
                  atOrder.paymentStatus === "Оплачено"
                    ? atOrder.clientTotal || atOrder.totalAmount || 0
                    : 0,
