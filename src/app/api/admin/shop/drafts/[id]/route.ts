...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      draftValidUntil: (draft as any).draftValidUntil
        ? (draft as any).draftValidUntil.toISOString()
        : null,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      quoteSentAt: (draft as any).quoteSentAt ? (draft as any).quoteSentAt.toISOString() : null,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      quoteAcceptedAt: (draft as any).quoteAcceptedAt
        ? (draft as any).quoteAcceptedAt.toISOString()
        : null,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      quoteDeclinedAt: (draft as any).quoteDeclinedAt
        ? (draft as any).quoteDeclinedAt.toISOString()
        : null,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.validUntil !== undefined)
      data.draftValidUntil = body.validUntil ? new Date(body.validUntil) : null;
    ...
    if (data.shippingCost !== undefined || data.taxAmount !== undefined) {
      data.total =
        ...
        Number(data.shippingCost ?? draft.shippingCost) +
        Number(data.taxAmount ?? draft.taxAmount);
    }
