...
export async function POST(request: NextRequest) {
  try {
    ...
    await assertAdminRequest(cookieStore, ADMIN_PERMISSIONS.SHOP_ORDERS_WRITE || "ADMIN_ALL");
    ...
    if (!customer) {
      return NextResponse.json(
        { error: "Customer not found. Please register them first." },
        { status: 404 }
      );
    }
    ...
    const productTitle = `${brand} ${itemName}`;
    const productSlug = `turn14-${partNumber.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`;
    ...
    return NextResponse.json(
      { error: error.message || "Failed to process Quick Order" },
      { status: 500 }
    );
  }
}
