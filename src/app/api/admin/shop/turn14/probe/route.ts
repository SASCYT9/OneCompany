...
async function probe(
  ...
  try {
    ...
    for (const it of items) {
      const name = (it?.attributes?.brand_name || it?.attributes?.brand || "<none>")
        .toString()
        .trim();
    ...
    return {
      ...
      firstItem: first
        ? {
            ...
            brandId:
              first.attributes?.brand_id !== undefined
                ? String(first.attributes.brand_id)
                : undefined,
...
export async function GET(req: Request) {
  ...
  const probes = [
    ...
    {
      q: `filter%5Bbrand_id%5D=${brandId}`,
      label: "C. ?filter%5Bbrand_id%5D=N (encoded JSON:API)",
    },
  ...
  try {
    ...
    if (r.ok) {
      ...
      const dist: Record<string, number> = {};
      for (const it of items) {
        const n = (it?.attributes?.brand_name || it?.attributes?.brand || "<none>")
          .toString()
          .trim();
        dist[n] = (dist[n] || 0) + 1;
      }
      results.push({
        ...
        firstItem: items[0]
          ? {
              ...
              partNumber: items[0].attributes?.part_number,
              brandId: String(items[0].attributes?.brand_id ?? ""),
              brandName: items[0].attributes?.brand_name,
            }
          : undefined,
      });
    } else {
