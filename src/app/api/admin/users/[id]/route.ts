...
type AdminUsersPatchDependencies = {
  ...
  writeAdminAuditLog: (
    ...
    session: ReturnType<typeof assertAdminRequest>,
...
const defaultDependencies: AdminUsersPatchDependencies = {
  ...
  requiredPermission: ADMIN_PERMISSIONS.SHOP_SETTINGS_WRITE,
};
...
export function createPatchAdminUserRoute(
  ...
  return async function PATCH(request: NextRequest, context: AdminUsersPatchContext) {
    try {
      ...
      const session = dependencies.assertAdminRequest(cookieStore, dependencies.requiredPermission);
