          ...
          {categoryData.filter(cat => cat.segment === 'moto').map((cat) => (
            ...
            >
              ...
              <div className="relative flex flex-col flex-1">
                ...
                <div className="mt-4 min-h-[80px] flex flex-wrap content-start gap-1.5 text-[10px] uppercase tracking-[0.25em] sm:gap-2 sm:text-[11px] sm:tracking-[0.3em]">
                  {cat.brands.slice(0, 4).map((name) => (
                    <span key={name} className="rounded-full border border-white/20 bg-white/5 px-2.5 py-0.5 text-white/70 hover:border-white/40 hover:bg-white/10 transition-colors sm:px-3 sm:py-1">
                      {name}
                    </span>
                  ))}
                  {cat.brands.length > 4 && (
                    <span className="rounded-full border border-white/20 bg-white/10 px-2.5 py-0.5 text-white/50 sm:px-3 sm:py-1">
                      +{cat.brands.length - 4} {tPage('more')}
                    </span>
                  )}
                </div>
                {/* Open link - always at bottom */}
                <div className="mt-auto pt-4 flex items-center gap-2 text-[10px] uppercase tracking-[0.3em] sm:pt-6 sm:gap-3 sm:text-xs sm:tracking-[0.35em]">
