  ...
  return (
    <div className="relative bg-black min-h-screen text-white overflow-hidden">
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-[500px] h-[500px] bg-[radial-gradient(circle,rgba(255,255,255,0.03)_0%,transparent_70%)] rounded-full blur-3xl" />
      </div>
      ...
      <main className="relative container mx-auto px-6 py-24">
        <header className="text-center mb-16">
          ...
          <h1 className="text-5xl md:text-6xl font-light tracking-tight mb-4 bg-clip-text text-transparent bg-linear-to-b from-white to-white/70 text-balance">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {CATEGORY_META.map((cat) => (
            <Link
              key={cat.slug}
              href={cat.slug}
              className="group relative p-8 bg-white/2 hover:bg-white/5 transition-all duration-500 backdrop-blur-3xl rounded-3xl border border-white/10 shadow-[0_30px_60px_rgba(0,0,0,0.5)] hover:scale-[1.02]"
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 h-px bg-linear-to-r from-transparent via-white/0 to-transparent group-hover:via-white/20 transition-all duration-500" />
            </Link>
          ))}
        </section>
      </main>
