...
export default function ShopPageClient({ locale, variant = "default" }: ShopPageClientProps) {
  ...
  const brandCards = useMemo(() => {
    ...
    const order = new Map<string, number>(
      featuredBrandOrder.map((brand, index) => [brand.toLowerCase(), index])
    );
  ...
  const filteredProducts = useMemo(() => {
    ...
    const filtered = scopedProducts.filter((product) => {
      ...
      const shortDescription = localizeShopDescription(
        locale,
    ...
    return filtered.sort((a, b) => {
      const priceA = computePricesFromUah(
        a.price,
        rates && { EUR: rates.EUR, USD: rates.USD, UAH: rates.UAH }
      );
      const priceB = computePricesFromUah(
        b.price,
        rates && { EUR: rates.EUR, USD: rates.USD, UAH: rates.UAH }
      );
      ...
      const featuredScore = (brand: string) => {
        const index = featuredBrandOrder.findIndex(
  ...
  const stats = useMemo(() => {
    return {
      ...
      categories: new Set(
        scopedProducts.map((product) => localizeShopText(locale, product.category))
      ).size,
    };
  }, [locale, scopedProducts]);
          ...
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-light text-[#171511]">
                {isUa ? "Обери бренд" : "Select a brand"}
              </h2>
            </div>
            <button
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            ...
            >
              ...
              <p className="mt-1 text-xs opacity-70">
                {isUa ? "Доступні в каталозі" : "Available in catalog"}
              </p>
            </button>
        ...
        <section className="space-y-5">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              ...
              <h2 className="mt-2 text-3xl font-light text-[#171511]">
                {isUa ? "Категорії та колекції" : "Collections and categories"}
              </h2>
            </div>
            <button
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            >
              <p className="text-[11px] uppercase tracking-[0.24em] opacity-75">
                {isUa ? "Усі категорії" : "All categories"}
              </p>
            ...
            <div className="relative">
              ...
              >
                ...
                <option value="priceLow">
                  {isUa ? "Ціна: спочатку дешевші" : "Price: low to high"}
                </option>
                <option value="priceHigh">
                  {isUa ? "Ціна: спочатку дорожчі" : "Price: high to low"}
                </option>
              </select>
            </div>
          </div>
                          ...
                          return (
                            <>
                              <p className="mt-1 text-2xl font-light text-[#171511]">
                                {currency === "USD" && formatPrice(locale, primaryAmount, "USD")}
