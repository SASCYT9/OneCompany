...
import { Search, Filter, ChevronDown, ShoppingCart, Package, Loader2, X, Check } from 'lucide-react';
...
export default function StockPage() {
  ...
  // Load initial data: years + brands
  useEffect(() => {
    Promise.all([
      fetch('/api/shop/turn14/fitment').then(r => r.json()),
      fetch('/api/shop/turn14/brands').then(r => r.json()),
      fetch('/api/shop/stock/search?q=').then(r => r.json()),
    ]).then(([fitmentRes, brandsRes, catalogRes]) => {
      setYears(fitmentRes.data || []);
      setBrands(brandsRes.data || []);
      setItems(catalogRes.data || []);
      setTotalPages(catalogRes.meta?.totalPages || 1);
      setHasSearched(true);
    }).catch(console.error);
  }, []);
  ...
  }, [query, brand, year, make, model, submodel]);
        ...
        <aside className="w-full lg:w-72 shrink-0">
          <div className="sticky top-28">
            ...
            <form onSubmit={handleSearch} className="flex flex-col gap-5">
              {/* Text Search */}
              <div>
                <label className="block text-[9px] uppercase tracking-widest text-white/30 mb-2">{isUa ? 'Пошук' : 'Search'}</label>
                <div className="flex items-center border border-white/10 bg-white/[0.02] focus-within:border-white/30 transition-colors">
        ...
        <main className="flex-1 min-w-0">
          ...
          {hasActiveFilters && (
            <div className="flex flex-wrap gap-2 mb-6">
              {year && <span className="text-[10px] uppercase tracking-widest bg-white/5 border border-white/10 px-3 py-1.5 text-white/60">{year}</span>}
              {make && <span className="text-[10px] uppercase tracking-widest bg-white/5 border border-white/10 px-3 py-1.5 text-white/60">{make}</span>}
              {model && <span className="text-[10px] uppercase tracking-widest bg-white/5 border border-white/10 px-3 py-1.5 text-white/60">{model}</span>}
              {submodel && <span className="text-[10px] uppercase tracking-widest bg-white/5 border border-white/10 px-3 py-1.5 text-white/60">{submodel}</span>}
              {brand && <span className="text-[10px] uppercase tracking-widest bg-white/5 border border-white/10 px-3 py-1.5 text-white/60">{brand}</span>}
              {query && <span className="text-[10px] uppercase tracking-widest bg-white/5 border border-white/10 px-3 py-1.5 text-white/60">"{query}"</span>}
            </div>
          )}
          ...
          {loading ? (
            <div className="flex flex-col items-center justify-center py-40">
              ...
              <p className="text-white/30 text-sm uppercase tracking-widest">{isUa ? 'Завантаження каталогу...' : 'Loading catalog...'}</p>
            </div>
            ...
            <div className="text-center py-40 border border-white/5 bg-white/[0.01]">
              ...
              <h3 className="text-lg font-light text-white mb-2 tracking-wide">
                {isUa ? "Деталей не знайдено" : "No parts found"}
              </h3>
              <p className="text-white/30 text-sm font-light">
                {isUa ? "Спробуйте змінити параметри пошуку або обрати інший автомобіль." : "Try adjusting your search or select a different vehicle."}
              </p>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-px bg-white/5 border border-white/5">
                {items.map((item) => (
                  <div key={item.id} className="bg-[#0a0a0c] p-5 flex flex-col group hover:bg-white/[0.02] transition-colors">
                    {/* Image */}
                    ...
                    <div className="flex-1">
                      <p className="text-[9px] uppercase tracking-widest text-white/40 mb-1">{item.brand}</p>
                      <h3 className="text-sm font-light text-white mb-1 leading-tight line-clamp-2">{item.name}</h3>
                      <p className="text-[10px] text-white/30 mb-3">PN: {item.partNumber}</p>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-white/5 mt-auto">
                      ...
                      <button
                        ...
                        className="text-[9px] uppercase tracking-widest font-bold text-white border border-white/20 px-4 py-2 hover:bg-white hover:text-black transition-colors"
                      >
                        {isUa ? 'ЗАМОВИТИ' : 'ORDER'}
                      </button>
                    </div>
            ...
            {orderSuccess ? (
              ...
              <form onSubmit={async (e) => {
                ...
                <div className="bg-white/[0.03] p-3 mb-5 flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-white">{orderItem.price ? `$${orderItem.price.toFixed(2)}` : '—'}</span>
                </div>
                ...
                <button type="submit" disabled={ordering}
                  ...
                  {ordering ? (isUa ? 'ОФОРМЛЯЄМО...' : 'PROCESSING...') : (isUa ? 'ПІДТВЕРДИТИ ЗАМОВЛЕННЯ' : 'CONFIRM ORDER')}
                </button>
              </form>
            )}
          </div>
