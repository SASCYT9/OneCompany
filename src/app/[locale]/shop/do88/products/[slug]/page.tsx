...
export async function generateStaticParams() {
  const slugs = await getTopProductSlugsByBrand("do88", 25);
  const params = [];
...
export async function generateMetadata({ params }: Props) {
  ...
  return getShopProductPageMetadata({ locale: resolvedLocale, slug, mode: "do88" });
}

export default async function Do88ShopProductPage({ params }: Props) {
  const { locale, slug } = await params;
  const resolvedLocale = resolveLocale(locale);
  return <ShopProductDetailPage locale={resolvedLocale} slug={slug} mode="do88" />;
}
