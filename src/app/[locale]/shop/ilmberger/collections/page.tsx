...
export default async function IlmbergerCollectionsPage({ params }: Props) {
  ...
  const [settingsRecord, ilmbergerRows, systemBrandMap] = await Promise.all([
  ...
  const viewerContext = buildShopViewerPricingContext(
    ...
    null,
    {
      ...
      // customerBrandDiscountMap is omitted (anon SSR baseline); the
      // client-side `useShopViewerContext` lazy-loads per-customer
      // overrides via `/api/shop/brand-discounts/me` after hydration.
    }
  );
