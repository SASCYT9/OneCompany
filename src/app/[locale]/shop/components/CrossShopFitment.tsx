...
function CrossShopCard({ match, locale }: { match: CrossShopMatch; locale: SupportedLocale }) {
  ...
  return (
    <Link
      href={href}
      role="listitem"
      className="group flex w-[280px] shrink-0 snap-start flex-col overflow-hidden rounded-2xl border border-white/12 bg-white/2 transition-all duration-300 hover:border-white/30 hover:bg-white/5 sm:w-[300px]"
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        <p className="text-[10px] uppercase tracking-[0.18em] text-white/45">{product.brand}</p>
        ...
        {fitmentChips.length ? (
          <ul className="flex flex-wrap gap-1.5" aria-label={isUa ? "Сумісність" : "Fitment"}>
            {fitmentChips.map((chip) => (
              <li
                key={chip}
                className="rounded-full border border-white/10 bg-white/4 px-2 py-0.5 text-[10px] uppercase tracking-[0.12em] text-white/60"
              >
                {chip}
              </li>
            ))}
