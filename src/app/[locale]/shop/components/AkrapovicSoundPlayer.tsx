...
export default function AkrapovicSoundPlayer({ entry, isUa }: Props) {
  ...
  const drawVisualizer = useCallback(() => {
    ...
    const draw = () => {
      ...
      for (let i = 0; i < bufferLength; i++) {
        ...
        // Create a burning/exhaust gradient
        const r = barHeight + (25 * (i / bufferLength));
        const g = 60 * (i / bufferLength);
        const b = 20;
  ...
  return (
    <div
      className={`ak-sound-card ${isPlaying ? "ak-sound-card--playing" : ""} overflow-hidden group border border-white/5 bg-[#080808] transition-colors duration-500 hover:border-red-500/30 rounded-sm relative`}
      onClick={toggle}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div className="ak-sound-card__img-wrap relative w-full aspect-[4/3] bg-black overflow-hidden flex items-center justify-center p-4">
        ...
        <div className={`absolute inset-0 bg-red-600/10 transition-opacity duration-1000 mix-blend-screen pointer-events-none ${isPlaying ? "opacity-100" : "opacity-0"}`}></div>
        ...
        {/* Play/Pause button */}
        <div className={`absolute inset-0 flex items-center justify-center bg-black/40 group-hover:bg-black/20 transition-all duration-300 z-20 ${isPlaying ? 'bg-transparent group-hover:bg-black/10' : ''}`}>
          <div className={`w-16 h-16 rounded-full flex items-center justify-center bg-red-600/90 text-white shadow-[0_0_30px_rgba(220,38,38,0.5)] transition-transform duration-500 ${isPlaying ? 'scale-75 opacity-20' : 'group-hover:scale-110 opacity-90'}`}>
            {isPlaying ? (
      ...
      <div className="ak-sound-card__body p-5 relative z-30 bg-[#080808]">
        ...
        <h3 className="ak-sound-card__model text-lg font-medium text-white mb-1 group-hover:text-red-500 transition-colors">{entry.model}</h3>
        ...
        <div className="ak-sound-card__specs flex items-center justify-between border-t border-white/5 pt-4 mt-auto">
          ...
          <span className="ak-sound-card__spec flex flex-col text-right">
            ...
            <strong className="text-[#ff3b3b] text-xs font-light">{entry.weightSaving}</strong>
          </span>
        </div>
      </div>
    </div>
