...
function DetailListPanel({
  ...
  return (
    <div
      className={`rounded-2xl border bg-foreground/15 dark:bg-black/30 p-5 ${accentClassName ?? "border-foreground/15"} ${className ?? ""}`}
    >
      {title ? (
      ...
      <ul
        className={`${title ? "mt-4" : ""} space-y-3 text-sm leading-relaxed text-foreground/95 dark:text-foreground/82 sm:text-[15px]`}
      >
        {items.map((item) => (
          <li key={item} className="flex gap-3">
            <span className="mt-2 inline-block h-1.5 w-1.5 shrink-0 rounded-full bg-[hsl(var(--primary))]/80" />
...
function DetailSpecPanel({
  ...
  return (
    <div
      className={`rounded-2xl border border-foreground/15 bg-foreground/15 dark:bg-black/30 p-5 ${className ?? ""}`}
    >
      {title ? (
        <p className="text-[11px] uppercase tracking-[0.24em] text-foreground/65 dark:text-foreground/50">
          {title}
      ...
      <dl className={`${title ? "mt-4" : ""} grid gap-3 text-sm sm:grid-cols-2`}>
        {specs.map((spec) => (
          <div
            ...
            className="grid gap-1 rounded-xl border border-foreground/10 bg-foreground/[0.03] p-3"
          >
...
function isWheelProduct(product: Pick<ShopProduct, "category" | "productType" | "tags">) {
  ...
  return values.some((value) =>
    /\b(wheel|wheels|tyre|tyres|tire|tires|диск|диски|шини)\b/i.test(value)
  );
}

function isKitProduct(
  product: Pick<ShopProduct, "category" | "productType" | "tags" | "bundle" | "title">
) {
  ...
  return (
    ...
    /\b(body\s?kit|bodykits|bodykit|full kit|complete kit|carbon fibre v2 kit|kit package|replacement bumper package|комплект обвісу|обвіси|повний комплект|пакет urban|пакет заміни бамперів)\b/i.test(
      kitHaystack
    )
  );
}
...
function isStandaloneUrbanComponent(product: Pick<ShopProduct, "title">) {
  ...
  return /(d-pillar|spoiler|roof|light bar|exhaust|tailpipe|bonnet|hood|mirror|trim|intake|насадк|спойлер|дахов|світлов|капот|оздоблен|повітрозабірник)/i.test(
    titleValue
  );
}
...
function isSameVehicleFamily(
  product: Pick<ShopProduct, "brand" | "vendor" | "title" | "collection" | "tags">,
  current: Pick<ShopProduct, "brand" | "vendor" | "title" | "collection" | "tags">
) {
...
function getFirstExternalProductImage(product: Pick<ShopProduct, "image" | "gallery">) {
  return (
    [product.image, ...(product.gallery ?? [])]
      ...
      .find(
        (value) => /^https?:\/\//i.test(value) && !/coming[\s_-]*soon|placeholder/i.test(value)
      ) ?? null
  );
}
...
export default async function ShopProductDetailPage({ locale, slug, mode = "default" }: Props) {
  const resolvedLocale = resolveLocale(locale);
  ...
  const descriptionSections = extractShopProductDescriptionSections(
    supplierLongDescription || shortDescription
  );
  if (do88Enriched) {
  ...
  const isAkrapovicMainProduct = /akrapovi[cč]/i.test(product.brand) && !isAkrapovicAccessory;
  ...
  const existingSpecKeys = new Set(
    detailSpecs.map(
      (spec) => `${normalizeSpecComparable(spec.label)}::${normalizeSpecComparable(spec.value)}`
    )
  );
  ...
  const isUrbanBodyKitPage =
    isUrbanMode && (product.slug === "urb-bun-25358207-v1" || isKitProduct(product));
  ...
  const safeImageUrl = resolvedImageStr.startsWith("//")
    ...
    : resolvedImageStr.startsWith("http") || resolvedImageStr.startsWith("/")
      ? resolvedImageStr
      : `${baseUrl}${resolvedImageStr}`;
  ...
  if (isUrbanMode) {
    ...
    contextLinkHref = urbanCollectionHandle
    ...
    contextLinkHref = do88CollectionHandle
      ? `/${resolvedLocale}/shop/do88/collections/${do88CollectionHandle}`
      : null;
