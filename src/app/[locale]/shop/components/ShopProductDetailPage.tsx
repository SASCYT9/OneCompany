...
function DetailListPanel({
  ...
  return (
    <div
      className={`rounded-2xl border bg-black/30 p-5 ${accentClassName ?? "border-white/12"} ${className ?? ""}`}
    >
      {title ? (
        <p className="text-[11px] uppercase tracking-[0.24em] text-white/50">{title}</p>
      ) : null}
      <ul
        className={`${title ? "mt-4" : ""} space-y-3 text-sm leading-relaxed text-white/82 sm:text-[15px]`}
      >
        {items.map((item) => (
              ...
              <div className="rounded-2xl border border-white/15 bg-black/40 p-5 space-y-4">
                ...
                {pricing.b2bVisible ? (
                  <div className="rounded-xl bg-cyan-950/30 border border-cyan-500/20 p-4 space-y-3">
                          ...
                          return (
                            <div className="pl-7">
                              ...
                              <p className="text-[11px] text-cyan-100/50 mt-1">
                                {formatPrice(resolvedLocale, b2bPrices.usd, "USD")} /{" "}
                                {formatPrice(resolvedLocale, b2bPrices.uah, "UAH")}
                              </p>
                            </div>
                          );
                        })()
                    ...
                    ) : pricing.bands.b2b?.source === "b2b-discount" &&
                      pricing.bands.b2b.discountPercent != null ? (
                      <div className="pl-7">
