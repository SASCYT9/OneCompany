...
function productMatchesMake(product: ShopProduct, activeMake: string) {
  ...
  if (activeMake === "car_make:chevrolet") {
    return (
      product.tags?.includes("car_make:chevrolet") || product.tags?.includes("car_make:corvette")
    );
  }
...
function extractGirodiscModels(product: ShopProduct, makeKey: string | null) {
  ...
  const candidates = segments
    ...
    .map((value) =>
      value.replace(new RegExp(`^(${terms.map(escapeRegExp).join("|")})\\s+`, "i"), "")
    )
...
function GirodiscProductImage({ src, alt }: { src: string | null | undefined; alt: string }) {
...
export default function GirodiscVehicleFilter({
  ...
  const activeMakeKey = activeMake.startsWith("car_make:")
    ? activeMake.slice("car_make:".length)
    : null;
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeModel !== "all") {
      result = result.filter((p) =>
        extractGirodiscModels(p, activeMakeKey).some((model) => model.key === activeModel)
      );
    }
    ...
    >
      <div className="max-w-[1700px] mx-auto px-6 md:px-12 lg:px-16 pb-20">
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
          ...
          {mobileFilterOpen && (
            <div className="lg:hidden fixed inset-0 bg-black/40 z-20" onClick={closeMobileFilter} />
          )}
              ...
              <div className="py-32 text-center bg-black/40 backdrop-blur-xs border border-foreground/12 dark:border-white/5 rounded-2xl flex flex-col items-center">
                ...
                <p className="text-foreground/65 dark:text-white/50 text-sm max-w-md mx-auto mb-8 leading-relaxed">
                  {searchQuery
                    ...
                    : isUa
                      ? "Діапазон деталей відсутній."
                      : "Components for this category/make are currently unavailable."}
                </p>
                <button
              ...
              <>
                <GirodiscSpotlightGrid className="grid grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-6 lg:gap-8">
                  {displayedProducts.map((product) => {
                    ...
                    const productMakeLabel = productMakeKey
                      ? (GIRODISC_MAKE_LABELS[productMakeKey] ??
                        formatGirodiscLabel(productMakeKey))
                      : null;
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          {/* Card Body */}
                          <div className="px-3 pb-3 pt-3 sm:px-6 sm:pb-6 sm:pt-5 flex flex-col grow relative">
                            <p className="text-[8px] sm:text-[10px] uppercase tracking-[0.16em] sm:tracking-[0.25em] font-bold text-red-600 mb-2 sm:mb-3">
                              {product.sku}
                            </p>
                          ...
                          <div className="absolute top-0 left-0 w-full h-px bg-linear-to-r from-transparent via-red-600 to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-center" />
                          <div className="absolute bottom-0 left-0 w-full h-[2px] bg-linear-to-r from-transparent via-red-600 to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-700 origin-center" />
                        </Link>
