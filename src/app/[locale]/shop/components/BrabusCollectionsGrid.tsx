...
import {
  BRABUS_COLLECTION_CARDS,
  BRABUS_COLLECTIONS_GRID_SETTINGS,
  BRABUS_COLLECTION_BRANDS,
} from '../data/brabusCollectionsList';
import type { BrabusCollectionCard } from '../data/brabusCollectionsList';

const ONE_COMPANY_LOGO = 'https://onecompany.global/branding/logo-light.svg';

type BrabusCollectionsGridProps = {
  locale: SupportedLocale;
  /** When provided, only these cards are shown (e.g. those with a collection page config). */
  cards?: BrabusCollectionCard[];
};
...
export default function BrabusCollectionsGrid({ locale, cards: cardsProp }: BrabusCollectionsGridProps) {
  const isUa = locale === 'ua';
  const rootRef = useRef<HTMLElement>(null);
  const [visible, setVisible] = useState(false);
  const [filter, setFilter] = useState<string>('all');

  const baseCards = cardsProp ?? BRABUS_COLLECTION_CARDS;
  ...
  const subheading = isUa
    ? BRABUS_COLLECTIONS_GRID_SETTINGS.subheadingUk
    : BRABUS_COLLECTIONS_GRID_SETTINGS.subheading;
  ...
  return (
    <section
      ...
      style={{ '--accent': '#ff0000' } as React.CSSProperties} // Brabus red
    >
      <div className="ucg__hero">
        <div
          className="ucg__hero-bg"
          style={{
            backgroundImage: `url(${BRABUS_COLLECTIONS_GRID_SETTINGS.heroImage})`,
          }}
        />
      ...
      {BRABUS_COLLECTIONS_GRID_SETTINGS.showFilters && (
        <div className="ucg__filters">
          ...
          {BRABUS_COLLECTION_BRANDS.map((brand) => {
      ...
      <div className="ucg__grid">
        {filteredCards.map((card, idx) => (
          ...
          >
            <Link
              href={`/${locale}/shop/brabus/collections/${card.collectionHandle}`}
            ...
            {card.productCount && (
              <div className="ucg__card-count" style={{ background: 'var(--accent)' }}>{card.productCount}</div>
            )}
