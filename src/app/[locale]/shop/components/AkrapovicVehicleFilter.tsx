...
export default function AkrapovicVehicleFilter({
  ...
  const availableBrands = useMemo(() => {
    ...
    return [...counts.entries()]
      .sort((a, b) => (brandOrder.indexOf(a[0]) === -1 ? 99 : brandOrder.indexOf(a[0])) - (brandOrder.indexOf(b[0]) === -1 ? 99 : brandOrder.indexOf(b[0])))
    ...
    <>
      ...
      <div>
        ...
        <div className="relative">
          <select value={activeBrand} onChange={(e) => setActiveBrand(e.target.value)} className={selectCls}>
      ...
      {availableLines.length > 0 && (
        <div>
          ...
          <div className="relative">
            <select value={activeLine} onChange={(e) => setActiveLine(e.target.value)} className={selectCls}>
              <option value="all">{isUa ? "Всі типи" : "All Types"}</option>
              {availableLines.map((l) => (
                <option key={l.key} value={l.key}>{l.label} ({l.count})</option>
              ))}
            </select>
            <ChevronDown size={14} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-white/40" />
          </div>
      ...
      <div className={`max-w-[1700px] mx-auto px-6 md:px-12 lg:px-16 ${filterOnly ? "pb-8 pt-4" : "pb-20 pt-4"}`}>
          ...
          <div className="flex items-center justify-between gap-6">
            ...
            <div className="relative w-[260px]">
              ...
              <input
                ...
                className="w-full bg-[#0a0a0a] border border-white/10 rounded-lg pl-9 pr-8 py-2.5 text-xs text-white placeholder-white/30 focus:outline-none focus:border-[#e50000]/50 transition-colors"
              />
              {searchQuery && (
        ...
        {filteredProducts.length === 0 ? (
          <div className="py-32 text-center bg-black/40 backdrop-blur-sm border border-white/5 rounded-2xl flex flex-col items-center">
            ...
            <h3 className="text-xl font-light text-white mb-3">{isUa ? "Нічого не знайдено" : "No Components Found"}</h3>
                      ...
                      {(product.image && product.image.length > 5) ? (
                        <Image
                          ...
                          onError={(e) => {
                            ...
                            const fallback = img.parentElement?.querySelector('.fallback-img') as HTMLImageElement | null;
