...
export default function AkrapovicVehicleFilter({
  ...
  const selectCls =
    "appearance-none bg-card dark:bg-[#0a0a0a] border border-foreground/15 dark:border-white/10 text-foreground dark:text-white text-xs px-4 py-2.5 pr-9 rounded-lg outline-none focus:border-[#e50000]/50 cursor-pointer w-full";
    ...
    <>
      ...
      <div>
        ...
        <div className="relative">
          ...
          <ChevronDown
            size={14}
            className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-foreground/55 dark:text-white/40"
          />
        </div>
      </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="hidden lg:flex flex-col gap-4 mb-8 bg-card/75 dark:bg-[#050505]/70 backdrop-blur-md border border-foreground/10 dark:border-white/4 rounded-2xl p-5 shadow-2xl">
          <div className="flex items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              {hasActiveFilters && (
                <button
                  ...
                  className="text-[10px] uppercase tracking-widest text-[#e50000] hover:text-foreground dark:hover:text-white transition-colors font-medium whitespace-nowrap"
                >
                  {isUa ? "Скинути" : "Reset"}
                </button>
              )}
            ...
            <div className="relative w-[260px]">
              <Search
                size={14}
                className="absolute left-3 top-1/2 -translate-y-1/2 text-foreground/45 dark:text-white/30"
              />
              <input
                type="text"
                value={searchQuery}
              ...
              {searchQuery && (
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-foreground/45 dark:text-white/30 hover:text-foreground dark:hover:text-white"
                >
                  <X size={12} />
                </button>
              )}
          ...
          <div className="grid gap-4 grid-cols-5">
            ...
            {filterOnly ? (
              <div className="flex items-end">
                <Link
                  href={catalogHref}
                  className="w-full flex items-center justify-center gap-2 bg-[#e50000] text-foreground dark:text-white text-[10px] uppercase tracking-[0.28em] font-semibold px-5 py-3 rounded-lg hover:bg-white hover:text-black transition-colors"
                >
        ...
        <div className="lg:hidden sticky top-[72px] z-30 -mx-3 mb-5 flex items-center justify-between gap-3 border-y border-foreground/12 dark:border-white/6 bg-black/75 px-3 py-3 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            <button
              type="button"
              onClick={toggleMobileFilter}
              className="flex items-center gap-2 px-4 py-2.5 bg-card/85 dark:bg-[#050505]/80 backdrop-blur-md border border-foreground/12 dark:border-white/8 rounded-lg text-foreground dark:text-white text-[10px] uppercase tracking-[0.15em] font-semibold hover:border-[#e50000]/40 transition-colors shadow-xl"
            >
              <SlidersHorizontal size={13} />
              {isUa ? "Фільтри" : "Filters"}
              {hasActiveFilters && <span className="w-1.5 h-1.5 rounded-full bg-[#e50000] ml-1" />}
            </button>
            <span className="text-foreground/55 dark:text-white/40 text-xs">
          ...
          {filterOnly ? (
            <Link
              href={catalogHref}
              className="px-4 py-2.5 bg-[#e50000] text-foreground dark:text-white text-[10px] uppercase tracking-[0.15em] font-semibold rounded-lg"
            >
              {isUa ? "Показати" : "Show"}
            </Link>
          ) : (
            <div className="relative">
              <select
                value={sortOrder}
                onChange={(e) => setSortOrder(e.target.value as SortOrder)}
                className="appearance-none bg-card/85 dark:bg-[#050505]/80 border border-foreground/15 dark:border-white/10 text-foreground dark:text-white text-[10px] uppercase tracking-[0.15em] font-semibold px-4 py-2.5 pr-8 rounded-lg outline-none cursor-pointer"
              >
              ...
              <ChevronDown
                size={12}
                className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 text-foreground/65 dark:text-white/50"
              />
            </div>
          )}
        </div>
        ...
        <div
          className={`lg:hidden fixed inset-y-0 left-0 z-50 w-[88vw] max-w-[360px] bg-card dark:bg-[#050505] border-r border-foreground/12 dark:border-white/8 shadow-2xl transform transition-transform duration-300 ${mobileFilterOpen ? "translate-x-0" : "-translate-x-full"}`}
        >
          <div className="flex flex-col gap-5 h-full overflow-y-auto px-5 py-6">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="p-1.5 text-foreground/55 dark:text-white/40 hover:text-foreground dark:hover:text-white"
              >
                <X size={18} />
              </button>
            </div>
            {hasActiveFilters && (
              <button
                ...
                className="text-[10px] uppercase tracking-widest text-[#e50000] hover:text-foreground dark:hover:text-white transition-colors font-medium self-start"
              >
            ...
            {filteredProducts.length === 0 ? (
              <div className="py-32 text-center bg-black/40 backdrop-blur-sm border border-foreground/12 dark:border-white/5 rounded-2xl flex flex-col items-center">
                ...
                <button
                  ...
                  className="px-8 py-3 bg-[#e50000]/15 border border-[#e50000]/40 text-foreground dark:text-white text-[10px] uppercase tracking-widest hover:bg-[#e50000]/25 hover:border-[#e50000]/70 transition-all duration-500 rounded-md font-medium"
                >
                ...
                <AkrapovicSpotlightGrid className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-5 lg:gap-6">
                  {displayedProducts.map((product) => {
                    ...
                    return (
                      <article
                        key={product.slug}
                        className="group relative bg-card/65 dark:bg-[#050505]/60 backdrop-blur-xl overflow-hidden flex flex-col hover:bg-[rgba(10,10,10,0.85)] transition-all duration-500 border border-foreground/10 dark:border-white/4 shadow-2xl"
                      >
                        <Link
                        ...
                        <div className="px-3 pb-3 pt-0 sm:px-5 sm:pb-5 z-20 relative flex gap-2">
                          <Link
                            href={`${productPathPrefix}/${product.slug}`}
                            className="flex-1 flex min-w-0 items-center justify-center gap-1.5 py-2 sm:py-2.5 border border-foreground/20 dark:border-white/15 text-[9px] sm:text-[10px] tracking-widest sm:tracking-[0.3em] uppercase font-light text-foreground/92 dark:text-white/85 hover:text-foreground dark:hover:text-white hover:border-foreground/45 dark:hover:border-white/40 hover:bg-foreground/8 dark:hover:bg-white/6 transition-all duration-300"
                          >
                            {isUa ? "Деталі" : "View"}{" "}
                            <ArrowRight
                              size={11}
                          ...
                          <AddToCartButton
                            ...
                            label={isUa ? "В кошик" : "Add to cart"}
                            labelAdded={isUa ? "У кошику ✓" : "In cart ✓"}
                            className="flex-1 flex min-w-0 items-center justify-center py-2 sm:py-2.5 border border-[#e50000] text-[9px] sm:text-[10px] tracking-widest sm:tracking-[0.3em] uppercase font-light text-foreground dark:text-white bg-[#e50000]/90 hover:bg-[#e50000] transition-all duration-300"
                            variant="inline"
                          />
                        </div>
                      </article>
