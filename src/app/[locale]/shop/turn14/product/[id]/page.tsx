...
export default async function Turn14ProductPage({
  ...
  if (!detailData || !detailData.data) {
    return (
      <div className="min-h-screen bg-background text-foreground pt-32 pb-24 text-center">
  ...
  const primaryImage =
    media.length > 0 ? media[0].links?.image_preview || media[0].links?.image_thumbnail : null;
              ...
              <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                {media.slice(1).map((m: any, idx: number) => {
                  ...
                  return (
                    <div
                      key={idx}
                      className="relative w-24 h-24 shrink-0 cursor-pointer overflow-hidden rounded-xl border border-white/10 bg-white/5 opacity-60 hover:opacity-100 transition-opacity"
                    >
          ...
          <div className="flex flex-col">
            <div className="mb-6">
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs tracking-wider uppercase text-white/40 mt-4">
                ...
                {attr.length && (
                  <span>
                    Dims: {attr.length}x{attr.width}x{attr.height} in
                  </span>
                )}
              </div>
            </div>
              ...
              {displayPriceEur > 0 ? (
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-xs text-white/40">
                    {isUa ? "* Орієнтовна міжнародна ціна" : "* Estimated global price"}
                  </span>
                </div>
              ) : (
            ...
            <MobileProductDisclosure title={isUa ? "Деталі" : "Details"}>
              <div className="prose prose-invert prose-sm max-w-none text-white/70">
                ...
                <p className="whitespace-pre-wrap">
                  {attr.marketing_description ||
                    (isUa
                      ? "Опис відсутній в каталозі дистриб'ютора."
                      : "No description available in distributor catalog.")}
                </p>
