        ...
        <div className="mb-12 border-b border-white/10 pb-8 flex flex-col md:flex-row gap-6 md:items-end justify-between">
          ...
          <form className="relative w-full md:max-w-md pt-2">
            <input
              ...
              placeholder={
                isUa
                  ? "Пошук по артикулу, бренду, ключовим словам..."
                  : "Search for part number, brand..."
              }
              className="w-full rounded-full border border-white/20 bg-white/5 px-5 py-3.5 pl-12 text-sm text-white placeholder:text-white/40 focus:border-indigo-500/50 focus:bg-white/10 focus:outline-hidden focus:ring-1 focus:ring-indigo-500/50 transition-all shadow-[inset_0_2px_10px_rgba(0,0,0,0.5)]"
            />
          ...
          <div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {items.map((item: any) => {
                ...
                const imageUrl =
                  Array.isArray(attr.files) && attr.files.length > 0
                  ...
                  >
                    <div className="relative mb-4 aspect-square w-full overflow-hidden rounded-xl bg-white/5 flex items-center justify-center p-4">
                      {imageUrl ? (
                        <Image
                          src={imageUrl}
                          alt={attr.part_description || item.id}
                          fill
                          className="object-contain"
                          sizes="300px"
                        />
                      ) : (
                        <span className="text-white/20 uppercase tracking-widest text-xs">
                          No Image
                        </span>
                      )}
                    </div>
                    <div className="mt-auto flex flex-col gap-1">
                      ...
                      <h3 className="line-clamp-2 text-sm text-white/90 group-hover:text-white">
            ...
            {meta?.last_page > 1 && (
              <div className="mt-12 flex items-center justify-center gap-2">
                {pageNum > 1 && (
                  <Link
                    href={`/${resolvedLocale}/shop/turn14?q=${q || ""}&page=${pageNum - 1}`}
                    className="px-4 py-2 border border-white/20 rounded-lg text-sm hover:bg-white/10"
                  >
                    {isUa ? "Попередня" : "Prev"}
                  </Link>
                )}
            ...
            {items.map((brand: any) => {
              ...
              return (
                <Link
                  ...
                  className="flex aspect-3/2 items-center justify-center rounded-xl border border-white/10 bg-white/2 p-4 text-center text-sm font-medium text-white/70 transition-all hover:bg-white/5 hover:text-white"
                >
                  {name}
                </Link>
              );
