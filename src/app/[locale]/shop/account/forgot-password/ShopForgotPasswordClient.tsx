      ...
      <div className="mx-auto flex min-h-screen max-w-md items-center px-6 py-24">
        <div className="w-full rounded-[28px] border border-white/10 bg-white/[0.04] p-8 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
          ...
          {!submitted ? (
            <form onSubmit={handleSubmit} className="mt-6 space-y-4">
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full rounded-full border border-white bg-white py-3 text-xs font-semibold uppercase tracking-[0.22em] text-black transition hover:bg-[#c29d59] hover:border-[#c29d59] hover:text-white disabled:opacity-50"
              >
                {submitting
                  ? isUa
                    ? 'Надсилаємо…'
          ...
          <div className="mt-8 flex items-center justify-between text-xs text-white/55">
            <Link href={`/${locale}/shop/account/login`} className="hover:text-white transition">
