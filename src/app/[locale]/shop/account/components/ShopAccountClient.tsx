      ...
      <div className="mx-auto max-w-6xl px-4 pb-20 pt-28 sm:px-6">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <section className="space-y-6 rounded-[28px] border border-white/10 bg-white/5 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
            ...
            <div>
              <h2 className="text-lg font-medium text-white">
                {isUa ? "Баланс (Airtable)" : "Balance (Airtable)"}
              </h2>
                ...
                {crmLoading ? (
                  ...
                  <div className="flex flex-wrap items-end justify-between gap-4">
                    <div>
                      ...
                      <p
                        className={`text-2xl font-light ${crmBalance < 0 ? "text-red-400" : "text-emerald-400"}`}
                      >
                        {crmBalance === 0
                          ? "$0"
                          : crmBalance > 0
                    ...
                    {crmWhoOwes && (
                      <div className="text-right">
                        <span
                          className={`text-[10px] uppercase font-bold tracking-widest px-3 py-1 rounded-full border ${crmWhoOwes.toLowerCase().includes("клиент") || crmWhoOwes.toLowerCase().includes("клієнт") || crmWhoOwes.toLowerCase().includes("должен") ? "border-red-500/20 text-red-400 bg-red-500/5" : "border-emerald-500/20 text-emerald-400 bg-emerald-500/5"}`}
                        >
                          {crmWhoOwes}
                        </span>
                      </div>
              ...
              {profile.defaultShippingAddress ? (
                ...
                <p className="mt-3 text-sm text-white/55">
                  {isUa
                    ? "Поки що немає збереженої адреси. Вона збережеться після оформлення замовлення."
                    : "No saved shipping address yet. It will be stored after your first checkout."}
                </p>
              )}
            </div>
          </section>
                        ...
                        <div className="flex min-w-0 items-start gap-4">
                          ...
                          <div className="min-w-0">
                            <div className="flex flex-wrap items-center gap-2">
                              <div className="font-mono text-sm text-white hover:text-white/90">
                                {order.orderNumber}
                              </div>
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`text-[9px] uppercase tracking-widest px-2 py-0.5 rounded-full border ${
                                o.orderStatus === "Выполнен"
                                  ? "border-emerald-500/20 text-emerald-400 bg-emerald-500/5"
                                  : o.orderStatus === "Отменен"
                          ...
                          {o.items.map((item, idx) => (
                            ...
                            >
                              <div className="min-w-0 flex-1">
                                ...
                                {item.brand && (
                                  <span className="text-white/30 ml-1">({item.brand})</span>
                                )}
                              </div>
                              <div className="text-white/40 shrink-0">
                                {item.quantity} × ${item.price.toLocaleString()}
                              </div>
                              <div className="text-white/70 font-medium shrink-0 w-20 text-right">
...
function InfoCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-black/25 p-4 overflow-hidden">
      ...
      <div className="mt-2 text-sm text-white/80 truncate" title={value}>
        {value}
      </div>
    </div>
  );
