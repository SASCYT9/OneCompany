...
export default function ShopAddressesClient({
  ...
  return (
    <main className="min-h-screen bg-background text-foreground dark:bg-[radial-gradient(circle_at_top,rgba(120,120,120,0.16),transparent_30%),linear-gradient(180deg,#070707_0%,#0f0f0f_55%,#050505_100%)]">
      <div className="mx-auto max-w-4xl px-4 pb-20 pt-36 sm:px-6 md:pt-40 lg:pt-44">
        <Link
        ...
        <header className="mb-8 flex flex-wrap items-end justify-between gap-4">
          ...
          {!showForm ? (
            <button
              type="button"
              onClick={startCreate}
              className="inline-flex items-center gap-2 rounded-full border border-foreground/15 bg-primary px-5 py-2 text-xs uppercase tracking-[0.2em] text-primary-foreground transition hover:bg-primary/90"
            >
        ...
        {showForm ? (
          <section className="mb-10 rounded-[28px] border border-foreground/10 bg-foreground/5 p-6 shadow-[0_24px_80px_rgba(0,0,0,0.35)]">
            ...
            <div className="mt-4 space-y-4">
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <button
                  ...
                  onClick={() => void submit()}
                  disabled={submitting}
                  className="rounded-full border border-primary bg-primary px-6 py-2.5 text-xs uppercase tracking-[0.2em] text-primary-foreground transition hover:bg-primary/90 disabled:opacity-50"
                >
                  {submitting
                    ? isUa
                      ? "Збереження…"
                ...
                <button
                  ...
                  onClick={cancel}
                  disabled={submitting}
                  className="rounded-full border border-foreground/15 bg-transparent px-6 py-2.5 text-xs uppercase tracking-[0.2em] text-foreground/85 dark:text-foreground/70 transition hover:border-foreground/30 hover:text-foreground"
                >
        ...
        <section className="space-y-4">
          {addresses.length === 0 ? (
            <div className="rounded-[28px] border border-dashed border-foreground/15 bg-primary/2 p-10 text-center text-foreground/70 dark:text-foreground/55">
              <MapPin className="mx-auto mb-4 h-8 w-8 text-foreground/45 dark:text-foreground/25" />
              <p>
                {isUa
            ...
            addresses.map((address) => (
              <article
                key={address.id}
                className="flex flex-col gap-4 rounded-[28px] border border-foreground/10 bg-foreground/5 p-6 shadow-[0_18px_48px_rgba(0,0,0,0.22)] sm:flex-row sm:items-center sm:justify-between"
              >
                <div className="min-w-0">
                  ...
                  <p className="mt-3 text-sm text-foreground/90 dark:text-foreground/75">
                    {address.line1}
                  </p>
                  {address.line2 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    disabled={submitting}
                    className="inline-flex items-center gap-1.5 rounded-full border border-foreground/15 bg-foreground/5 px-4 py-2 text-xs uppercase tracking-[0.18em] text-foreground/90 dark:text-foreground/75 transition hover:border-foreground/30 hover:text-foreground disabled:opacity-50"
                  >
...
function Toggle({
  ...
  return (
    <label className="inline-flex cursor-pointer items-center gap-2 text-sm text-foreground/90 dark:text-foreground/75">
      <input
        type="checkbox"
        checked={checked}
        onChange={(e) => onChange(e.target.checked)}
        className="h-4 w-4 rounded border-foreground/30 bg-foreground/5 accent-primary"
      />
      {label}
    </label>
  );
