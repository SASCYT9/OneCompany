...
export default function ShopCartClient({ locale }: { locale: SupportedLocale }) {
  ...
  const subtotal = items.reduce(
    (s, i) => s + (i.price ? getPrice(i.price, currency) * i.quantity : 0),
    0
  );
          ...
          <>
              ...
              {items.map((i) => (
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-light text-lg text-white truncate">
                      {localize(locale, i.title)}
                    </p>
                    {i.variantTitle ? (
                      <p className="mt-1 text-xs text-[#c29d59]/80 uppercase tracking-widest truncate">
                        {i.variantTitle}
                      </p>
                    ) : null}
                    <p className="mt-2 text-sm text-white/55">
                      {i.price ? formatPrice(locale, getPrice(i.price, currency), currency) : ""} ×{" "}
                      {i.quantity}
                    </p>
                  ...
                  <div className="text-left text-lg font-light text-white sm:text-right sm:shrink-0">
                    {i.price
                      ? formatPrice(locale, getPrice(i.price, currency) * i.quantity, currency)
                      : "—"}
                  </div>
                </li>
              ))}
            ...
            <div className="mt-8 rounded-3xl border border-white/10 bg-black/40 p-8 shadow-2xl backdrop-blur-xl">
              <div className="flex justify-between items-center text-xl font-light text-white">
                <span className="text-[#c29d59] uppercase tracking-widest text-xs font-normal">
