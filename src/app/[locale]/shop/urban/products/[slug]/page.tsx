...
export async function generateStaticParams() {
  const slugs = await getTopProductSlugsByBrand("Urban Automotive", 25);
  const params = [];
...
export async function generateMetadata({ params }: Props) {
  ...
  return getShopProductPageMetadata({ locale: resolvedLocale, slug, mode: "urban" });
}

export default async function UrbanShopProductPage({ params }: Props) {
  const { locale, slug } = await params;
  const resolvedLocale = resolveLocale(locale);
  return <ShopProductDetailPage locale={resolvedLocale} slug={slug} mode="urban" />;
}
