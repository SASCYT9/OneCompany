...
export default async function AkrapovicCollectionsPage({ params }: Props) {
  ...
  const akrapovicProducts = products.filter(p => p.brand?.toLowerCase() === 'akrapovič' || p.brand?.toLowerCase() === 'akrapovic' || p.tags?.includes('Akrapovic'));
  ...
  return (
    <div className="relative min-h-screen bg-black">
      ...
      <div className="fixed inset-0 z-0">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-b from-[#0a0000]/65 via-black/85 to-[#050000] backdrop-blur-[8px]" />
      ...
      <div className="relative z-10 pt-[100px]">
        <div className="w-full max-w-[1700px] mx-auto px-6 md:px-12 lg:px-16 pb-6">
          <Link href={`/${locale}/shop/akrapovic`} className="text-[10px] uppercase tracking-[0.2em] text-white/40 hover:text-white transition-colors">
