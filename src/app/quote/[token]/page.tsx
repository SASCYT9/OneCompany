...
export default function CustomerQuotePage() {
      ...
      <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-4">
        <div className="max-w-md rounded-xl border border-red-500/20 bg-red-500/5 p-6 text-center">
          ...
          <p className="mt-2 text-sm text-zinc-400">
            {error || "This quote link is invalid or has been removed."}
          </p>
        </div>
      </div>
    );
  ...
  if (actionResult === "accepted") {
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-4">
        <div className="max-w-md rounded-xl border border-emerald-500/30 bg-emerald-500/6 p-6 text-center">
          ...
          <p className="mt-2 text-sm text-zinc-400">
            ...
            <span className="font-mono text-emerald-300">{quote.orderNumber}</span> is now active.
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-4">
        <div className="max-w-md rounded-xl border border-red-500/20 bg-red-500/5 p-6 text-center">
          ...
          <p className="mt-2 text-sm text-zinc-400">
            We&apos;ve noted that you don&apos;t want to proceed with this quote. If you change your
            mind, contact us.
          </p>
        </div>
      </div>
  ...
  if (quote.expired) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-950 px-4">
        <div className="max-w-md rounded-xl border border-amber-500/25 bg-amber-500/5 p-6 text-center">
          <FileText className="mx-auto h-8 w-8 text-amber-300" />
          <h1 className="mt-3 text-lg font-bold text-zinc-100">Quote expired</h1>
          <p className="mt-2 text-sm text-zinc-400">
            This quote was valid until{" "}
            {quote.validUntil ? new Date(quote.validUntil).toLocaleDateString() : "—"}. Contact us
        ...
        <div className="mb-8 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500">
              For {quote.customerName} · {quote.email}
            </p>
          </div>
        </div>

        <div className="rounded-2xl border border-white/8 bg-[#171717] p-6 shadow-2xl">
              ...
              <div key={idx} className="flex items-center justify-between gap-4 py-3">
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-xs text-zinc-500">
                    Quantity: {it.quantity} × {formatMoney(it.price, quote.currency)}
                  </div>
                </div>
                <div className="font-medium text-zinc-100 tabular-nums">
                  {formatMoney(it.total, quote.currency)}
                </div>
              </div>
            ))}
          </div>

          <div className="mt-6 space-y-2 border-t border-white/8 pt-4 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-zinc-200 tabular-nums">
                {formatMoney(quote.shippingCost, quote.currency)}
              </span>
            </div>
            {quote.taxAmount > 0 ? (
              <div className="flex justify-between">
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row">
            <button
              ...
              disabled={acting}
              className="flex-1 rounded-full bg-linear-to-b from-emerald-500 to-emerald-700 px-6 py-3 text-sm font-bold uppercase tracking-wider text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.18),0_2px_8px_rgba(16,185,129,0.4)] transition hover:from-emerald-400 hover:to-emerald-600 disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={acting}
              className="flex-1 rounded-full border border-white/10 bg-white/3 px-6 py-3 text-sm font-medium text-zinc-300 transition hover:border-red-500/30 hover:bg-red-500/6 hover:text-red-300 disabled:opacity-50"
            >
