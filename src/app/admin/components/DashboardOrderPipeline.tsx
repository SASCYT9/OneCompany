...
export function DashboardOrderPipeline({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-white/4 bg-black/30 px-4 py-2.5">
        ...
        <div className="text-[10px] font-bold uppercase tracking-[0.16em] tabular-nums text-zinc-500">
          ...
          <span className={cn(cancellationRatePct > 5 ? "text-blue-300" : "text-zinc-200")}>
            {cancellationRatePct}%
          </span>
        </div>
      </div>
...
function StageCell({
  ...
  return (
    <Link
      ...
      className={cn(
        "group relative px-4 py-3.5 transition-colors hover:bg-white/3 focus-visible:outline-hidden focus-visible:bg-white/4 focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-inset",
        dim && "opacity-50"
      )}
    >
      ...
      <div className="mt-1.5 text-[22px] font-bold leading-none tabular-nums text-zinc-50">
        {stage.count}
      </div>
      ...
      {stage.oldestAgeDays != null && stage.count > 0 ? (
        <div
          className={cn(
            "mt-1 text-[10px] font-semibold uppercase tracking-wider tabular-nums",
            warn ? "text-amber-300" : "text-zinc-500"
          )}
        >
