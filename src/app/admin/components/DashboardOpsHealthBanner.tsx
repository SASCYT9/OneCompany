...
export function DashboardOpsHealthBanner({ lights }: { lights: HealthLight[] }) {
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 divide-y divide-white/4 sm:grid-cols-3 sm:divide-x sm:divide-y-0 lg:grid-cols-6">
        {lights.map((light) => (
...
function HealthLightCell({ light }: { light: HealthLight }) {
  ...
  if (light.href) {
    return (
      <Link
        href={light.href}
        className="block focus-visible:outline-hidden focus-visible:ring-2 focus-visible:ring-blue-500/50 focus-visible:ring-offset-[-2px] focus-visible:ring-offset-[#0A0A0A]"
