...
export default function AdminIntegrationsPage() {
  ...
  async function testConnection(provider: Provider) {
    setTesting(provider);
    try {
      const response = await fetch(`/api/admin/integrations/${provider}?action=test`, {
        method: "POST",
      });
  ...
  const integrationByProvider = new Map<Provider, Integration>(
  ...
  return (
    <AdminPage className="space-y-6">
      ...
      <AdminInlineAlert tone="warning">
        ...
        <code className="rounded-none bg-white/6 px-1 text-xs">INTEGRATIONS_ENCRYPTION_KEY</code> у
        змінних середовища.
      </AdminInlineAlert>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {allProviders.map((provider) => {
          ...
          return (
            <AdminInspectorCard key={provider} title={info.name} description={info.description}>
              {loading ? (
                ...
                <>
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    {integration?.lastSyncStatus ? (
                      <AdminStatusBadge
                        tone={integration.lastSyncStatus === "SUCCESS" ? "success" : "warning"}
                      >
                        Остання синхр.{" "}
                  ...
                  {connected && integration && !isEditing ? (
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center justify-between rounded-none border border-white/4 bg-black/25 px-3 py-1.5 text-xs">
                      ...
                      {integration.lastSyncError ? (
                        <div className="rounded-none border border-red-500/20 bg-red-500/4 px-3 py-1.5 text-xs text-red-300">
                          {integration.lastSyncError}
                        </div>
                      ) : null}
                      ...
                      <div className="flex flex-wrap items-center gap-2 pt-2">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded-none border border-blue-500/25 bg-blue-500/6 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-blue-300 transition hover:bg-blue-500/12 disabled:opacity-50"
                        >
                          {testing === provider ? (
                        ...
                        <button
                          ...
                          className="rounded-none border border-white/10 bg-white/3 px-3 py-1.5 text-xs text-zinc-300 transition hover:bg-white/6"
                        >
                          Оновити ключі
                        </button>
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 rounded-none border border-red-500/25 bg-red-500/4 px-3 py-1.5 text-xs text-red-300 transition hover:bg-red-500/10"
                        >
