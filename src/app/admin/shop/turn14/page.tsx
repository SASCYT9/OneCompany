  ...
  return (
    <AdminPage className="space-y-6">
      ...
      <AdminMetricGrid>
        ...
        <AdminMetricCard
          label="Stock status"
          value={stockChecking ? "Checking…" : stockFilter}
          meta="Фільтр на таблицю"
        />
      </AdminMetricGrid>
      ...
      <AdminActionBar>
        <div className="space-y-1">
          ...
          <div className="text-sm text-zinc-300">
            Search supplier inventory, звірити з One Company stock, імпортувати товар або створити
            швидке замовлення для клієнта.
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <AdminStatusBadge tone={stockChecking ? "warning" : "default"}>
            {stockChecking ? "Stock checking" : "Stock ready"}
          </AdminStatusBadge>
          <button
            type="button"
            onClick={handleSync}
          ...
          <label className="w-full min-w-0 flex-1 md:min-w-[260px]">
            ...
            <div className="flex items-center gap-2 rounded-none border border-white/10 bg-[#0F0F0F] px-4 py-3">
              ...
              <input
                ...
                className="w-full bg-transparent text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-hidden"
              />
            </div>
          </label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-none border border-white/10 bg-[#09090b] p-8 shadow-[0_0_50px_rgba(0,0,0,0.8)]">
            ...
            <p className="mt-2 text-sm leading-relaxed text-white/50">
              Система створить нове `UNPAID` замовлення для{" "}
              {(selectedItem.attributes as any)?.product_name ||
                (selectedItem.attributes as any)?.item_name ||
                "деталі Turn14"}
              .
            </p>
            <form onSubmit={handleOrderSubmit} className="mt-6 space-y-5">
              <label className="block">
                <span className="mb-2 block text-[11px] uppercase tracking-wider text-white/50">
                  Email клієнта
                </span>
                <input
                  type="email"
                  ...
                  className="w-full rounded-none border border-white/10 bg-black/40 px-4 py-3 text-sm text-white focus:border-white/20 focus:outline-hidden"
