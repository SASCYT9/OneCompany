...
export default function CatalogQualityPage() {
  ...
  async function load(mode: "initial" | "refresh" = "initial") {
    ...
    try {
      ...
      const data = (await response.json().catch(() => ({}))) as CatalogQualityReport & {
        error?: string;
      };
      if (!response.ok) {
  ...
  return (
    <AdminPage className="space-y-6">
      <AdminPageHeader
        ...
        actions={
          <button
            ...
            disabled={refreshing}
            className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/3 px-4 py-2 text-sm text-zinc-200 transition hover:bg-white/6 disabled:opacity-50"
          >
      ...
      {report ? (
        <>
          <AdminMetricGrid>
            <AdminMetricCard
              label="Quality score"
              value={`${report.score}%`}
              meta={`${report.cleanProducts} clean of ${report.totalProducts}`}
              tone="accent"
            />
            <AdminMetricCard
              label="Products with issues"
              value={report.issueProducts}
              meta="Need catalog review"
            />
            <AdminMetricCard
              label="No image"
              value={report.issueCounts.NO_IMAGE}
              meta="Blocks premium storefront trust"
            />
            <AdminMetricCard
              label="Bad SEO"
              value={report.issueCounts.BAD_SEO}
              meta="Needs UA/EN search cleanup"
            />
          </AdminMetricGrid>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-sm text-zinc-500">
                  Use filters to isolate one issue class, then open affected products in edit mode.
                  Destructive bulk mutations stay out of this first version.
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div className="space-y-4">
              <AdminFilterBar>
                ...
                <SelectFilter
                  label="Issue"
                  value={issueFilter}
                  onChange={(value) => setIssueFilter(value as "ALL" | CatalogQualityIssueKey)}
              ...
              {visibleProducts.length ? (
                    ...
                    <table className="w-full min-w-[1080px] text-left text-sm">
                      ...
                      <tbody className="divide-y divide-white/6">
                        {visibleProducts.map((product) => (
                          <tr key={product.id} className="align-top transition hover:bg-white/3">
                            ...
                            <td className="px-4 py-4">
                              <Link
                                ...
                                className="inline-flex items-center gap-2 rounded-full border border-blue-500/25 bg-blue-500/8 px-3 py-2 text-xs font-medium uppercase tracking-[0.18em] text-blue-300 transition hover:bg-blue-500/12"
                              >
                ...
                <AdminEmptyState
                  title="No products match current quality filters"
                  description="Change issue, brand, or search filters to inspect another part of the catalog."
                />
              )}
            </div>
            ...
            <aside className="space-y-4">
              <AdminInspectorCard
                ...
                description="Most common catalog gaps."
              >
                <AdminBarList
              ...
              <AdminInspectorCard
                title="Brand quality score"
                description="Worst brand groups first."
              >
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <div className="mt-1 text-xs text-zinc-500">
                            {brand.issueProducts} with issues · {brand.total} total
                          </div>
                        </div>
                        <AdminStatusBadge tone={qualityTone(brand.score)}>
                          {brand.score}%
                        </AdminStatusBadge>
                      </div>
                      {brand.topIssues.length ? (
                        <div className="mt-2 text-xs text-zinc-500">
                          {brand.topIssues
                            .map(
                              (issue) =>
                                `${CATALOG_QUALITY_ISSUES[issue.key].label}: ${issue.count}`
                            )
                            .join(" · ")}
                        </div>
                      ) : null}
        ...
        <AdminEmptyState
          title="Catalog quality is unavailable"
          description={error || "Quality report did not load."}
        />
      )}
    </AdminPage>
  );
