...
function quoteStage(d: DraftRow): {
  label: string;
...
export default function AdminDraftsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/shop/drafts?${params.toString()}`, {
          cache: "no-store",
        });
  ...
  return (
    <AdminPage className="space-y-6">
      ...
      <AdminMetricGrid>
        <AdminMetricCard
          label="Всього чернеток"
          value={stats.total}
          meta="Активні та в обробці"
          tone="accent"
        />
        <AdminMetricCard
          label="Надіслано · очікуємо"
          value={stats.sent}
          meta="Котирування надіслане, без рішення"
        />
