...
export default function AdminNewDraftPage() {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        setCustomerOptions(
          data
            .filter((c) =>
              [c.email, c.fullName, c.companyName].filter(Boolean).some((v) => String(v).toLowerCase().includes(needle))
            )
            .slice(0, 8)
        );
      } catch {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        const response = await fetch(`/api/admin/shop/products?search=${encodeURIComponent(productSearch)}&limit=12`, {
          cache: 'no-store',
        });
        if (!response.ok) return;
        ...
        <div className="space-y-5">
          ...
          >
              ...
              <div className="mt-2 overflow-hidden rounded-lg border border-white/[0.05] bg-[#0F0F0F]">
                {customerOptions.map((c) => (
                  <button
                    ...
                    className="flex w-full items-center justify-between gap-3 border-b border-white/[0.04] px-3 py-2 text-left text-sm transition last:border-b-0 hover:bg-white/[0.04]"
                  >
                    <div className="min-w-0">
                      <div className="truncate font-medium text-zinc-100">{c.fullName || c.email}</div>
                      <div className="mt-0.5 truncate text-[11px] text-zinc-500">{c.email}{c.companyName ? ` · ${c.companyName}` : ''}</div>
                    </div>
                    <span className="rounded-full border border-white/[0.08] bg-white/[0.04] px-1.5 py-0 text-[9px] font-bold uppercase tracking-wider text-zinc-400">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <AdminInputField label="Email" value={manualEmail} onChange={setManualEmail} type="email" />
              <AdminInputField label="Customer name" value={manualName} onChange={setManualName} />
              <AdminInputField label="Phone (optional)" value={manualPhone} onChange={setManualPhone} />
              <AdminSelectField
                label="Currency"
                value={currency}
              ...
              <div className="mt-2 max-h-72 overflow-auto rounded-lg border border-white/[0.05] bg-[#0F0F0F]">
                {productOptions.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0">
                        <div className="truncate font-medium text-zinc-100">{p.titleEn || p.titleUa}</div>
                ...
                <AdminTableShell>
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-white/6">
                      {items.map((it, idx) => (
                        <tr key={`${it.productSlug}-${idx}`} className="hover:bg-white/[0.03]">
          ...
          <AdminInspectorCard title="Quote settings" description="Validity, shipping, internal note.">
            <div className="grid gap-4 md:grid-cols-2">
              <AdminInputField label={`Shipping (${currency})`} value={shippingCost} onChange={setShippingCost} type="number" step="0.01" />
              <AdminInputField label="Valid until" value={validUntil} onChange={setValidUntil} type="datetime-local" />
            </div>
            <div className="mt-4">
              <AdminTextareaField label="Internal note" value={internalNote} onChange={setInternalNote} rows={3} />
            </div>
          </AdminInspectorCard>
        </div>
        ...
        <aside className="space-y-4">
          <AdminInspectorCard title="Summary" description="Quote totals.">
            ...
            <button
              ...
              className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-gradient-to-b from-blue-500 to-blue-700 px-4 py-2 text-sm font-bold uppercase tracking-wider text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.18),0_2px_8px_rgba(59,130,246,0.4)] transition hover:from-blue-400 hover:to-blue-600 disabled:opacity-50"
            >
