  ...
  for (const row of rows) {
    ...
    if (!map.has(handle)) {
      map.set(handle, {
        ...
        retailPrice:
          parseFloat(row["Variant Compare At Price"] || row["Variant Price"] || "0") || null,
...
export default function AdminStockPage() {
  ...
  const [stats, setStats] = useState<{ total: number; distributors: DistributorStat[] } | null>(
    null
  );
  ...
  const handleDelete = async (dist: string) => {
    const ok = await confirm({
      ...
      description:
        "Усі імпортовані товари цього дистриб'ютора будуть видалені з каталогу. Дію не можна скасувати.",
    ...
    <div className="w-full h-full overflow-auto text-white">
      <div className="w-full px-4 py-8 md:px-8 lg:px-12 max-w-[1920px]">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-white/45">
              Імпорт Shopify CSV від дистриб&apos;юторів (IND та інших). Дані з&apos;являються на
              сторінці Stock.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={loadStats}
              className="inline-flex items-center gap-2 rounded-none border border-white/8 bg-transparent hover:bg-white/5 transition-all px-4 py-2 text-sm font-medium text-white/80"
            >
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-4">
          <div className="rounded-none border border-white/8 bg-black/40 p-5 backdrop-blur-xs">
          ...
          {stats?.distributors?.map((d) => (
            <div
              key={d.name}
              className="rounded-none border border-white/8 bg-black/40 p-5 backdrop-blur-xs"
            >
        ...
        {/* Import Section */}
        <div className="rounded-none border border-white/8 bg-black/40 p-6 backdrop-blur-xs">
          ...
          <div className="grid gap-4 md:grid-cols-[200px_1fr_auto]">
            <div>
              ...
              <select
                ...
                className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-white/30"
              >
                <option value="IND" className="bg-[#121216]">
                  IND
                </option>
                <option value="OTHER" className="bg-[#121216]">
                  Інший
                </option>
              </select>
            </div>
            ...
            <div>
              ...
              <label className="flex items-center gap-3 cursor-pointer bg-white/3 border border-white/10 border-dashed rounded-none px-4 py-2.5 hover:bg-white/5 transition-colors">
          ...
          <div className="mt-4 rounded-none bg-blue-500/15 text-zinc-100/5 border border-blue-500/10 p-3">
            <p className="text-[10px] uppercase tracking-widest text-zinc-500/70 mb-1.5 font-medium">
              Shopify CSV формат
            </p>
            <p className="text-xs text-white/40 leading-relaxed">
              Стандартний експорт із Shopify Admin → Products → Export. Колонки:{" "}
              <code className="text-white/60">Handle</code>,{" "}
              <code className="text-white/60">Title</code>,{" "}
              <code className="text-white/60">Vendor</code>,{" "}
              <code className="text-white/60">Type</code>,{" "}
              <code className="text-white/60">Variant SKU</code>,{" "}
              <code className="text-white/60">Variant Price</code>,{" "}
              <code className="text-white/60">Image Src</code>. Мульти-варіантні товари автоматично
              групуються по Handle.
            </p>
          </div>
          ...
          {showPreview && preview && preview.length > 0 && (
            <div className="mt-4 rounded-none border border-white/10 overflow-hidden">
              <div className="bg-white/3 px-4 py-2 text-[10px] uppercase tracking-widest text-white/40 font-medium border-b border-white/10">
                  ...
                  <div key={i} className="px-4 py-3 flex items-center gap-4 text-xs">
                    {p.thumbnail ? (
                      <img
                        src={p.thumbnail}
                        alt=""
                        className="w-10 h-10 object-contain rounded-none bg-white/5 shrink-0"
                      />
                    ) : (
          ...
          <div className="mt-4 rounded-none bg-emerald-900/20 border border-emerald-500/20 p-4 flex items-center gap-3">
            ...
            <div className="text-sm text-emerald-300">
              ...
              {importResult.errors > 0 && (
                <span className="text-blue-300">, {importResult.errors} помилок</span>
              )}
            </div>
          </div>
        )}
