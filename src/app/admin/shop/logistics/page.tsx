...
export default function LogisticsPage() {
  ...
  const [brandWarehouses, setBrandWarehouses] = useState<
    { id: string; code: string; name: string; nameUa: string }[]
  >([]);
  ...
  async function deleteWarehouse(id: string) {
    const ok = await confirm({
      ...
      description:
        "Склад буде помічено як неактивний. Зони і бренди залишаться, але не використовуватимуться у розрахунку доставки.",
      confirmLabel: "Деактивувати",
    });
    if (!ok) return;
    const response = await fetch(`/api/admin/shop/logistics/warehouses?id=${id}`, {
      method: "DELETE",
    });
    if (!response.ok) {
  ...
  const filteredBrands = configs.filter((c) =>
    c.brandName.toLowerCase().includes(filter.toLowerCase())
  );
      ...
      <div className="w-full px-4 py-8 md:px-8 lg:px-12">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-white/40 text-sm max-w-2xl">
            Управління складами, зонами доставки та логістичними тарифами. Кожен склад має власний
            набір зон з незалежними тарифами.
          </p>
              ...
              {warehouses
                ...
                .map((w) => (
                  <motion.button
                    ...
                    whileTap={{ scale: 0.98 }}
                    className={`
                    relative shrink-0 min-w-[180px] p-4 rounded-none border transition-all duration-300 text-left group sm:min-w-[200px]
                    ${
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex gap-3 mt-3">
                        <span className="text-[10px] text-zinc-400/80">
                          ...
                          {w._count?.zones || 0} зон
                        </span>
                        <span className="text-[10px] text-amber-400/80">
                          <Package className="w-3 h-3 inline mr-1" />
                          {w._count?.brands || 0} брендів
                        </span>
                      </div>
                    </div>
                  </motion.button>
        ...
        {selectedWarehouse && (
          <>
            <div className="flex items-center gap-4 mb-6 border-b border-white/6 pb-4">
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-none text-sm font-medium transition-all ${
                  activeTab === "outbound"
                    ...
                    : "bg-white/3 text-white/40 hover:bg-white/6 hover:text-white/60"
                }`}
              >
            ...
            {loading ? (
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-10 pr-4 py-2.5 bg-white/3 border border-white/8 rounded-none text-sm text-white focus:outline-hidden focus:border-blue-500/50 placeholder-white/20 transition-colors md:min-w-[300px]"
                    />
                  </div>
                  <div className="text-sm text-white/30">
                    Завод <ChevronRight className="w-3 h-3 inline mx-1" />{" "}
                    {selectedWh?.nameUa || "Склад"}
                  </div>
                </div>
                ...
                <div className="overflow-hidden rounded-none border border-white/6 bg-black/60 backdrop-blur-2xl shadow-2xl">
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-white/6 bg-white/2 text-[10px] uppercase tracking-[0.15em] text-white/40">
                        ...
                        <th className="font-medium px-5 py-4" title="Тариф ($/кг)">
                          Тариф ($/кг)
                        </th>
                        <th className="font-medium px-5 py-4" title="Об'ємний Дільник">
                          Дільник
                        </th>
                        <th className="font-medium px-5 py-4" title="Штраф за кожен кг перевищення">
                          Штраф (Vol/кг)
                        </th>
                    ...
                    <tbody className="divide-y divide-white/4 text-sm">
                      {filteredBrands.map((c, i) => {
                        ...
                        return (
                          <tr key={c.brandName} className="hover:bg-white/2 transition-colors">
                            <td className="px-5 py-3 font-medium text-white/90">
                              {c.brandName}
                              {!c.id && (
                                <span className="ml-2 text-[10px] text-yellow-500/70 uppercase">
                                  New
                                </span>
                              )}
                            </td>
                            <td className="px-5 py-3">
                              <select
                                value={c.warehouseId || ""}
                                onChange={(e) =>
                                  updateBrandField(realIdx, "warehouseId", e.target.value || null)
                                }
                                className="bg-black/40 border border-white/10 rounded-none px-2 py-1.5 outline-hidden focus:border-blue-500 text-xs text-white/70"
                              >
                            ...
                            <td className="px-5 py-3">
                              <select
                                value={c.originZone}
                                onChange={(e) =>
                                  updateBrandField(realIdx, "originZone", e.target.value)
                                }
                                className="bg-black/40 border border-white/10 rounded-none px-2 py-1.5 outline-hidden focus:border-blue-500 text-xs text-white/70"
                              >
                            ...
                            <td className="px-5 py-3">
                              <input
                                ...
                                value={c.ratePerKg}
                                onChange={(e) =>
                                  updateBrandField(realIdx, "ratePerKg", parseFloat(e.target.value))
                                }
                                className="w-16 bg-black/40 border border-white/10 rounded-none px-2 py-1.5 outline-hidden text-right text-white/70 focus:border-blue-500"
                              />
                            </td>
                            <td className="px-5 py-3">
                              <input
                            ...
                            <td className="px-5 py-3 text-center">
                              <input
                                ...
                                checked={c.isActive}
                                onChange={(e) =>
                                  updateBrandField(realIdx, "isActive", e.target.checked)
                                }
              ...
              <div className="space-y-6">
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-sm text-white/30">
                    {selectedWh?.nameUa || "Склад"} <ChevronRight className="w-3 h-3 inline mx-1" />{" "}
                    Клієнт
                  </div>
                </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="border-b border-white/6 bg-white/2 text-[10px] uppercase tracking-[0.15em] text-white/40">
                        ...
                        <th
                          className="font-medium px-5 py-4"
                          title="Базовий тариф за 1 фізичний кг"
                        >
                          Факт. вага ($/кг)
                        </th>
                        <th className="font-medium px-5 py-4" title={"Штраф за об'ємну перевагу"}>
                          Штраф об&apos;єм ($/кг)
                        </th>
                        <th
                          className="font-medium px-5 py-4"
                          title="Фіксована комісія за оформлення"
                        >
                          Базова комісія ($)
                        </th>
                        <th className="font-medium px-5 py-4" title="Орієнтовний час доставки">
                          ETA (днів)
                        </th>
                    ...
                    <tbody className="divide-y divide-white/4 text-sm">
                      {zones.map((c, i) => (
                        <tr key={c.id || c.zoneCode} className="hover:bg-white/2 transition-colors">
                          <td className="px-5 py-4">
                            <div className="font-medium text-white/90 flex flex-col">
                              <span>{c.labelUa}</span>
                              <span className="text-[10px] text-white/25 uppercase mt-0.5 font-mono">
                                {c.zoneCode} · {c.label}
                              </span>
                            </div>
                            {!c.id && (
                              <span className="text-[10px] text-yellow-500/70 uppercase">
                                Defaults
                              </span>
                            )}
                          </td>
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                value={c.ratePerKg}
                                onChange={(e) =>
                                  updateZoneField(i, "ratePerKg", parseFloat(e.target.value))
                                }
                                className="w-20 bg-black/40 border border-white/10 rounded-none px-2 py-1.5 outline-hidden focus:border-blue-500 text-white/70"
                              />
                            </div>
                          </td>
                          <td className="px-5 py-3">
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <input
                                ...
                                value={c.baseFee}
                                onChange={(e) =>
                                  updateZoneField(i, "baseFee", parseFloat(e.target.value))
                                }
                                className="w-20 bg-black/40 border border-white/10 rounded-none px-2 py-1.5 outline-hidden focus:border-blue-500 text-white/70"
                              />
                            </div>
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-1">
                              <input
                                ...
                                value={c.etaMinDays}
                                onChange={(e) =>
                                  updateZoneField(i, "etaMinDays", parseInt(e.target.value) || 1)
                                }
                                className="w-12 bg-black/40 border border-white/10 rounded-none px-1.5 py-1.5 outline-hidden text-center focus:border-blue-500 text-white/70 text-xs"
                              />
                              ...
                              <input
                                ...
                                value={c.etaMaxDays}
                                onChange={(e) =>
                                  updateZoneField(i, "etaMaxDays", parseInt(e.target.value) || 1)
                                }
                                className="w-12 bg-black/40 border border-white/10 rounded-none px-1.5 py-1.5 outline-hidden text-center focus:border-blue-500 text-white/70 text-xs"
                              />
                            </div>
                ...
                <div className="p-4 bg-blue-500/4 border border-blue-500/10 rounded-none flex gap-3 text-sm text-red-200/80">
                  <AlertCircle className="shrink-0 w-5 h-5 text-zinc-400/60" />
                  <div>
                    <p className="font-semibold mb-1 text-red-200/90">
                      Як працює формула Вихідної Зони?
                    </p>
                    <p className="text-red-200/50">
                      Якщо клієнт купує деталь (Фактична вага = 10 кг, Об&apos;ємна = 15 кг). <br />
                      І Зона встановлена так:{" "}
                      <code className="text-zinc-500/60">Тариф: $14/кг</code>,{" "}
                      <code className="text-zinc-500/60">Штраф: $2/кг</code>,{" "}
                      <code className="text-zinc-500/60">Комісія: $5</code>.<br />
                      Розрахунок:{" "}
                      <code className="text-zinc-500/60">
                        $5 + (10кг × $14) + ((15кг − 10кг) × $2)
                      </code>
                      .
                    </p>
                  </div>
          ...
          >
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewWarehouse((p) => ({ ...p, code: e.target.value.toUpperCase() }))
                    }
                    placeholder="US_NY"
                    className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-blue-500/40 placeholder-white/15 font-mono"
                  />
                </div>
                <div>
                  <label className="block text-[9px] uppercase tracking-widest text-white/30 mb-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => setNewWarehouse((p) => ({ ...p, country: e.target.value }))}
                    placeholder="US"
                    className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-blue-500/40 placeholder-white/15"
                  />
                </div>
                ...
                <div className="col-span-2">
                  ...
                  <input
                    value={newWarehouse.nameUa}
                    onChange={(e) => setNewWarehouse((p) => ({ ...p, nameUa: e.target.value }))}
                ...
                <div className="col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setNewWarehouse((p) => ({ ...p, city: e.target.value }))}
              ...
              <div className="mt-4 pt-4 border-t border-white/6">
                <p className="text-[9px] uppercase tracking-widest text-white/25 mb-3 font-semibold">
                  Адреса складу (для розрахунку доставки)
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    <label className="block text-[9px] uppercase tracking-widest text-white/30 mb-1">
                      Адреса
                    </label>
                    <input
                      ...
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2 rounded-none focus:outline-hidden focus:border-blue-500/40 placeholder-white/15"
                    />
                  </div>
                  <div className="col-span-2">
                    <label className="block text-[9px] uppercase tracking-widest text-white/30 mb-1">
                    ...
                    <input
                      ...
                      onChange={(e) => setNewWarehouse((p) => ({ ...p, address2: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewWarehouse((p) => ({ ...p, state: e.target.value }))}
                      placeholder="NY"
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2 rounded-none focus:outline-hidden focus:border-blue-500/40 placeholder-white/15"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewWarehouse((p) => ({ ...p, postalCode: e.target.value }))
                      }
                      placeholder="10001"
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2 rounded-none focus:outline-hidden focus:border-blue-500/40 placeholder-white/15"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewWarehouse((p) => ({ ...p, phone: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewWarehouse((p) => ({ ...p, contactName: e.target.value }))
                      }
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  ...
                  disabled={!newWarehouse.code || !newWarehouse.name}
            ...
            >
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewZone((p) => ({ ...p, zoneCode: e.target.value.toUpperCase() }))
                      }
                      placeholder="AE"
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-blue-500/40 placeholder-white/15 font-mono"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setNewZone((p) => ({ ...p, baseFee: Number(e.target.value) }))
                      }
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-blue-500/40"
                    />
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    value={newZone.labelUa}
                    onChange={(e) => setNewZone((p) => ({ ...p, labelUa: e.target.value }))}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      type="number"
                      step="0.1"
                      value={newZone.ratePerKg}
                      onChange={(e) =>
                        setNewZone((p) => ({ ...p, ratePerKg: Number(e.target.value) }))
                      }
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-blue-500/40"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      type="number"
                      step="0.1"
                      value={newZone.volSurchargePerKg}
                      onChange={(e) =>
                        setNewZone((p) => ({ ...p, volSurchargePerKg: Number(e.target.value) }))
                      }
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-blue-500/40"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      min="1"
                      max="90"
                      value={newZone.etaMinDays}
                      onChange={(e) =>
                        setNewZone((p) => ({ ...p, etaMinDays: Number(e.target.value) }))
                      }
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-blue-500/40"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      min="1"
                      max="90"
                      value={newZone.etaMaxDays}
                      onChange={(e) =>
                        setNewZone((p) => ({ ...p, etaMaxDays: Number(e.target.value) }))
                      }
                      className="w-full bg-white/3 border border-white/10 text-sm text-white px-3 py-2.5 rounded-none focus:outline-hidden focus:border-blue-500/40"
                    />
                  </div>
              ...
              <div className="flex gap-3 mt-6">
                ...
                <button
                  onClick={addNewZone}
                  disabled={!newZone.zoneCode || !newZone.label}
                  className="flex-1 text-center py-2.5 text-xs uppercase tracking-widest font-bold bg-blue-500/15 text-zinc-100 rounded-none hover:bg-blue-400 disabled:opacity-30 transition-all"
                >
                  Створити Зону
                </button>
