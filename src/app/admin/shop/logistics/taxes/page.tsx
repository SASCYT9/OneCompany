...
export default function TaxRegionPage() {
  ...
  const [newRule, setNewRule] = useState<TaxRule>({
    regionCode: '', regionName: '', regionNameUa: '', taxType: 'VAT',
    taxRate: 20, taxLabel: null, taxLabelUa: null, customsDutyPct: 0,
  ...
  const avgRate = rules.length > 0 ? (rules.reduce((s, r) => s + r.taxRate, 0) / rules.length).toFixed(1) : '0';
    ...
    <div className="relative h-full w-full overflow-auto bg-black text-white">
      ...
      <div className="w-full px-4 py-8 md:px-8 lg:px-12">
        <Link href="/admin/shop/logistics"
          ...
          <ArrowLeft className="h-4 w-4 transform transition-transform group-hover:-translate-x-1" /> Логістика
        </Link>
        ...
        <div className="flex items-end justify-between mb-8 gap-4 flex-wrap">
          <div>
            ...
            <p className="mt-2 text-sm text-white/40 max-w-xl">
              VAT, мито та інші податки по регіонах. Застосовуються автоматично під час checkout на основі адреси клієнта.
            </p>
          </div>
        ...
        <div className="mb-6 flex items-center gap-3 rounded-none border border-white/[0.06] bg-black/40 px-4 py-3">
          ...
          <span className="text-[10px] text-white/15">{filtered.length} / {rules.length}</span>
        </div>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-white/[0.06] bg-white/[0.02]">
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35">Регіон</th>
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35">Тип</th>
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35 w-28">Ставка %</th>
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35 w-28">Мито %</th>
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35">Мітка</th>
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35 text-center">Inclusive</th>
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35 text-center">Активно</th>
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35">Нотатки</th>
                  <th className="px-5 py-4 font-medium text-[10px] tracking-[0.15em] uppercase text-white/35 w-32">Дії</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/[0.04]">
                {filtered.map((r, i) => {
                  ...
                  return (
                    <tr key={r.id || r.regionCode} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-5 py-4">
                        <div className="font-medium text-white/90">{r.regionNameUa || r.regionName}</div>
                        <div className="text-[10px] text-white/25 font-mono mt-0.5">{r.regionCode}</div>
                      </td>
                      ...
                      <td className="px-5 py-3">
                        <input value={r.taxLabelUa || r.taxLabel || ''} placeholder="ПДВ 20%"
                          onChange={e => updateField(realIdx, 'taxLabelUa', e.target.value || null)}
                      ...
                      <td className="px-5 py-3 text-center">
                        <button onClick={() => updateField(realIdx, 'isInclusive', !r.isInclusive)}
                      ...
                      <td className="px-5 py-3">
                        <input value={r.notes || ''} placeholder="B2B exempt..."
                          onChange={e => updateField(realIdx, 'notes', e.target.value || null)}
                          className="w-full bg-transparent border-b border-white/5 text-xs text-white/40 placeholder-white/10 px-0 py-1 focus:outline-none focus:border-white/20" />
                      </td>
                      <td className="px-5 py-3">
            ...
            >
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[9px] uppercase tracking-widest text-white/30 mb-1.5">Код Регіону *</label>
                ...
                <div className="flex items-center gap-4 pt-1">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={newRule.isInclusive}
