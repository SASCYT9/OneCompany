...
type Credit = {
  ...
  createdAt: string;
  redemptions: Array<{ id: string; amount: number; currency: string; redeemedAt: string; orderId: string | null; note: string | null }>;
};
          ...
          {Object.entries(data.balanceByCurrency).map(([cur, bal]) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold uppercase tracking-wider text-emerald-200">{cur}</span>
              </div>
              <span className="text-base font-bold text-emerald-200 tabular-nums">{formatMoney(bal, cur)}</span>
            </div>
          ))}
        </div>
      ) : (
      ...
      <div className="mt-3">
        {!showIssueForm ? (
          ...
          <div className="space-y-3 rounded-lg border border-blue-500/20 bg-blue-500/[0.03] p-3">
            ...
            />
            <AdminInputField label="Причина (коротко)" value={reason} onChange={setReason} placeholder="напр. компенсація за пошкоджене відправлення" />
            <AdminInputField label="Спливає (опціонально)" value={expiresAt} onChange={setExpiresAt} type="date" />
            <AdminTextareaField label="Внутрішні нотатки" value={notes} onChange={setNotes} rows={2} />
            <div className="flex items-center gap-2">
              <button
                type="button"
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    <AdminStatusBadge tone={statusTone(c.status)}>{c.status.replace('_', ' ')}</AdminStatusBadge>
                  </div>
                  {c.reason ? <div className="mt-0.5 truncate text-[11px] text-zinc-400">{c.reason}</div> : null}
                  <div className="mt-1 text-[10px] text-zinc-600">
                    {c.type.replace('_', ' ').toLowerCase()} · {new Date(c.createdAt).toLocaleDateString()}
                    {c.expiresAt ? ` · спливає ${new Date(c.expiresAt).toLocaleDateString()}` : ''}
                  </div>
                  {c.amountUsed > 0 ? (
                    <div className="mt-1 text-[11px] text-zinc-500">
                      Використано: <span className="text-zinc-300 tabular-nums">{formatMoney(c.amountUsed, c.currency)}</span> · Залишок:{' '}
                      <span className="text-emerald-300 tabular-nums">{formatMoney(c.remainingAmount, c.currency)}</span>
                    </div>
                  ) : null}
                </div>
