...
function createForm(customer: CustomerDetail): CustomerForm {
  return {
    ...
    b2bDiscountPercent:
      customer.b2bDiscountPercent != null ? String(customer.b2bDiscountPercent) : "",
...
export default function AdminShopCustomerDetailPage() {
  ...
  const [customerMarkup, setCustomerMarkup] = useState<{
    markupPct: number;
    notes: string | null;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    setCrmLoading(true);
    fetch(
      `/api/admin/crm?type=orders&maxRecords=50&filter=${encodeURIComponent(`FIND("${airtableId}", ARRAYJOIN({Клиент}))`)}`
    )
  ...
  async function runAction(action: "approve_b2b" | "revert_b2c" | "create_setup_link") {
    ...
    try {
      ...
      if ((data as { setupLinkUrl?: string }).setupLinkUrl) {
        ...
        const msg =
          action === "approve_b2b" ? "Customer approved for B2B" : "Customer reverted to B2C";
        setSuccess(msg + ".");
        toast.success(msg);
      }
    } finally {
    ...
    <AdminPage className="space-y-6">
      <AdminPageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            <AdminStatusBadge tone={customerGroupTone(customer.group)}>
      ...
      <AdminMetricGrid>
        ...
        <AdminMetricCard
          ...
          meta="Shipping + billing records"
        />
        <AdminMetricCard
          label="Carts"
          value={customer.carts.length}
          meta="Open storefront activity"
        />
        <AdminMetricCard
          label="Order value"
          value={totalOrderValue.toFixed(2)}
          meta={customer.currencyPref || "EUR"}
        />
      </AdminMetricGrid>
          ...
          <>
            ...
            <section className="rounded-none border border-white/10 bg-[#171717] p-6">
              <div className="mb-5">
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  Редагування account profile, B2B terms, балансів і локалі.
                </p>
              </div>
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <AdminInputField
                  ...
                  onChange={(value) => updateForm("vatNumber", value)}
                />
                <AdminInputField
                  label="Discount %"
                  value={form.b2bDiscountPercent}
                  onChange={(value) => updateForm("b2bDiscountPercent", value)}
                  type="number"
                />
                <AdminInputField
                  label="Discount tier"
                  value={form.discountTier}
                  onChange={(value) => updateForm("discountTier", value)}
                />
                <AdminInputField
                  label="Balance"
                  value={form.balance}
            ...
            <section className="rounded-none border border-white/10 bg-[#171717] p-6">
              <div className="mb-5 flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-500">
                    Storefront orders and the fastest route to open the full order detail.
                  </p>
                </div>
                <Link
            ...
            <section className="rounded-none border border-white/10 bg-[#171717] p-6">
              <div className="mb-5">
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  Stored shipping data and current storefront activity.
                </p>
              </div>
                ...
                <div className="space-y-3">
                  ...
                  {customer.addresses.length ? (
                    customer.addresses.map((address) => (
                      <div
                        key={address.id}
                        className="rounded-none border border-white/10 bg-black/25 px-4 py-4 text-sm text-zinc-300"
                      >
            ...
            <section className="rounded-none border border-white/10 bg-[#171717] p-6">
              <div className="mb-5 flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-zinc-500">
                    Airtable-linked orders and customer-specific markup signals.
                  </p>
                </div>
              </div>
                  ...
                  {customerMarkup ? (
                    <div className="mt-3 space-y-2">
                      <div className="text-2xl font-semibold text-emerald-300">
                        {customerMarkup.markupPct}%
                      </div>
                      <div className="text-sm text-zinc-400">
                        Custom pricing override
                        {customerMarkup.notes ? ` · ${customerMarkup.notes}` : ""}
                      </div>
                    </div>
                  ) : (
            ...
            >
              <AdminKeyValueGrid
                rows={[
                  ...
                  {
                    label: "Last login",
                    value: customer.account?.lastLoginAt
                      ? new Date(customer.account.lastLoginAt).toLocaleString()
                      : "Never",
                  },
                  {
                    label: "Email verified",
                    value: customer.account?.emailVerifiedAt
                      ? new Date(customer.account.emailVerifiedAt).toLocaleString()
                      : "No",
                  },
