...
const FIELDS = [
  ...
  { value: 'lastOrderDays', label: 'Days since last order', operators: ['gte', 'lte'], type: 'number' },
] as const;
...
type EditorProps = {
  ...
  onSave: (payload: { name: string; description: string | null; rulesJson: SegmentRules }) => Promise<void>;
};

export function SegmentEditor({ initialName = '', initialDescription = '', initialRules, onSave }: EditorProps) {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
      <div className="space-y-5">
        <AdminInspectorCard title="Identity" description="Name and optional description for this segment.">
          <div className="space-y-3">
            <AdminInputField label="Segment name" value={name} onChange={setName} placeholder="e.g. B2B churned 90d" />
          ...
          <div className="mb-3 flex items-center gap-2 text-sm">
            ...
            <div className="inline-flex overflow-hidden rounded-full border border-white/10 bg-white/[0.03]">
              <button
                ...
                className={`px-3 py-1 text-xs ${match === 'all' ? 'bg-blue-500/[0.15] text-blue-300' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                ALL conditions
              </button>
              <button
                ...
                className={`px-3 py-1 text-xs ${match === 'any' ? 'bg-blue-500/[0.15] text-blue-300' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                ANY condition
              </button>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <AdminSelectField
                    ...
                    options={operators.map((op) => ({ value: op, label: OPERATOR_LABEL[op] || op }))}
                  />
                  <ConditionValueInput
