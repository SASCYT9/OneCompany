...
import {
  SegmentEditor,
  type SegmentRules,
} from "@/app/admin/shop/customers/segments/components/SegmentEditor";
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data = (await response.json().catch(() => ({}))) as SegmentDetail & {
          error?: string;
        };
  ...
  return (
    <AdminPage className="space-y-6">
      ...
      <AdminPageHeader
        ...
        actions={
          <span className="inline-flex items-center gap-1.5 rounded-full border border-blue-500/25 bg-blue-500/8 px-3 py-1.5 text-xs font-bold uppercase tracking-wider text-blue-300">
          ...
          <div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            {segment.memberSample.map((c) => (
              <Link
                ...
                className="rounded-none border border-white/5 bg-[#171717] p-2.5 text-sm transition hover:border-blue-500/30 hover:bg-blue-500/4"
              >
                ...
                <div className="mt-1 flex items-center gap-1.5 text-[10px] text-zinc-600">
                  <span className="rounded-full border border-white/8 bg-white/3 px-1.5 py-0 uppercase tracking-wider">
