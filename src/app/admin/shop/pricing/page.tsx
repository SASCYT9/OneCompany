...
export default function CustomerPricingPage() {
  ...
  const [editedMarkups, setEditedMarkups] = useState<
    Record<string, { markupPct: number; notes: string }>
  >({});
  ...
  const filtered = useMemo(() => {
    ...
    return markups.filter(
      (m) => m.customerName.toLowerCase().includes(q) || m.notes?.toLowerCase().includes(q)
    );
  }, [markups, search]);
    ...
    setEditedMarkups((prev) => ({
      ...
      [customerId]: {
        markupPct:
          field === "markupPct"
            ? Number(value)
            : (prev[customerId]?.markupPct ??
              markups.find((m) => m.customerId === customerId)?.markupPct ??
              25),
        notes:
          field === "notes"
            ? String(value)
            : (prev[customerId]?.notes ??
              markups.find((m) => m.customerId === customerId)?.notes ??
              ""),
      },
    }));
  }
  ...
  async function handleDelete(customerId: string) {
    ...
    const response = await fetch(
      `/api/admin/shop/pricing/customer-markups?customerId=${customerId}`,
      { method: "DELETE" }
    );
    if (!response.ok) {
  ...
  return (
    <AdminPage className="space-y-6">
      ...
      <AdminMetricGrid className="xl:grid-cols-3">
        <AdminMetricCard
          label="Customers with markup"
          value={markups.length}
          meta="CRM accounts with custom rule"
          tone="accent"
        />
        <AdminMetricCard
        ...
        <AdminMetricCard
          label="CRM customers"
          value={airtableCustomers.length}
          meta="Total CRM customers available for assignment"
        />
      </AdminMetricGrid>
      ...
      <AdminActionBar className="bg-[#171717]">
        <div className="space-y-1">
          ...
          <div className="text-sm text-zinc-300">
            {hasChanges
              ? `${Object.keys(editedMarkups).length} pending customer pricing changes`
              : "All pricing rules are in sync"}
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-white/6 bg-white/2">
                <th className="px-5 py-4 font-medium text-[11px] uppercase tracking-[0.18em] text-zinc-500">
                  Клієнт
                </th>
                <th className="px-5 py-4 font-medium text-[11px] uppercase tracking-[0.18em] text-zinc-500 w-40">
                  Націнка %
                </th>
                <th className="px-5 py-4 font-medium text-[11px] uppercase tracking-[0.18em] text-zinc-500">
                  Множник
                ...
                <th className="px-5 py-4 font-medium text-[11px] uppercase tracking-[0.18em] text-zinc-500 w-24">
                  Дії
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/4">
              {filtered.map((m) => {
                ...
                return (
                  <tr
                    key={m.id}
                    className={`transition-colors hover:bg-white/2 ${isEdited ? "bg-emerald-500/3" : ""}`}
                  >
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          value={pct}
                          onChange={(e) => handleEdit(m.customerId, "markupPct", e.target.value)}
                          className="w-16 rounded-none border border-white/10 bg-white/3 px-2 py-1.5 text-center text-sm text-zinc-100 focus:border-amber-200/30 focus:outline-hidden"
                        />
                    ...
                    <td className="px-5 py-4">
                      <input
                        ...
                        className="w-full border-b border-white/5 bg-transparent px-0 py-1 text-sm text-zinc-300 placeholder:text-zinc-600 focus:border-white/20 focus:outline-hidden"
                      />
                    </td>
                    <td className="px-5 py-4">
                      <button
      ...
      {showAddModal && (
        ...
        >
          <div
            className="w-full max-w-md rounded-none border border-white/10 bg-[#111] p-6"
          ...
          >
            ...
            <div className="space-y-4">
              <div>
                <label className="mb-1.5 block text-[11px] font-medium uppercase tracking-[0.18em] text-zinc-400">
                  CRM Клієнт *
                </label>
                <select
                  ...
                  className="w-full rounded-none border border-white/10 bg-[#171717] px-3.5 py-2.5 text-sm text-zinc-100 focus:border-amber-200/30 focus:outline-hidden"
                >
              ...
              <div>
                ...
                <input
                  ...
                  min="0"
                  max="200"
                  value={newCustomer.markupPct}
                  onChange={(e) =>
                    setNewCustomer((prev) => ({ ...prev, markupPct: Number(e.target.value) }))
                  }
                  className="w-full rounded-none border border-white/10 bg-[#171717] px-3.5 py-2.5 text-sm text-zinc-100 focus:border-amber-200/30 focus:outline-hidden"
                />
              </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                disabled={!newCustomer.customerId || saving}
