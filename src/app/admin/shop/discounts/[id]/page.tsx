  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const data = (await response.json().catch(() => ({}))) as DiscountDetail & {
          error?: string;
        };
  ...
  return (
    <AdminPage className="space-y-6">
      ...
      <AdminPageHeader
        ...
        title={discount.code}
        description={
          discount.description || "Discount code editor — change conditions, status, and validity."
        }
        actions={
          <>
            <AdminStatusBadge
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-5">
                ...
                label={
                  discount.type === "PERCENTAGE"
                    ...
                    : discount.type === "FIXED_AMOUNT"
                      ? `Amount off (${discount.currency ?? "EUR"})`
                      : "Value"
                }
                value={value}
                onChange={setValue}
          ...
          <AdminInspectorCard
            title="Conditions"
            description="Limits on who and when this code can be used."
          >
            <div className="grid gap-4 md:grid-cols-2">
              <AdminInputField
          ...
          {discount.redemptions.length > 0 ? (
            <AdminInspectorCard
              title={`Redemptions · ${discount.redemptionCount}`}
              description="Recent uses of this code."
            >
              <div className="space-y-1.5">
                {discount.redemptions.map((r) => (
                  <div
                    key={r.id}
                    className="flex items-center justify-between gap-3 rounded-none border border-white/5 bg-black/25 px-3 py-2 text-xs"
                  >
          ...
          <AdminInspectorCard title="Stats">
            <AdminKeyValueGrid
              rows={[
                ...
                {
                  label: "Usage",
                  value: `${discount.usageCount}${discount.usageLimit ? ` / ${discount.usageLimit}` : ""}`,
                },
