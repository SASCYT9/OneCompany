  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const response = await fetch(`/api/admin/shop/discounts?${params.toString()}`, {
          cache: "no-store",
        });
    ...
    <AdminPage className="space-y-6">
      ...
      <AdminMetricGrid>
        <AdminMetricCard
          label="Всього кодів"
          value={stats.total}
          meta="Усі кампанії, включно з чернетками"
        />
        <AdminMetricCard
          label="Активні"
          value={stats.active}
          meta="Доступні для використання"
          tone="accent"
        />
                      ...
                      <tr key={d.id} className="align-top transition hover:bg-white/3">
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-mono text-sm font-bold tracking-wide text-zinc-100">
                              {d.code}
                            </span>
                            <button
                              type="button"
                              onClick={() => void copyCode(d.code)}
                              className="rounded-none p-1 text-zinc-600 transition hover:bg-white/6 hover:text-zinc-300"
                        ...
                        <td className="px-4 py-4">
                          <div className="text-zinc-200">
                            {typeLabel(d.type, d.value, d.currency)}
                          </div>
                        ...
                        <td className="px-4 py-4">
                          <AdminStatusBadge tone={statusTone(d.status)}>
                            {d.status}
                          </AdminStatusBadge>
                        </td>
                        <td className="px-4 py-4 text-xs text-zinc-400">
                          {d.minOrderValue != null ? (
                            <div>Мін. замовлення: {d.minOrderValue}</div>
                          ) : null}
                          {d.usageLimitPerUser != null ? (
                            <div>На клієнта: {d.usageLimitPerUser}</div>
                          ) : null}
                          {!d.minOrderValue && !d.usageLimitPerUser ? (
                            <span className="text-zinc-600">Без обмежень</span>
                          ) : null}
                        </td>
                        <td className="px-4 py-4">
                          ...
                          {d.usageLimit != null ? (
                            <div className="mt-1 h-1.5 w-20 overflow-hidden rounded-full bg-white/4">
                              <div
                        ...
                        <td className="px-4 py-4 text-xs text-zinc-400">
                          ...
                          {d.validUntil ? (
                            <div
                              className={new Date(d.validUntil) < new Date() ? "text-red-400" : ""}
                            >
                              До {new Date(d.validUntil).toLocaleDateString()}
                            </div>
                          ) : null}
