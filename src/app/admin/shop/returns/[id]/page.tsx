...
type ReturnReason = 'WRONG_ITEM' | 'DAMAGED' | 'DEFECTIVE' | 'NOT_AS_DESCRIBED' | 'CHANGED_MIND' | 'ORDERING_ERROR' | 'OTHER';
  ...
  async function handleStatusTransition(nextStatus: ReturnStatus) {
    ...
    try {
      const response = await fetch(`/api/admin/shop/returns/${ret.id}`, {
        ...
        body: JSON.stringify({
          status: nextStatus,
          ...(nextStatus === 'REFUNDED' ? { externalRefundId, refundMethod, refundAmount: parseFloat(refundAmount) } : {}),
        }),
      });
  ...
  return (
    <AdminPage className="space-y-6">
      <div className="flex items-center gap-2 text-xs text-zinc-500">
        <Link href="/admin/shop/returns" className="inline-flex items-center gap-1 transition hover:text-zinc-300">
      ...
      <AdminPageHeader
        ...
        actions={
          <>
            <AdminStatusBadge tone={statusTone(ret.status)}>{STATUS_LABEL[ret.status]}</AdminStatusBadge>
            {isB2B ? (
            ...
            ret.allowedNextStatuses.map((next) => (
              <button
                ...
                className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm transition disabled:opacity-50 ${
                  next === 'REFUNDED'
                    ? 'border-emerald-500/30 bg-emerald-500/[0.08] text-emerald-300 hover:bg-emerald-500/[0.15]'
                    : next === 'REJECTED'
                      ? 'border-red-500/30 bg-red-500/[0.08] text-red-300 hover:bg-red-500/[0.15]'
                      : 'border-blue-500/25 bg-blue-500/[0.08] text-blue-300 hover:bg-blue-500/[0.15]'
                }`}
              >
                {next === 'REFUNDED' ? <Check className="h-4 w-4" /> : next === 'REJECTED' ? <X className="h-4 w-4" /> : <ArrowRight className="h-4 w-4" />}
                {STATUS_LABEL[next]}
              </button>
            ))
          )}
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-5">
              ...
              <table className="w-full min-w-[640px] text-left text-sm">
                  ...
                  {ret.items.map((it) => (
                    <tr key={it.id} className="hover:bg-white/[0.03]">
                      <td className="px-4 py-3">
                        ...
                        {it.conditionNote ? (
                          <div className="mt-1 text-xs italic text-zinc-500">{it.conditionNote}</div>
                        ) : null}
                      </td>
                      <td className="px-4 py-3 text-zinc-300 tabular-nums">{it.quantity}</td>
                      <td className="px-4 py-3 text-zinc-300 tabular-nums">{formatMoney(it.unitPrice, ret.currency)}</td>
                      <td className="px-4 py-3 font-medium text-zinc-100 tabular-nums">{formatMoney(it.refundAmount, ret.currency)}</td>
                      <td className="px-4 py-3">
                        {it.restockedAt ? (
                ...
                <tfoot>
                  <tr className="border-t border-white/10 bg-white/[0.03]">
                    <td colSpan={3} className="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-zinc-500">
                      Total refund
                    </td>
          ...
          {/* Refund settings */}
          <AdminInspectorCard title="Refund settings" description="Method, amount, and external refund tracking.">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              />
              <AdminInputField label={`Refund amount (${ret.currency})`} value={refundAmount} onChange={setRefundAmount} type="number" step="0.01" min={0} />
              <AdminInputField
              ...
              />
              <AdminCheckboxField label="Restock on RECEIVED" checked={restockOnReceive} onChange={setRestockOnReceive} />
            </div>
            <div className="mt-4">
              <AdminTextareaField label="Internal admin note" value={adminNote} onChange={setAdminNote} rows={3} />
            </div>
            <div className="mt-4">
              <button
                type="button"
          ...
          <AdminInspectorCard title="Timeline">
            <AdminKeyValueGrid
              rows={[
                ...
                { label: 'Approved', value: ret.approvedAt ? new Date(ret.approvedAt).toLocaleString() : '—' },
                { label: 'Received', value: ret.receivedAt ? new Date(ret.receivedAt).toLocaleString() : '—' },
                { label: 'Refunded', value: ret.refundedAt ? new Date(ret.refundedAt).toLocaleString() : '—' },
              ]}
            />
          </AdminInspectorCard>
