...
function NewReturnPageContent() {
  ...
  const [refundMethod, setRefundMethod] = useState<
    "NONE" | "STRIPE_REFUND" | "STORE_CREDIT" | "BANK_TRANSFER" | "REPLACEMENT"
  >("NONE");
      ...
      <AdminPageHeader
        ...
        actions={
          isB2B ? (
            ...
            <span className="rounded-full border border-emerald-500/25 bg-emerald-500/8 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-emerald-300">
              B2C → Stripe refund
            </span>
          )
        }
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-5">
          ...
          <AdminTableShell>
            <div className="border-b border-white/10 px-5 py-4">
              ...
              <p className="mt-1 text-xs text-zinc-500">
                Check items, set quantity, and adjust refund amount per line.
              </p>
            </div>
                ...
                <tbody className="divide-y divide-white/6">
                  {order.items.map((it) => {
                    ...
                    return (
                      <tr
                        key={it.id}
                        className={`align-top transition hover:bg-white/3 ${draft.selected ? "bg-blue-500/4" : ""}`}
                      >
                        ...
                        <td className="px-4 py-3 text-xs text-zinc-400">
                          <div>
                            {it.quantity} × {formatMoney(it.price, order.currency)}
                          </div>
                          <div className="text-zinc-600">
                            = {formatMoney(it.total, order.currency)}
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <input
          ...
          <AdminInspectorCard
            ...
            description="Choose the primary reason. Detailed notes go below."
          >
            ...
            <div className="mt-4">
              <AdminCheckboxField
                label="Restock items on RECEIVED"
        ...
        <aside className="space-y-4">
          <AdminInspectorCard
            title="Submit RMA"
            description="Creates the return in REQUESTED state. You can move it through the workflow next."
          >
            <div className="space-y-3">
              ...
              <div className="rounded-none border border-emerald-500/20 bg-emerald-500/4 p-3">
                <div className="text-[10px] uppercase tracking-wider text-zinc-500">
                  Refund total
                </div>
