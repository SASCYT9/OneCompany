...
export default function AdminCreateOrderPage() {
  ...
  useEffect(() => {
    ...
    else if (region === "UA" || region.includes("УКРАЇН") || region.includes("UKRAIN"))
      setZone("UA");
    else if (region === "US" || region.includes("USA") || region.includes("AMERICA")) setZone("US");
    else if (
      ["DE", "FR", "IT", "ES", "PL", "CZ", "NL", "BE", "AT", "CH"].includes(region) ||
      region.includes("EUROP")
    )
      setZone("EU");
  }, [selectedCustomer]);
  ...
  async function submitOrder() {
    if (!selectedCustomer) {
      setSubmitResult({ success: false, message: "Оберіть клієнта" });
      return;
    }
    if (items.length === 0 || items.every((i) => !i.title)) {
      setSubmitResult({ success: false, message: "Додайте хоча б одну позицію" });
      return;
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSubmitResult({
          success: true,
          message: `Замовлення ${data.orderNumber} створено! Перенаправлення...`,
  ...
  const sections = [
    ...
    { id: "shipping", label: "Shipping", description: "Розрахунок логістики по зоні та override." },
    {
      id: "summary",
      label: "Totals & submit",
      description: "Підсумок, нотатки та створення замовлення.",
    },
  ];
        ...
        <div className="space-y-4">
          {items.some((item) => item.sourceType === "manual" && item.title) ? (
            <AdminInlineAlert tone="warning">
              Manual items are not linked to catalog products. Keep SKU, brand, price and dimensions
              filled before submit.
            </AdminInlineAlert>
          ) : null}
              ...
              rows={[
                {
                  ...
                  value: selectedCustomer
                    ? `${selectedCustomer.fullName} · ${selectedCustomer.group}`
                    : "Не обрано",
                },
                {
                  label: "Позицій",
                  value: String(items.filter((item) => item.title).length || items.length),
                },
                {
                  label: "Manual",
                  value: String(
                    items.filter((item) => item.sourceType === "manual" && item.title).length
                  ),
                },
          ...
          <AdminInspectorCard
            title="Статус submit"
            description="Фінальний крок створює order draft через існуючий API flow."
          >
            <div className="space-y-3">
              ...
              <button
                ...
                onClick={submitOrder}
                disabled={submitting}
                className="flex w-full items-center justify-center gap-2 rounded-full bg-linear-to-b from-blue-500 to-blue-700 px-4 py-3 text-sm font-bold uppercase tracking-wider text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.18),0_2px_8px_rgba(59,130,246,0.4)] transition hover:from-blue-400 hover:to-blue-600 disabled:opacity-50"
              >
    ...
    >
        ...
        <div className="space-y-1">
          ...
          <div className="text-sm text-zinc-300">
            {selectedCustomer
              ...
              : "Оберіть клієнта для order draft"}
          </div>
        </div>
          ...
          <div className="flex items-center justify-between rounded-none border border-emerald-500/20 bg-emerald-500/5 p-4">
            <div>
              ...
              <div className="mt-1 text-xs text-white/45">
                {selectedCustomer.email} · {selectedCustomer.group}
              </div>
              {selectedCustomer.b2bDiscountPercent ? (
                <div className="mt-1 text-xs text-emerald-400">
                  Знижка: {selectedCustomer.b2bDiscountPercent}%
                </div>
              ) : null}
            </div>
            <button
                ...
                {filteredCustomers.map((c) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-white/40">
                      {c.email} · {c.group}
                      {c.b2bDiscountPercent ? ` · −${c.b2bDiscountPercent}%` : ""}
                    </div>
                  </button>
                ))}
              </div>
          ...
          <div key={item.key} className="mb-4 rounded-none border border-white/10 bg-black/20 p-4">
            ...
            <div className="mb-4 space-y-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="inline-flex overflow-hidden rounded-full border border-white/10 bg-white/3">
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-3 py-2 text-xs font-medium transition ${item.entryMode === "search" ? "bg-zinc-100 text-black" : "text-zinc-300 hover:bg-white/6"}`}
                  >
                  ...
                  <button
                    ...
                    className={`inline-flex items-center gap-2 px-3 py-2 text-xs font-medium transition ${item.entryMode === "manual" ? "bg-zinc-100 text-black" : "text-zinc-300 hover:bg-white/6"}`}
                  >
              ...
              {item.entryMode === "search" ? (
                <div className="relative" ref={addingToItemIdx === idx ? dropdownRef : undefined}>
                  <label className="block">
                    ...
                    <input
                      ...
                      className="h-11 w-full rounded-none border border-blue-500/25 bg-zinc-950 px-3 text-sm text-white placeholder:text-white/25 focus:border-blue-500/40 focus:outline-hidden"
                    />
                  </label>
                      ...
                      {!turn14Loading && turn14Results.length === 0 ? (
                        <div className="rounded-none border border-dashed border-white/10 bg-black/30 p-3">
                          ...
                          <div className="mt-1 text-xs leading-5 text-zinc-500">
                            Keep this text and create a manual line item. You can still fill SKU,
                            brand, price and dimensions.
                          </div>
                          <button
                            type="button"
                            onClick={() => createManualItemFromQuery(idx)}
                            className="mt-3 inline-flex items-center gap-2 rounded-full border border-blue-500/25 bg-blue-500/8 px-3 py-2 text-xs font-medium uppercase tracking-[0.14em] text-blue-300 transition hover:bg-blue-500/12"
                          >
                            <Edit3 className="h-3.5 w-3.5" />
                            Create manual item
                          </button>
                ...
                <div className="rounded-none border border-blue-500/25 bg-blue-500/6 p-3">
            ...
            {item.title && (
              <>
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 grid gap-3 md:grid-cols-2">
                    <Field
                      ...
                      onChange={(v) => updateItem(idx, { partNumber: v })}
                    />
                    <Field
                      label="Бренд"
                      value={item.brand}
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-4">
                  <NumField
                    label="Закупка ($)"
                    value={item.baseCostUsd}
                    onChange={(v) => updateItem(idx, { baseCostUsd: v })}
                  />
                  <NumField
                    label="Націнка (%)"
                    value={item.markupPct}
                    onChange={(v) => updateItem(idx, { markupPct: v })}
                  />
                  <NumField
                    label="Знижка (%)"
                  ...
                  <NumField
                    label="Кількість"
                    value={item.quantity}
                    onChange={(v) => updateItem(idx, { quantity: Math.max(1, Math.round(v)) })}
                    step={1}
                  />
                </div>
                ...
                <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-3 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    {item.title && !item.isAILoading && (
                      <button
                        type="button"
                        onClick={() =>
                          triggerAIEstimation(idx, item.title, item.brand, item.partNumber)
                        }
                  ...
                  {item.isAILoading && (
                    <span className="flex items-center gap-1 text-[10px] text-amber-400 shadow-amber-400 drop-border border-white/5 motion-safe:animate-pulse font-mono tracking-wider">
                      <RefreshCw className="h-3 w-3 motion-safe:animate-spin" /> ШІ ФОРМУЄ
                      ГАБАРИТИ...
                    </span>
                  )}
                </div>
                ...
                <div className="relative grid gap-3 md:grid-cols-4">
                  ...
                  <NumField
                    ...
                    onChange={(v) => updateItem(idx, { weightLbs: v })}
                  />
                  <div
                    className={`rounded-none border px-3 py-2 transition-colors ${item.isAILoading ? "border-amber-500/20" : "border-white/10 bg-zinc-950"}`}
                  >
                    <div className="text-[10px] text-white/40 uppercase tracking-wider mb-1">
                      Вага (KG)
                    </div>
                    <div
                      className={`text-sm ${item.isAILoading ? "text-amber-400" : "text-white"}`}
                    >
                ...
                <div className="mt-3 flex items-center justify-between rounded-none border border-white/10 bg-white/3 px-4 py-2">
      ...
      >
        ...
        <div className="space-y-1.5 rounded-none border border-white/10 bg-black/20 p-3 text-xs">
          ...
          <Row
            label={`Базова: ${totalWeightKg.toFixed(1)}кг × $${ratePerKg}`}
            value={fmtUsd(autoShippingBase)}
          />
          {volSurchargeKg > 0 && (
            <Row
              label={`Об'ємна: ${volSurchargeKg.toFixed(1)}кг × $${volSurchargePerKg}`}
              value={fmtUsd(autoShippingVol)}
            />
          )}
      ...
      >
        ...
        <button
          ...
          onClick={submitOrder}
          disabled={submitting}
          className="mt-4 flex w-full items-center justify-center gap-2 rounded-none bg-linear-to-b from-blue-500 to-blue-700 px-4 py-3 text-sm font-bold uppercase tracking-wider text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.18),0_2px_8px_rgba(59,130,246,0.4)] hover:from-blue-400 hover:to-blue-600 disabled:opacity-50"
        >
          <Save className="h-4 w-4" />
          {submitting ? "Створення..." : "Створити замовлення"}
        </button>
...
function SearchItemResult({ item, onSelect }: { item: any; onSelect: () => void }) {
  ...
  const partNumber = compactText(
    item.internal_part_number || attrs.internal_part_number || item.part_number || attrs.part_number
  );
  ...
  return (
    <button
      type="button"
      onClick={onSelect}
      className="mb-1 grid w-full grid-cols-[52px_minmax(0,1fr)_112px] items-center gap-3 rounded-none border border-white/8 bg-white/3 p-2.5 text-left transition last:mb-0 hover:border-blue-500/25 hover:bg-white/6"
    >
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="h-10 w-full min-w-0 rounded-none border border-white/10 bg-zinc-950 px-3 text-sm text-white placeholder:text-white/20 focus:border-white/30 focus:outline-hidden"
      />
    </label>
  );
}
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between">
      <span className={`text-white/${bold ? "70" : "40"} ${bold ? "font-medium" : ""}`}>
        {label}
      </span>
      <span
        className={`font-mono ${bold ? "text-white text-sm font-medium" : "text-white/70"} ${highlight ? "text-amber-400" : ""}`}
      >
        {value}
      </span>
    </div>
