...
export default function CrmCustomerDetailPage() {
  ...
  const balanceTone =
    customer.balance > 0 ? "success" : customer.balance < 0 ? "danger" : "default";
    ...
    <AdminPage className="space-y-6">
      <div className="space-y-4">
        ...
        <AdminPageHeader
          ...
          description={
            ...
            "Customer profitability, orders, balance state, and purchased products."
          }
          actions={
            customer.markup ? (
              <Link
      ...
      <AdminMetricGrid className="xl:grid-cols-5">
        <AdminMetricCard
          ...
          meta="Customer sales volume"
          tone="accent"
        />
        <AdminMetricCard
          label="Profit"
          value={`$${customer.totalProfit.toLocaleString()}`}
          meta="Profit generated by this customer"
        />
        <AdminMetricCard
          label="Payments"
          value={`$${customer.totalPayments.toLocaleString()}`}
          meta="Incoming customer payments"
        />
        <AdminMetricCard
          label="Balance"
          value={`$${customer.balance.toLocaleString()}`}
          meta="Current balance state"
        />
        <AdminMetricCard
          label="Orders"
          value={customer.orders.length}
          meta="CRM orders linked to this customer"
        />
      </AdminMetricGrid>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <AdminTableShell>
          <div className="border-b border-white/10 px-5 py-4">
            ...
            <p className="mt-1 text-xs text-zinc-500">
              Order history and profitability linked to this customer.
            </p>
          </div>
                ...
                <tbody className="divide-y divide-white/4">
                  {customer.orders.map((order) => (
                    <tr key={order.id} className="hover:bg-white/2">
                      ...
                      <td className="px-5 py-4">
                        <AdminStatusBadge
                          tone={order.paymentStatus === "Оплачено" ? "success" : "warning"}
                        >
                          {order.paymentStatus}
                        </AdminStatusBadge>
                      </td>
        ...
        <AdminTableShell>
          <div className="border-b border-white/10 px-5 py-4">
            ...
            <p className="mt-1 text-xs text-zinc-500">
              Highest-value purchased products for this customer.
            </p>
          </div>
              ...
              {customer.topProducts.map((product) => (
                <div key={product.name} className="px-5 py-4">
                  ...
                  <div className="mt-2 grid gap-2 text-xs text-zinc-400 sm:grid-cols-3">
                    ...
                    <div
                      className={product.totalProfit >= 0 ? "text-emerald-300" : "text-blue-300"}
                    >
