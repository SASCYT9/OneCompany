  ...
  return (
    <AdminPage className="space-y-6">
      ...
      <AdminMetricGrid className="xl:grid-cols-5">
        <AdminMetricCard
          label="Client total"
          value={`$${order.clientTotal.toLocaleString()}`}
          meta="Amount billed to customer"
          tone="accent"
        />
        <AdminMetricCard
          label="Purchase cost"
          value={`$${order.purchaseCost.toLocaleString()}`}
          meta="Supplier-side item purchase cost"
        />
        <AdminMetricCard
          label="Additional costs"
          value={`$${order.additionalCosts.toLocaleString()}`}
          meta="Non-item order costs"
        />
        <AdminMetricCard
          label="Profit"
          value={`$${order.profit.toLocaleString()}`}
          meta={`Margin ${(order.marginality * 100).toFixed(1)}%`}
        />
        <AdminMetricCard
          label="Items"
      ...
      <AdminTableShell>
        <div className="border-b border-white/10 px-5 py-4">
          ...
          <p className="mt-1 text-xs text-zinc-500">
            Order composition with customer pricing, purchase cost, and profit per item.
          </p>
        </div>
        {order.items.length ? (
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-white/4">
                {order.items.map((item) => (
                  <tr key={item.id} className="hover:bg-white/2">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        {item.imageUrl ? (
                          ...
                          <div className="flex h-11 w-11 items-center justify-center rounded-none border border-white/10 bg-white/3">
                    ...
                    <td
                      className={`px-5 py-4 text-right font-medium ${item.profitPerItem >= 0 ? "text-emerald-300" : "text-blue-300"}`}
                    >
              ...
              <tfoot>
                <tr className="border-t border-white/6 bg-white/2">
                  <td
                    colSpan={3}
                    className="px-5 py-4 text-right text-[11px] font-medium uppercase tracking-[0.18em] text-zinc-500"
                  >
                    Totals
                  </td>
                  <td className="px-5 py-4 text-right font-semibold text-zinc-100">
                    ${order.clientTotal.toLocaleString()}
                  </td>
                  <td className="px-5 py-4 text-right font-semibold text-amber-200">
                    ${order.purchaseCost.toLocaleString()}
                  </td>
                  <td
                    className={`px-5 py-4 text-right font-semibold ${profitTone === "success" ? "text-emerald-300" : "text-blue-300"}`}
                  >
