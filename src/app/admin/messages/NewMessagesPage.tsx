    ...
    <div className="h-full bg-black text-white flex flex-col font-ua overflow-hidden">
      ...
      <div className="flex-none bg-zinc-900/30 border-b border-white/5 overflow-x-auto">
        <div className="flex p-2 gap-2 min-w-max">
          <div className="px-4 py-2 bg-black rounded-none border border-white/10 min-w-[100px]">
            <div className="text-zinc-500 text-[10px] uppercase tracking-widest font-medium mb-1">
              Total
            </div>
          ...
          <div className="px-4 py-2 bg-black rounded-none border border-white/10 min-w-[100px]">
            ...
            <div className="text-xl font-light tracking-tight text-zinc-400">{stats.new}</div>
          </div>
          <div className="px-4 py-2 bg-black rounded-none border border-white/10 min-w-[100px]">
            <div className="text-zinc-500 text-[10px] uppercase tracking-widest font-medium mb-1">
              Read
            </div>
            <div className="text-xl font-light tracking-tight text-zinc-400">{stats.read}</div>
          </div>
          <div className="px-4 py-2 bg-black rounded-none border border-white/10 min-w-[100px]">
            <div className="text-zinc-500 text-[10px] uppercase tracking-widest font-medium mb-1">
              Replied
      ...
      <div className="flex-1 flex overflow-hidden relative bg-black">
        {/* List View */}
        <div
          className={`w-full lg:w-[400px] flex flex-col border-r border-white/10 bg-black ${selectedMessage ? "hidden lg:flex" : "flex"}`}
        >
          {/* Search & Filter */}
          <div className="p-3 border-b border-white/10 flex-none space-y-3 bg-black/50 backdrop-blur-xs">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-zinc-900/50 border border-white/10 rounded-none focus:outline-hidden focus:border-white/20 text-sm placeholder:text-zinc-600 transition-colors"
              />
            </div>
                ...
                {filteredMessages.map((msg) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <span
                        className={`text-base truncate pr-2 ${msg.status === "NEW" ? "font-bold text-white" : "font-medium text-zinc-300"}`}
                      >
                        {msg.userName}
                      </span>
                    ...
                    <p
                      className={`text-sm line-clamp-2 mb-3 ${msg.status === "NEW" ? "text-zinc-300" : "text-zinc-500"}`}
                    >
                      {msg.messageText}
                    </p>
                    <div className="flex items-center gap-2">
                      <span
                        className={`px-2.5 py-1 rounded-none text-xs font-medium uppercase tracking-wider ${
                          msg.status === "NEW"
                            ? "bg-zinc-800/40 text-zinc-400"
                            : msg.status === "READ"
                              ? "bg-zinc-800/40 text-zinc-400"
                              : msg.status === "REPLIED"
                                ? "bg-emerald-950/30 border border-emerald-900/50 text-emerald-500/10 text-green-400"
                      ...
                      {msg.contactMethod && (
                        <span className="text-xs text-zinc-500 border border-white/10 px-2 py-0.5 rounded-none bg-white/5">
                          {msg.contactMethod}
                        </span>
                      )}
                    </div>
        ...
        >
          {selectedMessage ? (
            <>
              ...
              <div className="flex-none p-6 border-b border-white/10 bg-black flex flex-col gap-6">
                  ...
                  {selectedMessage.userPhone && (
                    <>
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 bg-[#229ED9]/10 text-[#229ED9] border border-[#229ED9]/20 rounded-none text-sm font-medium hover:bg-[#229ED9]/20 transition-all active:scale-95"
                      >
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 bg-[#25D366]/10 text-[#25D366] border border-[#25D366]/20 rounded-none text-sm font-medium hover:bg-[#25D366]/20 transition-all active:scale-95"
                      >
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                  {(["READ", "REPLIED", "ARCHIVED"] as const).map((status) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-none text-xs font-medium transition whitespace-nowrap border ${
                ...
                {selectedMessage.metadata && (
                  <div className="grid grid-cols-2 gap-4 text-sm bg-zinc-900/30 p-5 rounded-none border border-white/5">
                    ...
                    {selectedMessage.metadata.vin && (
                      <div>
                        <span className="text-zinc-500 block mb-1">VIN</span>{" "}
                    ...
                    {selectedMessage.metadata.budget && (
                      <div className="col-span-2 pt-3 border-t border-white/5">
                        <span className="text-zinc-500 block mb-1">Budget</span>{" "}
                        <span className="text-zinc-200 font-medium">
                          {selectedMessage.metadata.budget}
                        </span>
                      </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 bg-black">
                <div className="bg-zinc-900/20 rounded-none p-6 border border-white/5">
                  ...
                  <div className="mt-8 space-y-6">
                    ...
                    {selectedMessage.replies.map((reply, index) => (
                      <div key={index} className="ml-6 pl-6 border-l border-zinc-800 relative">
                        ...
                        <p className="text-base text-zinc-400 whitespace-pre-wrap leading-relaxed">
                          {reply.replyText}
                        </p>
              ...
              <div className="flex-none p-6 border-t border-white/10 bg-black">
                <div className="relative group">
                  <textarea
                    ...
                    className="w-full p-4 pr-16 bg-zinc-900/30 border border-white/10 rounded-none focus:outline-hidden focus:border-white/20 focus:bg-zinc-900/50 resize-none text-base min-h-[120px] transition-all placeholder:text-zinc-700"
                  />
                  <button
                    onClick={sendReply}
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-zinc-600 p-8 text-center">
              ...
              <p className="text-xl font-medium text-zinc-500 tracking-tight">
                No message selected
              </p>
              <p className="text-sm text-zinc-600 max-w-xs mt-3 font-light">
