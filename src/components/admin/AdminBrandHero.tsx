...
export function AdminBrandHero({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -left-32 -top-32 h-[28rem] w-[28rem] rounded-full opacity-60 blur-3xl"
        style={{ background: theme.bgMedium }}
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -right-40 bottom-[-40%] h-[32rem] w-[32rem] rounded-full opacity-40 blur-3xl"
        style={{ background: theme.bgMedium }}
      />
      ...
      <div className="relative flex flex-wrap items-start justify-between gap-6">
        <div className="min-w-0 flex-1 space-y-5">
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href="/admin/shop"
              className="inline-flex items-center gap-1.5 rounded-full border border-white/[0.08] bg-white/[0.03] px-3 py-1 text-[11px] font-medium uppercase tracking-wider text-zinc-400 transition hover:border-white/20 hover:text-zinc-200"
            >
            ...
            >
              ...
              <X className="h-3 w-3 opacity-70 transition group-hover:opacity-100" aria-hidden="true" />
            </button>
          </div>
          ...
          <div className="flex flex-col items-start gap-6 sm:flex-row sm:items-end">
            {theme.logoSrc ? (
              <div
                className="relative flex shrink-0 items-center justify-center overflow-hidden rounded-none border border-white/[0.08] bg-black/50 backdrop-blur-md"
                style={{
                  width: 'clamp(96px, 12vw, 160px)',
                  height: 'clamp(96px, 12vw, 160px)',
                }}
              >
                <div className="absolute inset-0 opacity-30" style={{ background: theme.bgMedium }} aria-hidden="true" />
                <Image
                  src={theme.logoSrc}
