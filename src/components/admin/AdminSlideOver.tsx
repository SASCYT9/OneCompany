...
export function AdminSlideOver({
  ...
  const panelClass = isBottom
    ? cn(
        "absolute inset-x-0 bottom-0 flex max-h-[85dvh] w-full flex-col overflow-hidden border-t border-white/6 bg-[#0F0F0F] shadow-[0_-30px_80px_rgba(0,0,0,0.7)]",
        "pb-[max(env(safe-area-inset-bottom),0.5rem)]"
      )
    : cn(
        "absolute inset-y-0 right-0 flex w-full flex-col overflow-hidden border-l border-white/6 bg-[#0F0F0F] shadow-[-30px_0_80px_rgba(0,0,0,0.7)]",
        widthClass
      );
      ...
      {open ? (
        <div className="fixed inset-0 z-90" role="dialog" aria-modal="true">
          ...
          >
            ...
            <header className="flex items-start justify-between gap-3 border-b border-white/5 bg-[#171717] px-5 py-4">
              ...
              <button
                ...
                onClick={onClose}
                aria-label="Close"
                className="-mr-1 rounded-none p-1.5 text-zinc-400 transition hover:bg-white/4 hover:text-zinc-100"
              >
            ...
            {footer ? (
              <footer className="border-t border-white/5 bg-[#171717] px-5 py-3">{footer}</footer>
            ) : null}
          </motion.div>
        </div>
      ) : null}
