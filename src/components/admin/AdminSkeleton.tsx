...
export function AdminSkeleton({
  ...
  return (
    <div
      className={cn("motion-safe:animate-pulse rounded-none bg-white/6", className)}
      style={style}
    />
  );
}
...
export function AdminSkeletonKpi() {
  return (
    <div className="rounded-none border border-white/5 bg-[#171717] px-5 pt-5 pb-3">
...
export function AdminSkeletonCard({ rows = 4, className }: { rows?: number; className?: string }) {
  return (
    <div className={cn("rounded-none border border-white/5 bg-[#171717] p-5", className)}>
      <div className="space-y-2 border-b border-white/4 pb-3">
...
export function AdminSkeletonTable({ rows = 5, cols = 5 }: { rows?: number; cols?: number }) {
  return (
    <div className="overflow-hidden rounded-none border border-white/5 bg-[#171717]">
      <table className="w-full">
        <thead className="border-b border-white/4">
          <tr>
            {Array.from({ length: cols }).map((_, i) => (
...
export function AdminSkeletonKpiGrid({ count = 4 }: { count?: number }) {
  return (
    <div
      className={cn("grid gap-3 md:grid-cols-2", count === 6 ? "xl:grid-cols-6" : "xl:grid-cols-4")}
    >
