...
import { CheckCircle, Inbox, MessageSquare, Search, Send, Trash2, X } from "lucide-react";
...
const statusCopy: Record<string, { label: string; badge: string }> = {
  ...
  archived: {
    label: "Архів",
    badge: "bg-zinc-200 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-100",
  },
};
  ...
  return (
    <div className="space-y-6 font-ua">
      ...
      <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
        <div className="rounded-none border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
          <div className="relative mb-4">
            ...
            <input
              ...
              className="w-full rounded-none border border-white/10 bg-black/20 py-2 pl-9 pr-4 text-sm text-white placeholder:text-white/40 focus:border-white/40 focus:outline-hidden"
            />
          </div>
            ...
            {filteredMessages.map((msg) => (
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`rounded-full px-3 py-1 text-[11px] uppercase tracking-[0.3em] ${statusCopy[msg.status]?.badge}`}
                  >
        ...
        <div
          className={`fixed inset-0 z-50 bg-black lg:static lg:bg-transparent ${selectedMessage ? "block" : "hidden lg:block"}`}
        >
          <div className="h-full rounded-none border-0 bg-black p-6 lg:rounded-none lg:border lg:border-white/10 lg:bg-black/30 lg:backdrop-blur-sm">
                ...
                >
                    ...
                    {selectedMessage.replies?.length > 0 && (
                      <div className="space-y-3">
                        ...
                        {selectedMessage.replies.map((reply, index) => (
                          <div
                            key={`${reply.repliedAt}-${index}`}
                            className="rounded-none border border-white/10 bg-white/5 p-3"
                          >
                  ...
                  <div className="mt-6 rounded-none border border-white/10 bg-white/5 p-4">
                    <textarea
                      ...
                      className="w-full rounded-none border border-white/10 bg-black/40 p-3 text-sm text-white placeholder:text-white/40 focus:border-white/40 focus:outline-hidden"
                    />
...
function StatCard({
  ...
  return (
    <div
      className={`rounded-none border ${accent ?? "border-white/10"} bg-white/5 p-4 text-white backdrop-blur-sm`}
    >
