...
export function AdminActivityTimeline({
    ...
    async function load() {
      ...
      try {
        const response = await fetch(
          `/api/admin/audit-log/${encodeURIComponent(entityType)}/${encodeURIComponent(entityId)}`,
          {
            cache: "no-store",
          }
        );
  ...
  if (error) {
    return (
      <div className={className}>
        <div className="rounded-none border border-red-500/20 bg-red-500/5 p-3 text-sm text-red-300">
          {error}
        </div>
      </div>
    );
  ...
  if (entries.length === 0) {
    return (
      <div className={className}>
        <div className="rounded-none border border-dashed border-white/8 bg-black/25 p-6 text-center">
  ...
  return (
    <div className={className}>
      <ol className="relative space-y-4 pl-6">
        {/* Vertical line */}
        <div
          className="absolute left-2 top-2 bottom-2 w-px bg-linear-to-b from-white/10 via-white/5 to-transparent"
          aria-hidden="true"
        />
        ...
        {entries.map((entry, idx) => {
          ...
          return (
            <li key={entry.id} className="relative">
              {/* Dot */}
              <div className="absolute left-[-18px] top-1 flex h-4 w-4 items-center justify-center rounded-full border border-white/10 bg-[#0F0F0F]">
              ...
              {idx === 0 ? (
                <div
                  className="absolute left-[-20px] top-0 h-2 w-2 motion-safe:animate-ping rounded-full bg-blue-500/40"
                  aria-hidden="true"
                />
              ) : null}
            </li>
...
function ActionIcon({ action }: { action: string }) {
  if (action.includes("status"))
    return <ArrowRight className="h-2.5 w-2.5 text-blue-400" aria-hidden="true" />;
  if (action.includes("payment"))
    return <CheckCircle2 className="h-2.5 w-2.5 text-emerald-400" aria-hidden="true" />;
  if (action.includes("shipment") || action.includes("ship"))
    return <Truck className="h-2.5 w-2.5 text-amber-300" aria-hidden="true" />;
  if (action.includes("order"))
    return <ShoppingCart className="h-2.5 w-2.5 text-blue-400" aria-hidden="true" />;
  if (action.includes("product"))
    return <Package className="h-2.5 w-2.5 text-blue-400" aria-hidden="true" />;
  if (action.includes("customer"))
    return <UserCheck className="h-2.5 w-2.5 text-emerald-400" aria-hidden="true" />;
...
function ActionDetail({
  ...
  if (typeof metadata.fromStatus === "string" && typeof metadata.toStatus === "string") {
    return (
      <div className="mt-1.5 flex flex-wrap items-center gap-1.5 text-xs">
        <span className="rounded-full border border-white/6 bg-white/3 px-1.5 py-0.5 font-mono text-[10px] text-zinc-400">
          {metadata.fromStatus}
        </span>
        <ArrowRight className="h-3 w-3 text-zinc-600" aria-hidden="true" />
        <span className="rounded-full border border-blue-500/25 bg-blue-500/8 px-1.5 py-0.5 font-mono text-[10px] text-blue-300">
          {metadata.toStatus}
        </span>
      </div>
    );
  ...
  if (
    metadata.updates &&
    typeof metadata.updates === "object" &&
      ...
      return (
        <div className="mt-1.5 flex flex-wrap gap-1">
          {keys.slice(0, 4).map((k) => (
            <span
              key={k}
              className="rounded-full border border-white/6 bg-white/3 px-1.5 py-0.5 font-mono text-[10px] text-zinc-400"
            >
              {k}
            </span>
          ))}
...
function actionLabel(action: string): string {
  return (
    ...
    action.replace(/[._]/g, " ").replace(/(^|\s)\w/g, (l) => l.toUpperCase())
  );
}
