...
const INPUT_BASE =
  "w-full rounded-none border border-white/8 bg-[#171717] px-3.5 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 transition-all duration-150 focus:border-blue-500/50 focus:bg-[#1A1A1A] focus:outline-hidden focus:ring-2 focus:ring-blue-500/15 disabled:cursor-not-allowed disabled:opacity-50";
  ...
  return (
    <label className={cn("block", className)}>
      ...
      {hasAffix ? (
        <div
          className={cn(
            "flex items-stretch overflow-hidden rounded-none border border-white/8 bg-[#171717] transition-all duration-150 focus-within:border-blue-500/50 focus-within:bg-[#1A1A1A] focus-within:ring-2 focus-within:ring-blue-500/15",
            disabled && "cursor-not-allowed opacity-50"
          )}
        >
          {prefix ? (
            <span className="flex items-center border-r border-white/6 bg-black/30 px-3 text-xs font-semibold uppercase tracking-wider text-blue-400">
              {prefix}
            </span>
          ) : null}
          <input
            ...
            disabled={disabled}
            className={cn(
              "min-w-0 flex-1 bg-transparent px-3.5 py-2.5 text-sm text-zinc-100 placeholder:text-zinc-600 focus:outline-hidden disabled:cursor-not-allowed",
              mono && "font-mono text-[13px] tabular-nums"
            )}
          />
          {suffix ? (
            <span className="flex items-center border-l border-white/6 bg-black/30 px-3 text-xs font-semibold uppercase tracking-wider text-zinc-400">
              {suffix}
            </span>
          ) : null}
        </div>
      ...
      <div className="relative">
        <select
          ...
          className={cn(
            ...
            "[&>option]:bg-[#171717] [&>option]:text-zinc-100"
          )}
        >
          {options.map((option) => (
