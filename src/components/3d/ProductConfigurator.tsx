...
function Scene({ config }: { config: SuspensionConfig }) {
  return (
    <>
      ...
      <SuspensionModel color={config.color} height={config.height} damping={config.damping} />
...
export default function ProductConfigurator() {
  ...
  return (
    <div className="relative w-full h-screen bg-linear-to-b from-slate-900 via-black to-slate-900">
      {/* 3D Viewer */}
      ...
      <motion.div
        initial={{ x: "100%" }}
        animate={{ x: 0 }}
        className="absolute right-0 top-0 h-full w-full md:w-1/3 bg-linear-to-b from-slate-900/95 to-black/95 backdrop-blur-xl border-l border-white/10 overflow-y-auto"
      >
        <div className="p-8">
          {/* Header */}
          <div className="mb-8">
          ...
          <div className="mb-8">
            <label className="text-white font-bold mb-4 block">
              Ride Height: {config.height > 0 ? "+" : ""}
              {config.height.toFixed(1)} cm
            </label>
            <input
              type="range"
              min="-3"
          ...
          {/* Price & CTA */}
          <div className="sticky bottom-0 bg-linear-to-t from-black via-black to-transparent pt-6 pb-4">
            ...
            <button className="w-full py-4 bg-linear-to-r from-amber-500 to-orange-500 text-white font-bold rounded-xl shadow-2xl shadow-amber-500/50 hover:shadow-amber-500/80 hover:scale-105 transition-all">
              Add to Cart
            </button>

            <button className="w-full mt-3 py-4 bg-white/10 backdrop-blur-xs text-white font-bold rounded-xl border border-white/20 hover:bg-white/20 transition-all">
              Save Configuration
            </button>
          </div>
        </div>
      ...
      {/* Mobile Toggle */}
      <button className="md:hidden absolute top-4 right-4 z-50 p-4 bg-black/80 backdrop-blur-xs text-white rounded-full border border-white/20">
