...
export function Navigation({ currentBrand = "home" }: NavigationProps) {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 border-b backdrop-blur-xl ${getBrandColor()} transition-all duration-300`}
    >
            ...
            {brands.map((brand) => (
              <Link
                ...
                className={`group relative px-4 py-2 rounded-lg transition-all duration-300 ${
                  pathname === brand.href
                    ? `bg-linear-to-r ${brand.color} text-white shadow-lg`
                    : "text-white/70 hover:text-white hover:bg-white/5"
                }`}
              >
                <span className="font-medium">{brand.name}</span>
                {pathname === brand.href && (
                  <div className="absolute -bottom-1 left-0 right-0 h-0.5 bg-linear-to-r from-transparent via-white to-transparent" />
                )}
              </Link>
            ))}
            <Link
            ...
            {brands.map((brand) => (
              <Link
                ...
                className={`block px-4 py-3 rounded-lg transition-all ${
                  pathname === brand.href
                    ? `bg-linear-to-r ${brand.color} text-white`
