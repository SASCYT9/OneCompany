    ...
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled || mobileMenuOpen
          ? "bg-white/95 dark:bg-black/95 backdrop-blur-xs border-b border-zinc-200 dark:border-white/10"
          : "bg-transparent"
      }`}
    >
      <div className="container mx-auto px-6 py-6">
        <div className="flex items-center justify-between">
          ...
          <Link
            href="/"
            className="text-xl font-light text-zinc-900 dark:text-white tracking-wider hover:opacity-70 transition-opacity"
          >
            ONECOMPANY
          </Link>
            ...
            <div className="flex items-center gap-1 border border-zinc-300 dark:border-white/20 rounded w-24">
              <button
                ...
                className={`px-3 py-1.5 text-xs font-light uppercase tracking-wider transition-all ${
                  locale === "ua"
                    ? "bg-black text-white dark:bg-white dark:text-black"
                    : "text-zinc-500 dark:text-white/60 cursor-pointer"
                }`}
              >
                UA
              </button>
