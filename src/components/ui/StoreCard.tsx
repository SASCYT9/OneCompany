  ...
  return (
    ...
    >
      ...
      <div className="relative h-full overflow-hidden rounded-3xl border border-white/10 bg-black/50 backdrop-blur-2xl transition-all duration-700 hover:border-white/30 hover:scale-[1.03] card-glow">
        ...
        <div
          className={`absolute inset-0 bg-linear-to-br ${color} opacity-0 group-hover:opacity-30 transition-opacity duration-700 blur-3xl gradient-shift`}
        />

        {/* Animated border gradient - neon effect */}
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700">
          <div
            className={`absolute inset-[-3px] bg-linear-to-br ${color} rounded-3xl blur-lg -z-10`}
            style={{
        ...
        <div className="relative h-full flex flex-col p-10">
          ...
          <div className="mb-8">
            <span
              className={`px-5 py-2 rounded-full bg-linear-to-r ${color} bg-opacity-10 backdrop-blur-md border border-white/20 text-xs font-medium text-white uppercase tracking-[0.3em] shadow-lg transition-all duration-300 group-hover:shadow-2xl group-hover:scale-105`}
            >
              {category}
            </span>
          </div>
          ...
          <div className="mb-6">
            <p
              className={`text-base font-light uppercase tracking-[0.4em] bg-linear-to-r ${color} bg-clip-text text-transparent transition-all duration-500 group-hover:tracking-[0.5em] neon-pulse`}
            >
              {tagline}
            </p>
          </div>
          ...
          <div className="relative mb-4">
            <div
              className={`flex items-center gap-4 text-white transition-all duration-700 ${isHovered ? "translate-x-0 opacity-100" : "-translate-x-8 opacity-0"}`}
            >
              <div
                className={`h-[2px] flex-1 bg-linear-to-r ${color} transition-all duration-700 shadow-lg ${isHovered ? "w-full shadow-2xl" : "w-0"}`}
              />
              <span className="text-xl font-light uppercase tracking-[0.3em] whitespace-nowrap">
                Відкрити магазин
              </span>
              <span className="text-3xl transform transition-all duration-500 group-hover:translate-x-3 group-hover:scale-125">
                →
              </span>
            </div>
          </div>
          ...
          <div
            className={`transition-all duration-700 ${isHovered ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}
          >
        ...
        >
          <div
            className={`absolute inset-0 bg-linear-to-b from-transparent via-white/10 to-transparent animate-scan`}
          />
        </div>
        ...
        <div className="absolute top-0 right-0 w-48 h-48 overflow-hidden rounded-tr-3xl">
          <div
            className={`absolute top-0 right-0 w-full h-full bg-linear-to-br ${color} opacity-20 blur-3xl transform rotate-45 translate-x-24 -translate-y-24 transition-all duration-700 group-hover:translate-x-12 group-hover:-translate-y-12 group-hover:opacity-40`}
          />
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-1 overflow-hidden rounded-b-3xl">
          <div
            className={`h-full bg-linear-to-r ${color} transition-all duration-700 ${isHovered ? "w-full opacity-100" : "w-0 opacity-0"}`}
          />
        </div>
      </div>
    </Link>
