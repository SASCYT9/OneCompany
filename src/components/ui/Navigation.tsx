...
export function Navigation() {
  ...
  const navCtaClass =
    "cta-primary inline-flex min-h-[44px] min-w-38 items-center justify-center whitespace-nowrap px-4 py-2.5 text-sm tracking-wide ease-soft-out";
  const navOutlineClass =
    "cta-outline inline-flex min-h-[44px] min-w-34 items-center justify-center whitespace-nowrap px-4 py-2.5 text-sm text-white/90 hover:bg-white hover:text-black focus:outline-hidden focus-visible:ring-2 focus-visible:ring-white/40";
  ...
  return (
    <nav
      ...
      style={{
        background: isScrolled ? "rgba(0, 0, 0, 0.4)" : "rgba(0, 0, 0, 0.1)",
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="hidden sm:block text-[11px] font-light text-white/60 tracking-wide">
              {locale === "ua"
                ? `З ${foundedYear} • ${years}+ років довіри`
                : `Since ${foundedYear} • ${years}+ years trusted`}
            </span>
          </div>
          <div className="hidden md:flex items-center gap-4">
              ...
              <div className="absolute top-full left-0 mt-3 w-72 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-500 group-hover:translate-y-0 -translate-y-2">
                <div
                  ...
                  style={{
                    ...
                    boxShadow:
                      "inset 0 1px 0 0 rgba(255,255,255,0.1), 0 12px 48px rgba(0,0,0,0.6), 0 0 0 1px rgba(255,255,255,0.08)",
                  }}
                >
                  {stores.map((store) => (
                    <Link
                    ...
                    >
                      <div className="absolute inset-0 bg-linear-to-r from-transparent via-white/5 to-transparent opacity-0 group-hover/item:opacity-100 transition-opacity duration-500" />
                      <div className="relative z-10 flex items-center justify-between">
                        ...
                        <span
                          className={
                            ...
                            store.color +
                            " opacity-0 group-hover/item:opacity-100 transition-all duration-400 group-hover/item:scale-125"
                          }
            ...
            {extraLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className={pathname === link.href ? navActiveClass : navGhostClass}
              >
            ...
            <div className="inline-flex min-h-[44px] items-center gap-1 rounded-full bg-white/5 px-3 py-2">
              <button
                ...
                className={
                  "px-3 py-1 text-sm font-light rounded-full transition-all duration-300 " +
                  (locale === "ua" ? "text-white bg-white/10" : "text-white/60 hover:text-white/80")
                }
              >
                UA
              </button>
              <button
                ...
                className={
                  "px-3 py-1 text-sm font-light rounded-full transition-all duration-300 " +
                  (locale === "en" ? "text-white bg-white/10" : "text-white/60 hover:text-white/80")
                }
              >
                EN
              </button>
            </div>
            <button onClick={() => setContactOpen(true)} className={navOutlineClass}>
          ...
          <div className="flex flex-col gap-4 py-4">
            <div className="flex items-center gap-2 px-4">
              <button
                onClick={() => setLocale("ua")}
                className={
                  "px-4 py-2 text-sm font-light rounded-full transition-all duration-300 " +
                  (locale === "ua" ? "text-white bg-white/10" : "text-white/60 hover:text-white/80")
                }
              >
                UA
      ...
      {contactOpen && (
        <div className="fixed inset-0 z-70 flex items-center justify-center p-4">
          <div
            ...
            onClick={() => (formState === "submitting" ? null : setContactOpen(false))}
          />
          <div
            className="relative z-10 w-full max-w-lg overflow-hidden"
            style={{
              background: "linear-gradient(165deg, rgba(22,22,26,0.92), rgba(12,12,16,0.92))",
            }}
          >
            <div className="p-8">
              <div className="flex items-center justify-between mb-6">
                ...
                <button
                  ...
                  className="text-white/50 hover:text-white transition-colors focus:outline-hidden focus-visible:ring-2 focus-visible:ring-white/40 rounded-sm"
              ...
              <div className="flex gap-3 mb-6">
                <button
                  ...
                  className={
                    ...
                    (payload.type === "auto"
                      ? "bg-linear-to-r from-orange-500/20 to-red-500/20 text-white shadow-lg shadow-orange-500/20"
                ...
                <button
                  ...
                  className={
                    "flex-1 py-3 px-4 rounded-lg text-sm font-medium transition-all duration-300 " +
                    (payload.type === "moto"
                      ? "bg-linear-to-r from-blue-500/20 to-cyan-500/20 text-white shadow-lg shadow-cyan-500/20"
                      : "bg-white/5 text-white/60 hover:bg-white/10 hover:text-white/80")
                  }
                >
              ...
              <form onSubmit={submit} className="space-y-4">
                <input
                  ...
                  className="w-full bg-white/5 text-white px-5 py-3 outline-hidden text-sm placeholder:text-white/30 rounded-md"
                  value={payload.name}
                ...
                <input
                  ...
                  className="w-full bg-white/5 text-white px-5 py-3 border border-white/10 focus:border-white/30 outline-hidden text-sm placeholder:text-white/30 rounded-md"
                ...
                <textarea
                  ...
                  rows={5}
                  className="w-full bg-white/5 text-white px-5 py-3 border border-white/10 focus:border-white/30 outline-hidden text-sm placeholder:text-white/30 resize-none rounded-md"
                ...
                <button
                  type="submit"
                  disabled={formState === "submitting"}
                  className="w-full py-3.5 rounded-full text-sm tracking-wider uppercase font-medium bg-white text-black hover:bg-white/90 transition-colors disabled:opacity-50 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-white/40"
                >
                ...
                {formState === "success" && (
                  <p className="text-center text-xs text-emerald-400">
                    {locale === "ua" ? "Надіслано успішно" : "Sent successfully"}
                  </p>
                )}
                {formState === "error" && (
                  <p className="text-center text-xs text-red-400">
                    {locale === "ua" ? "Помилка надсилання" : "Send error"}{" "}
                    {errorMsg && " • " + errorMsg}
                  </p>
                )}
              </form>
