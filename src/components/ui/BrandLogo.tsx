...
export const BrandLogo: React.FC<BrandLogoProps> = ({
  ...
  return (
    <div
      ref={ref}
      className={`relative flex items-center justify-center ${heightClass} ${className}`}
    >
      {shimmer && !loaded && (
        <div className="absolute inset-0 rounded-md overflow-hidden">
          <div className="w-full h-full bg-linear-to-r from-white/5 via-white/10 to-white/5 animate-[shimmer_1.6s_linear_infinite]" />
        </div>
      )}
      {inView && (
        <img
      ...
      <style jsx>{`
        ...
        div:has(> .animate-[shimmer_1.6s_linear_infinite])::after {
          display: none;
        }
      `}</style>
    </div>
