...
export const BrandLogo: React.FC<BrandLogoProps> = ({ name, src, className = '', alt, variant = 'default', monochrome = true, lazy = true, shimmer = true }) => {
    ...
    <div ref={ref} className={`relative flex items-center justify-center ${heightClass} ${className}`}>
      ...
      {inView && (
        <img
          src={current}
          alt={alt || name}
          className={`max-h-full max-w-full object-contain transition-all duration-500 ${loaded ? 'opacity-90 hover:opacity-100' : 'opacity-0'} ${monochrome ? 'grayscale contrast-125 hover:grayscale-0 hover:contrast-100' : ''}`}
