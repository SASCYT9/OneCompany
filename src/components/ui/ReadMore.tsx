...
export default function ReadMore({
  ...
  return (
    <div className="relative">
      <div
        className={`relative overflow-hidden transition-all duration-500 ${isOpen ? "max-h-full" : "max-h-[120px]"}`}
      >
        {children}

        {!isOpen && (
          <div className="absolute bottom-0 left-0 right-0 h-24 bg-linear-to-t from-white dark:from-black to-transparent" />
        )}
      </div>
