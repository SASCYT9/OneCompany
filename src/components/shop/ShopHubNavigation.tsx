...
import { ArrowDown, ArrowUpRight, LayoutGrid, Tags } from "lucide-react";
...
export function ShopHubNavigation({ locale, active, className }: ShopHubNavigationProps) {
  ...
  const destinations = getShopNavigationDestinations(locale).filter(
  ...
  return (
    <nav
      ...
      className={cn(
        "grid grid-cols-2 gap-1.5 rounded-[24px] border border-white/10 bg-[#0b0b0c]/96 p-1.5 text-white shadow-[0_22px_60px_rgba(0,0,0,0.28)] backdrop-blur-xl",
        className
      )}
    >
