...
13. The confirmed iPE cat-back system for the LCI BMW X5 M F95 / X6 M F96 is now part of the warehouse carousel and stock filter. The inventory contract matches both its canonical SKU `0WX595-NVNM0-2` and exact canonical slug instead of applying a broad brand/model rule. The carousel receives dedicated bilingual presentation copy while retaining the product's canonical media, price and URL. Browser verification shows 12 deduplicated warehouse products and the iPE product returned as in stock.
14. The trailing “All in-stock products” action duplicated the catalog filter and did not preserve its intended filtered state. It was removed rather than leaving a misleading control; the product rail now uses the full hero width and the carousel remains the single interaction surface for the confirmed warehouse selection.
